...
if ( ! defined( 'BULKEMAIL_VERSION' ) ) {
	wp_die( 'Please activate the Bulkmail Plugin!' );
}

$interval = isset( $_GET['interval'] ) ? (int) $_GET['interval'] : bulkmail_option( 'interval', 5 ) * 60;
if ( $request_url ) {
...
<head>
	...
	<title>Bulkmail <?php echo esc_html( BULKEMAIL_VERSION ); ?> - Cron</title>
...
if ( ( isset( $_GET[ $secret ] ) ) ||
	...
	( defined( 'BULKEMAIL_CRON_SECRET' ) && BULKEMAIL_CRON_SECRET == $secret ) ) :

	// spawn wp_cron if it should
	if ( wp_next_scheduled( 'bulkmail_cron' ) - $time < 0 ) {
		spawn_cron();
	}

	if ( bulkmail_option( 'cron_service' ) != 'cron' ) {
		echo '<h2>' . esc_html__( 'WordPress Cron in use!', 'bulkmail' ) . '</h2>';
	} else {
	...
	<script type="text/javascript">
		...
		window.addEventListener('load', function () {
			if(!finished) document.getElementById('info').innerHTML = '<h2>Your servers execution time has been exceed!</h2><p>No worries, emails still get sent. But it\'s recommended to increase the "max_execution_time" for your server, add <code>define("WP_MEMORY_LIMIT", "256M");</code> to your wp-config.php file  or decrease the <a href="<?php echo admin_url( '/edit.php?post_type=newsletter&page=bulkmail_settings' ); ?>#delivery" target="_blank" rel="noopener">number of mails sent</a> maximum in the settings!</p><p><a onclick="location.reload();" class="button" id="button">OK, now reload</a></p>';
		});

	</script>
	<div id="info"><p><?php esc_html_e( 'progressing', 'bulkmail' ); ?>&hellip;</p></div>
		<?php

		if ( isset( $argv[2] ) ) {
			$worker = $argv[2];
		} else {
			$worker = get_query_var( '_bulkmail_extra' );
		}
		if ( empty( $worker ) ) {
			do_action( 'bulkmail_cron_autoresponder' );
			do_action( 'bulkmail_cron_worker' );
			do_action( 'bulkmail_cron_bounce' );
			do_action( 'bulkmail_cron_cleanup' );
		} elseif ( in_array( $worker, apply_filters( 'bulkmail_cron_workers', array( 'autoresponder', 'worker', 'bounce', 'cleanup' ) ) ) ) {
			echo '<h2>' . esc_html__( 'Single Cron', 'bulkmail' ) . ': ' . ucwords( $worker ) . '</h2>';
			do_action( 'bulkmail_cron_' . $worker );
		} else {
			echo '<h2>' . esc_html__( 'Invalid Cron Worker!', 'bulkmail' ) . '</h2>';
		}
		?>
	<p>
		<a onclick="location.reload();clearInterval(i);" class="button" id="button"><?php esc_html_e( 'reload', 'bulkmail' ); ?></a>
	</p>
	<p>
		<small><?php echo $time = round( microtime( true ) - $time, 4 ); ?> <?php esc_html_e( 'sec', 'bulkmail' ); ?>.</small>
	</p>
...
<?php else : ?>
	<h2><?php esc_html_e( 'The Secret is missing or wrong!', 'bulkmail' ); ?></h2>
<?php endif; ?>
...
var a = <?php echo floor( $interval ); ?>,
	...
	f = setInterval(function(){
		...
		if(x<=0){
			o = '&#x27F2;';
			p = '<?php esc_html_e( 'progressing', 'bulkmail' ); ?>';
			clearInterval(f);
		}
	document.title = p+' '+c;
	b.innerHTML = d+' ('+o+')';
...
if ( $simple_output ) :
	...
	$output = bulkmail( 'helper' )->plain_text( $output );
	echo $output;
endif;
