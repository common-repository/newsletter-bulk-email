...
<?php if ( $editable ) : ?>
	...
	<p>
		<label>
		<input name="bulkmail_data[track_opens]" id="bulkmail_data_track_opens" value="1" type="checkbox" <?php echo ( isset( $this->post_data['track_opens'] ) ) ? ( ( $this->post_data['track_opens'] ) ? 'checked' : '' ) : ( bulkmail_option( 'track_opens' ) ? 'checked' : '' ); ?>> <?php esc_html_e( 'Track Opens', 'bulkmail' ); ?>
		</label>
	</p>
	<p>
		<label>
		<input name="bulkmail_data[track_clicks]" id="bulkmail_data_track_clicks" value="1" type="checkbox" <?php echo ( isset( $this->post_data['track_clicks'] ) ) ? ( ( $this->post_data['track_clicks'] ) ? 'checked' : '' ) : ( bulkmail_option( 'track_clicks' ) ? 'checked' : '' ); ?>> <?php esc_html_e( 'Track Clicks', 'bulkmail' ); ?>
		</label>
	</p>
	<label><?php esc_html_e( 'Colors', 'bulkmail' ); ?></label> <a class="savecolorschema"><?php esc_html_e( 'save this schema', 'bulkmail' ); ?></a>
	...
	foreach ( $current_colors as $i => $color ) {
		...
		?>
	<li class="bulkmail-color" id="bulkmail-color-<?php echo strtolower( substr( $value, 1 ) ); ?>">
	...
	<input type="text" class="form-input-tip color" name="bulkmail_data[newsletter_color][<?php echo substr( esc_attr( $color ), 1 ); ?>]"  value="<?php echo esc_attr( $value ); ?>" data-value="<?php echo esc_attr( $value ); ?>" data-default-color="<?php echo esc_attr( $value ); ?>">
	<a class="default-value bulkmail-icon" href="#" tabindex="-1"></a>
	</li>
		<?php
	}
	?>
	...
	<div class="clear"></div>
	<p>
		<label><?php esc_html_e( 'Colors Schemas', 'bulkmail' ); ?></label>
		<?php
		$customcolors = get_option( 'bulkmail_colors' );
		if ( isset( $customcolors[ $this->get_template() ] ) ) :
			?>
			<a class="colorschema-delete-all"><?php esc_html_e( 'Delete all custom schemas', 'bulkmail' ); ?></a>
		<?php endif; ?>
	</p>
	<ul class="colorschema" title="<?php esc_attr_e( 'original', 'bulkmail' ); ?>">
	<?php
	...
	<?php if ( strtolower( implode( '', $original_colors_temp ) ) != strtolower( implode( '', $current_colors ) ) ) : ?>
		<ul class="colorschema" title="<?php esc_attr_e( 'current', 'bulkmail' ); ?>">
...
<?php else : ?>
	...
	<?php esc_html_e( 'Track Opens', 'bulkmail' ); ?>
	</p>
	<p>
	<?php echo $this->post_data['track_clicks'] ? '&#10004;' : '&#10004;'; ?>
	<?php esc_html_e( 'Track Clicks', 'bulkmail' ); ?>
	</p>
	<label><?php esc_html_e( 'Colors Schema', 'bulkmail' ); ?></label><br>
