bulkmail = (function (bulkmail, $, window, document) {
	...
	outputnav.on('click', 'a.nav-tab', function () {
		...
		if (hash == '#systeminfo') {
			...
			textarea.val('...');
			bulkmail.util.ajax('get_system_info', function (response) {

				if (response.log)
					bulkmail.log(response.log);
	...
	function test(test_id) {
		...
		bulkmail.util.ajax('test', {
			...
			if (response.nexttest && !single_test) {
				...
				testinfo.html(bulkmail.util.sprintf(bulkmail.l10n.tests.running_test, tests_run, response.total, response.next));
			} else {
				progressbar.width('100%');
				setTimeout(function () {
					start_button.html(bulkmail.l10n.tests.restart_test).show();
					progress.hide();
					progressbar.width(0);
					testinfo.html(bulkmail.util.sprintf(bulkmail.l10n.tests.tests_finished, errors.error, errors.warning, errors.notice));
				}, 500);
			}
