bulkmail = (function (bulkmail, $, window, document) {
	...
	var nav = $('.mainnav'),
		...
		wpnonce = $('#bulkmail_nonce').val(),
		reservedtags = $('#reserved-tags').data('tags');

	$('form#bulkmail-settings-form')
		.on('submit.lock', function () {
			return false
		});
	if ($('#settingsloaded').length) {
		$('.submit-form').prop('disabled', false);
		$('form#bulkmail-settings-form').off('submit.lock');
	} else {
		bulkmail.log('error loading settings page');
	}
	...
	deliverynav.on('click', 'a.nav-tab', function () {
		...
		$('input.bulkmail_sendtest').val(bulkmail.l10n.settings.save_to_test).prop('disabled', true);
		return false;
	});
	...
	nav.on('click', 'a', function () {
		...
		location.hash = hash;
		$('form#bulkmail-settings-form').attr('action', 'options.php' + hash);
	...
	$('.system_mail').on('change', function () {
		$('[name="bulkmail_options[system_mail_template]"]').prop('disabled', $(this).val() == 0);
		$('[name="bulkmail_options[respect_content_type]"]').prop('disabled', $(this).val() == 0);
	});
	...
	$('#load_location_db').on('click', function () {
		...
		bulkmail.util.ajax('load_geo_data', function (response) {
	...
	$('#social-services')
		.on('change', '.social-service-dropdown', function () {
			...
			_this.next('.social-service-url-field').html('<label><span class="description">' + _url.replace('%s', '<input type="text" name="bulkmail_options[services][' + _this.val() + ']" value="" class="regular-text">') + '</span></label>').find('input').focus();
	...
	$('#bulkmail_spf').on('change', function () {
		($(this).is(':checked')) ?
		$('.spf-info').slideDown(200): $('.spf-info').slideUp(200);
	});
	$('#bulkmail_dkim').on('change', function () {
		($(this).is(':checked')) ?
	...
	$('#bulkmail_generate_dkim_keys').on('click', function () {
		return ($('#dkim_keys_active').length && confirm(bulkmail.l10n.settings.create_new_keys));
		return false;
	});

	$('input.smtp.secure').on('change', function () {
		$('#bulkmail_smtp_port').val($(this).data('port'));
	});
	...
	$('.bulkmail_sendtest').on('click', function () {
		var $this = $(this),
			cont = $this.closest('.bulkmail-testmail'),
			loader = cont.find('.test-ajax-loading').css({
				'visibility': 'visible'
			}),
			to = cont.find('input.bulkmail-testmail-email').val(),
			formdata = $('form#bulkmail-settings-form').serialize();
		...
		bulkmail.util.ajax('send_test', {
			...
			if (response.log)
				response.success ? bulkmail.log(response.log) : bulkmail.log(response.log, 'error');
	...
	$('.bulkmail_bouncetest').on('click', function () {
		var $this = $(this),
		...
		bulkmail.util.ajax('bounce_test', {
			formdata: formdata
		}, function (response) {
	...
	$('#bulkmail_add_tag').on('click', function () {
	...
	$('.tags')
		.on('change', '.tag-key', function () {
			...
			_this.val(val);
			_base.find('.tag-value').attr('name', 'bulkmail_options[custom_tags][' + val + ']');
	...
	$('#bulkmail_add_field').on('click', function () {

		var el = $('<div class="customfield"><a class="customfield-move-up" title="' + bulkmail.l10n.settings.move_up + '">&#9650;</a><a class="customfield-move-down" title="' + bulkmail.l10n.settings.move_down + '">&#9660;</a><div><span class="label">' + bulkmail.l10n.settings.fieldname + ':</span><label><input type="text" class="regular-text customfield-name"></label></div><div><span class="label">' + bulkmail.l10n.settings.tag + ':</span><span><code>{</code><input type="text" class="customfield-key code"><code>}</code></span></div><div><span class="label">' + bulkmail.l10n.settings.type + ':</span><select class="customfield-type"><option value="textfield">' + bulkmail.l10n.settings.textfield + '</option><option value="textarea">' + bulkmail.l10n.settings.textarea + '</option><option value="dropdown">' + bulkmail.l10n.settings.dropdown + '</option><option value="radio">' + bulkmail.l10n.settings.radio + '</option><option value="checkbox">' + bulkmail.l10n.settings.checkbox + '</option><option value="date">' + bulkmail.l10n.settings.datefield + '</option></select></div><ul class="customfield-additional customfield-dropdown customfield-radio"><li><ul class="customfield-values"><li><span>&nbsp;</span> <span class="customfield-value-box"><input type="text" class="regular-text customfield-value" value=""> <label><input type="radio" value="" title="' + bulkmail.l10n.settings.default_selected + '" class="customfield-default" disabled> ' + bulkmail.l10n.settings.default_field + '</label></span></li></ul><span>&nbsp;</span> <a class="customfield-value-add">' + bulkmail.l10n.settings.add_field + '</a></li></ul><div class="customfield-additional customfield-checkbox"><span>&nbsp;</span><label><input type="checkbox" value="1" title="' + bulkmail.l10n.settings.default_field + '" class="customfield-default" disabled> ' + bulkmail.l10n.settings.default_checked + '</label></div><a class="customfield-remove">remove field</a><br></div>').appendTo($('.customfields').eq(0));
	...
	$('#sync_list')
		...
		.on('click', '#sync_subscribers_wp', function () {
			if (event.target == this && !confirm(bulkmail.l10n.settings.sync_subscriber)) return false;
			...
			bulkmail.util.ajax('sync_all_subscriber', {
		...
		.on('click', '#sync_wp_subscribers', function () {
			if (event.target == this && !confirm(bulkmail.l10n.settings.sync_wp_user)) return false;
			...
			bulkmail.util.ajax('sync_all_wp_user', {
				offset: _this.data('offset')
			}, function (response) {
	...
	$('.customfields')
		...
		.on('change', '.customfield-key', function () {
			...
			_this.val(val);
			_base.find('.customfield-name').attr('name', 'bulkmail_options[custom_field][' + val + '][name]');
			_base.find('.customfield-type').attr('name', 'bulkmail_options[custom_field][' + val + '][type]');
			_base.find('.customfield-value').attr('name', 'bulkmail_options[custom_field][' + val + '][values][]');
			_base.find('.customfield-default').attr('name', 'bulkmail_options[custom_field][' + val + '][default]');
	...
	$('#bulkmail_import_data').on('click', function () {
		if (!confirm(bulkmail.l10n.settings.import_data)) return false;
	});
	$('#bulkmail_reset_data').on('click', function () {
		if (!confirm(bulkmail.l10n.settings.reset_data)) return false;
	});
	...
	function bounce_test_check(identifier, count, formdata, callback) {
		...
		bulkmail.util.ajax('bounce_test_check', {
	...
	function load_spf_data() {
		...
		bulkmail.util.ajax('spf_check', function (response) {
	...
	function load_dkim_data() {
		...
		bulkmail.util.ajax('dkim_check', function (response) {
	...
	function sanitize(string) {
		...
		if ($.inArray(tag, reservedtags) != -1) {
			alert(bulkmail.util.sprintf(bulkmail.l10n.settings.reserved_tag, '"' + tag + '"'));
			tag += '-a';
		}
		return tag;
	}
