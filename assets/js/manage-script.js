bulkmail = (function (bulkmail, $, window, document) {
	...
	var importstatus = $('.import-status'),
		...
		uploader_init = function () {
			...
			uploader.bind('UploadProgress', function (up, file) {
				importstatus.html(bulkmail.util.sprintf(bulkmail.l10n.manage.uploading, file.percent + '%'));
			...
			uploader.bind('UploadComplete', function (up, files) {
				importstatus.html(bulkmail.l10n.manage.prepare_data);
	...
	typeof wpUploaderInit == 'object' && bulkmail.events.push('documentReady', uploader_init);
	...
	$('.wrap')
		...
		.on('click', '.do-import', function () {
			...
			if (!/%5D=email/.test(order)) {
				alert(bulkmail.l10n.manage.select_emailcolumn);
				return false;
			}
			if (!$('input[name="status"]:checked').length) {
				alert(bulkmail.l10n.manage.select_status);
				return false;
			}

			if (!confirm(bulkmail.l10n.manage.confirm_import)) return false;
			...
			importstatus.html(bulkmail.l10n.manage.prepare_import);

			window.onbeforeunload = function () {
				return bulkmail.l10n.manage.onbeforeunloadimport;
			};
	...
	$('#paste-import')
		...
		.on('blur', function () {
			...
			if (value) {
				bulkmail.util.ajax('import_subscribers_upload_handler', {
					data: value
				}, function (response) {
	...
	$('#export-subscribers').on('submit', function () {
		...
		bulkmail.util.ajax('export_contacts', {
			data: data,
		}, function (response) {
			...
			if (response.success) {
				...
				window.onbeforeunload = function () {
					return bulkmail.l10n.manage.onbeforeunloadexport;
				};

				var limit = $('.performance').val();

				$('.step2').slideDown();
				$('.step2-body').html(bulkmail.util.sprintf(bulkmail.l10n.manage.write_file, '0.00 Kb'));
				progress.removeClass('finished error hidden');
				progressbar.stop().width(0);
	...
	$('#delete-subscribers')
		.on('submit', function () {
			...
			var input = prompt(bulkmail.l10n.manage.confirm_delete, '');
			...
			if ('delete' == input.toLowerCase()) {
				...
				bulkmail.util.ajax('delete_contacts', {
					data: data,
				}, function (response) {
	...
	function update_deletion_count() {
		...
		setTimeout(function () {
			...
			bulkmail.util.ajax('get_subscriber_count', {
				data: data
			}, function (response) {

				if (response.success) {
					$('#delete-subscriber-button').val(bulkmail.util.sprintf(bulkmail.l10n.manage.delete_n_subscribers, response.count)).prop('disabled', !response.count);
				}
	...
	function do_export(offset, limit, count, data) {
		...
		exportstatus.html(bulkmail.util.sprintf(bulkmail.l10n.manage.prepare_download, count, ''));

		bulkmail.util.ajax('do_export', {
			limit: limit,
			offset: offset,
			data: data
		}, function (response) {
			...
			if (response.success) {
				...
				progressbar.animate({
					...
					step: function (percentage) {
						exportstatus.html(bulkmail.util.sprintf(bulkmail.l10n.manage.prepare_download, count, Math.ceil(percentage) + '%'));
					},
					complete: function () {
						exportstatus.html(bulkmail.util.sprintf(bulkmail.l10n.manage.prepare_download, count, Math.ceil(percentage) + '%'));
						if (finished) {
							...
							progress.addClass('finished');
							$('.step2-body').html(bulkmail.l10n.manage.export_finished);

							exportstatus.html(bulkmail.util.sprintf(bulkmail.l10n.manage.downloading, count));
							if (response.filename) setTimeout(function () {
								document.location = response.filename
							}, 1000);

						} else {
							$('.step2-body').html(bulkmail.util.sprintf(bulkmail.l10n.manage.write_file, response.total));
						}
					}
				});
				...
				progress.addClass('error');
				window.onbeforeunload = null;
				exportstatus.html(bulkmail.l10n.manage.error_export);
		...
		bulkmail.util.ajax('do_import', {
			...
			if (response.success) {
				...
				progressbar.animate({
					...
					queue: false,
					step: function (percentage) {
						importstatus.html(bulkmail.util.sprintf(bulkmail.l10n.manage.import_contacts, Math.ceil(percentage) + '%'));
					},
					complete: function () {
						importstatus.html(bulkmail.util.sprintf(bulkmail.l10n.manage.import_contacts, Math.ceil(percentage) + '%'));
						if (finished) {
	...
	function get_import_data() {
		...
		bulkmail.util.ajax('get_import_data', {
	...
	function upload_error_handler(percentage, id, options) {
		...
		if (importerrors >= 5) {
			...
			alert(bulkmail.l10n.manage.error_importing);
			window.onbeforeunload = null;
			return;
		}
		...
		var i = importerrors * 5,
			...
			errorint = setInterval(function () {
				...
				if (i <= 0) {
					...
					str = '<span class="error">' + bulkmail.util.sprintf(bulkmail.l10n.manage.continues_in, (i--)) + '</span>';

				}
				importstatus.html(bulkmail.util.sprintf(bulkmail.l10n.manage.import_contacts, str));
	...
	function get_stats(imported, errors, total, percentage, memoryusage) {
		...
		return bulkmail.util.sprintf(bulkmail.l10n.manage.current_stats, '<strong>' + imported + '</strong>', '<strong>' + total + '</strong>', '<strong>' + errors + '</strong>', '<strong>' + memoryusage + '</strong>') + '<br>' +
			bulkmail.util.sprintf(bulkmail.l10n.manage.estimate_time, timeleft);
