...
bulkmail = (function (bulkmail, $, window, document) {
	...
	bar
		...
		.on('keyup change', '#bulkmail-editor', change)
		...
		.on('mouseenter', '#wp-bulkmail-editor-wrap, .imagelist, .postlist, .CodeMirror', disabledrag)
		.on('mouseleave', '#wp-bulkmail-editor-wrap, .imagelist, .postlist, .CodeMirror', enabledrag)
		.on('click', '.current-tag a', false)
		.on('keypress.bulkmail', function (event) {
			if (event.keyCode == 13 && event.target.nodeName.toLowerCase() != 'textarea') return false;
		})
		.on('keyup.bulkmail', function (event) {
	...
	bulkmail.util.getRealDimensions(bulkmail.$.iframe.contents().find("img").eq(0), function (w, h, f) {
		var ishighdpi = f >= 1.5;
		factor.val(f);
		...
		setTimeout(function () {
			...
			if (url) {
				...
				img.onerror = function () {
					if (e.type != 'paste') alert(bulkmail.util.sprintf(bulkmail.l10n.campaigns.invalid_image, '"' + url + '"'));
				};
				img.src = url;
			}
		}, 1);
					...
					$.each(currenttag.terms, function (i, term) {
						...
						$.each(term_ids, function (j, id) {
							...
							if (!select.length) {
								...
								$('<span> ' + bulkmail.l10n.campaigns.or + ' </span>').insertBefore(select);
							}
							select.val(id);
						});
					});
	...
	bulkmail.events.documentReady.push(function () {
		bar.draggable && bar.draggable({
			'distance': 20,
			'axis': 'y'
		});
		if (bulkmail.util.isTinyMCE && tinymce.get('bulkmail-editor')) {
			if (tinymce.majorVersion >= 4) {

				tinymce.get('bulkmail-editor').on('keyup', function () {
					mceUpdater(this);
				});
				tinymce.get('bulkmail-editor').on('ExecCommand', function () {
					mceUpdater(this);
				});

			} else {
				tinymce.get('bulkmail-editor').onKeyUp.add(function () {
					mceUpdater(this);
				});
				tinymce.get('bulkmail-editor').onExecCommand.add(function () {
					mceUpdater(this);
				});
			}
		}
	...
	function replaceImage() {
		...
		var f = factor.val(),
			...
			h = Math.round(w / 1.6),
			img = $('<img>', {
				'src': 'https://dummy.bulkmail.co/' + (w * f) + 'x' + (h * f) + '.jpg',
				'alt': current.content,
	...
	function checkForPosts() {
		...
		checkForPostsTimeout = setTimeout(function () {
			...
			bulkmail.util.ajax('check_for_posts', postargs, function (response) {
				loader(false);
				if (response.success) {
	...
	function dynamicImage(val, w, h, c, o) {
		...
		if (/^\{([a-z0-9-_,;:|~]+)\}$/.test(val)) {
			var f = factor.val();
			val = bulkmail.ajaxurl + '?action=bulkmail_image_placeholder&tag=' + val.replace('{', '').replace('}', '') + '&w=' + Math.abs(w) + '&h=' + Math.abs(h) + '&c=' + (c ? 1 : 0) + '&o=' + (o ? 1 : 0) + '&f=' + f;
		}
		return val;
	}

	function isDynamicImage(val) {
		if (-1 !== val.indexOf('?action=bulkmail_image_placeholder&tag=')) {
	...
	function save() {
		...
		if (current.type == 'img') {
			...
			if (currentimage) {
				...
				}).data('id', currentimage.id).addClass('bulkmail-loading');
					...
					if (response.success) {
						...
						if (is_img) {
							current.element.one('load error', function (event) {
								if ('error' == event.type) {
									alert(bulkmail.util.sprintf(bulkmail.l10n.campaigns.invalid_image, response.image.url));
								}
								current.element.removeClass('bulkmail-loading');
							...
							if (orgimageurl.val()) {
								if (is_root) {
									if (is_root = html.match(new RegExp('<v:background(.*)<\/v:background>', 's'))) {
										current.element.html(bulkmail.util.replace(html, is_root[0], bulkmail.util.replace(is_root[0], orgimageurl.val(), currentimage.url)));
									}
								} else {
									current.element.html(bulkmail.util.replace(html, orgimageurl.val(), currentimage.url));
			...
			if (!link && !confirm(bulkmail.l10n.campaigns.remove_btn)) return false;

			var btnsrc = base.find('a.active').find('img').attr('src');
			if (typeof btnsrc == 'undefined') {
				buttontype = 'text';
				if (!buttonlabel.val()) buttonlabel.val(bulkmail.l10n.campaigns.read_more);
			}
			...
			if (currenttext) {
						...
						if ('static' == insertmethod) {
							bulkmail.util.ajax('create_image', {
								...
								if (response.success) {
									...
									if ('img' == imgelement.prop('tagName').toLowerCase()) {
										...
										current.element.html(bulkmail.util.replace(current.element.html(), orgurl, response.image.url));
							...
							if ('img' == imgelement.prop('tagName').toLowerCase()) {
								...
								imgelement
									...
									.removeData('id')
									.css('background-image', '');
								current.element.html(bulkmail.util.replace(current.element.html(), orgurl, dynamicImage(currenttext.image[i], width, height, crop, org)));
								//remove id to re trigger tinymce
								current.element.find('single, multi').removeAttr('id');
							}
				...
				bulkmail.$.iframe.contents().find("html")
			...
			if (bulkmail.util.isTinyMCE && tinymce.get('bulkmail-editor') && !tinymce.get('bulkmail-editor').isHidden()) {
				var content = tinymce.get('bulkmail-editor').getContent();
	...
	function choosePost() {
		...
		if (current.type == 'btn') {
			...
			bulkmail.util.ajax('get_post', {
				...
				if (response.success) {
					currenttext = response.pattern;
					base.find('.editbarinfo').html(bulkmail.l10n.campaigns.curr_selected + ': <span>' + currenttext.title + '</span>');
				}
			}, function (jqXHR, textStatus, errorThrown) {
	...
	function open(data) {
		...
		if (type == 'img') {
			...
			factor.val(1);
			bulkmail.util.getRealDimensions(el, function (w, h, f) {
				var h = f >= 1.5;
				factor.val(f);
			...
			if (el.find('img').length) {
				...
				bulkmail.util.getRealDimensions(el.find('img'), function (w, h, f) {
					var h = f >= 1.5;
					factor.val(f);
					highdpi.prop('checked', h);
					(h) ? bar.addClass('high-dpi'): bar.removeClass('high-dpi');
		...
		offset = bulkmail.$.template.offset().top + (current.offset.top - (bulkmail.$.window.height() / 2) + (current.height / 2));

		offset = Math.max(bulkmail.$.template.offset().top - 200, offset);

		bulkmail.util.scroll(offset, function () {
			...
			//center the bar
			var baroffset = bulkmail.util.top() + (bulkmail.$.window.height() / 2) - bulkmail.$.template.offset().top - (bar.height() / 2);
			...
			if (type == 'single') {
				...
				if (current.areas.length > 1) {
					bar.find('.editbarposition').html(bulkmail.util.sprintf(bulkmail.l10n.campaigns.for_area, '#' + (position + 1))).show();
				} else {
			...
			bar.show(0, function () {
				...
				if (type == 'single') {
					...
					$('#bulkmail-editor').val(content);

					if (bulkmail.util.isTinyMCE && tinymce.get('bulkmail-editor')) {
						tinymce.get('bulkmail-editor').setContent(content);
						tinymce.execCommand('mceFocus', false, 'bulkmail-editor');
					}

				} else if (type == 'codeview') {

					codemirror = bulkmail.util.CodeMirror.fromTextArea(textarea.get(0), codemirrorargs);
	...
	function loadPosts(event, callback) {
		...
		bulkmail.util.ajax('get_post_list', data, function (response) {
			loader(false);
			if (response.success) {
	...
	function loadMorePosts() {
		...
		bulkmail.util.ajax('get_post_list', {
			type: type,
			posttypes: posttypes,
			search: searchstring,
			imagetype: imagesearchtype.filter(':checked').val(),
	...
	function openURL() {
		...
		if (!imageurl.val() && currentimage.src.indexOf(location.origin) == -1 && currentimage.src.indexOf('dummy.bulkmail.co') == -1) {
	...
	function openMedia() {
		...
		if (!wp.media.frames.bulkmail_editbar) {

			wp.media.frames.bulkmail_editbar = wp.media({
				title: bulkmail.l10n.campaigns.select_image,
				library: {
					type: 'image'
				},
				multiple: false
			...
			wp.media.frames.bulkmail_editbar.on('select', function () {
				var attachment = wp.media.frames.bulkmail_editbar.state().get('selection').first().toJSON(),
					el = $('img').data({
						id: attachment.id,
		...
		wp.media.frames.bulkmail_editbar.open();
