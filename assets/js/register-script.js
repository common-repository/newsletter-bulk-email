bulkmail = (function (bulkmail, $, window, document) {
	...
	$('.register_form_wrap')
		...
		.on('click', '.envato-signup', function () {
			...
			popup(this.href, 600, 900, 'bulkmail_envato_signup');
			return false;
		})
		.on('click', '.howto-purchasecode', function () {

			bulkmail.dialog.open('registration-dialog');
			return false;
		})
		...
		.on('submit', '.register_form', function () {
			...
			bulkmail.util.ajax('register', {
				...
				if (response.success) {
					...
					error += ' <a href="https://evp.to/error-' + response.code + '" target="_blank" rel="noopener">' + bulkmail.l10n.register.help + '</a>';
				...
				form.prop('disabled', false);
				wrap.removeClass('loading');
				alert(bulkmail.l10n.register.error + "\n\n" + errorThrown);
			});
		...
		.on('submit', '.register_form_2', function () {
			...
			bulkmail.util.ajax('register', {
				...
				if (response.success) {
					...
					bulkmail.$.document.trigger('verified.' + slug, [response.purchasecode, response.username, response.email]);
				} else {
	...
	window.verifyBulkmail = function (slug, purchasecode, username, email) {
