bulkmail = (function (bulkmail, $, window, document) {
	...
	bulkmail.events.push('documentReady', function () {
		...
		$('#use-it')
			.on('click', function () {
				tb_show(bulkmail.l10n.form.useit, '#TB_inline?&width=' + (Math.min(1200, bulkmail.$.window.width() - 50)) + '&height=' + (bulkmail.$.window.height() - 100) + '&inlineId=useitbox', null);
				if (saverequired) alert(bulkmail.l10n.form.not_saved);
				return false;
			});
		...
		function requireSave() {
			...
			window.onbeforeunload = function () {
				return bulkmail.l10n.form.require_save;
			};
		}
		...
		iframe.on('load', function () {
			_style = $('<style id="bulkmail_form_preview_css"></style>').appendTo(iframe.contents().find('head'));
			updateStyle();
			iframe.contents().find('style.bulkmail-custom-form-css').remove();

		});

		addcustomstyle.on('click', function () {
			var el = $(this).parent().find('.color-field'),
				style = '.bulkmail-form-' + ID + ' ' + el.data('selector') + '{\n    \n}\n';
		...
		customstylesamples.find('option').on('click', function () {
			customstyle.val(customstyle.val() + customstyleprefix.val() + '.bulkmail-form-' + ID + $(this).val() + "{\n    \n}\n");
		...
		$('#form-options')
			...
			.on('change', '.bulkmail_userschoice', function () {

				var checked = $(this).is(':checked');
				$(this).parent().parent().parent().parent().find('legend.bulkmail_userschoice_td').hide().eq(checked ? 1 : 0).show();
				$(this).parent().parent().find('.bulkmail_dropdown').prop('disabled', !checked);
				$(this).parent().parent().parent().parent().find('.bulkmail_precheck').prop('disabled', (checked ? 0 : 1));
		...
		function updateStyle() {
			...
			$.each(selectors, function (s, p) {
				...
				style += 'form.bulkmail-form.bulkmail-form-' + ID + ' ' + s + '{\n';
				$.each(p, function (k, v) {
					style += '\t' + k + ': ' + v + ';\n';
				});
				style += '}\n';
		...
		$('.embed-form-input')
			.on('change', function () {
				...
				output.val(bulkmail.util.sprintf(output.data('embedcode'), inputs.eq(0).val(), inputs.eq(1).val(), (inputs.eq(2).is(':checked') ? '&style=1' : '')));
		...
		function updateButton() {
			...
			var code = window.bulkmaildata.embedcode,
				id = $('#ID').val(),
			...
			if (window.BulkmailSubscribe && window.BulkmailSubscribe.loaded) window.BulkmailSubscribe.destroy();
			buttonpreview.html('');
