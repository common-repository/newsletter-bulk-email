jQuery(document).ready(function ($) {
	...
	$(window).on({
		'load.bulkmail resize.bulkmail': resize
	}).trigger('resize.bulkmail');
	...
	$('.social-services').on('click', 'a', function () {
		...
		var dimensions = $(this).data(),
			...
			newWindow = window.open(href, 'bulkmail_share', 'scrollbars=auto,resizable=1,menubar=0,toolbar=0,location=0,directories=0,status=0, width=' + dimensions.width + ', height=' + dimensions.height + ', top=' + top + ', left=' + left);
	...
	$('#emailform').on('submit', function () {
		...
		$.post(ajaxurl, {
			action: 'bulkmail_forward_message',
			data: data
		}, function (response) {
			loader.css({
				'visibility': 'hidden'
