bulkmail = (function (bulkmail, $, window, document) {
	...
	$('#posts-filter').on('submit', function () {
		...
		switch (v) {
		case 'finish':
			return confirm(bulkmail.l10n.campaigns.finish_campaigns);
			break;
		case 'start':
			return confirm(bulkmail.l10n.campaigns.start_campaigns);
			break;
		}
	...
	$('.column-status')
		.on('click', 'a.live-action', function () {
			...
			if ($(this).hasClass('finish') && !confirm(bulkmail.l10n.campaigns.finish_campaign)) {
				return false;
			}
	...
	bulkmail.$.document
		...
		.on('heartbeat-send', function (e, data) {
			...
			rows.each(function () {
				...
				if (bulkmail.util.inViewport(this))
					ids.push(id);
			});
		...
		.on('heartbeat-tick', function (e, data) {
			...
			if (data['bulkmail']) {
				...
				if (!current) {
					current = $.extend(data['bulkmail'], current);
					first = true;
				}
				...
				$.each(rows, function (id, row) {
					...
					if (!data['bulkmail'][id]) return;
					rowdata = data['bulkmail'][id];
				...
				current = $.extend(current, data['bulkmail']);
			}
