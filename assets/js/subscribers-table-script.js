bulkmail = (function (bulkmail, $, window, document) {
		...
		.on('submit', function (event) {
			...
			if (1 == $input.val()) {
				...
				window.onbeforeunload = function () {
					return bulkmail.l10n.subscribers.onbeforeunload;
				};

				bulk_update_info = $('<div class="alignright bulk-update-info spinner">' + bulkmail.l10n.subscribers.initprogess + '</div>').prependTo('.bulkactions');
	...
	function do_batch(data, page, cb) {
		...
		$.post(location.href, {
			...
			if (response.success_message) bulkmail.log(response.success_message);
			if (response.error_message) bulkmail.log(response.error_message, 'error');
