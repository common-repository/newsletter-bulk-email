bulkmail = (function (bulkmail, $, window, document) {
	...
	$('#bulkmail_templates')
		...
		.on('click', '.edit', function () {
			var $this = $(this),
				$container = $this.closest('.bulkmail-box'),
				$templates = $('.bulkmail-box'),
				href = $this.attr('href'),
				slug = $this.data('slug');
			...
			var count = Math.floor(($('#bulkmail_templates').outerWidth()) / ($container.width() + 22));
			...
			bulkmail.util.scroll(templateeditor.offset().top - 50);
		...
		.on('click', '.remove-file', function () {
			...
			if (confirm($this.data('confirm'))) {
				...
				bulkmail.util.ajax('remove_template', {
					file: $this.data('file'),
				}, function (response) {
		...
		.on('click', 'button.save, button.saveas', function () {
			...
			if ($this.is('.saveas') && !(name = prompt(bulkmail.l10n.templates.enter_template_name + ':', ''))) return false;
			...
			bulkmail.util.ajax('set_template_html', {
				content: content,
				name: name,
				slug: $('#slug').val(),
				file: $('#file').val()
				...
				if (response.success) {
					...
					if (response.newfile) {
						get_template_html('bulkmail/' + response.newfile);
					}
				} else {
					alert(response.msg);
				}
		...
		.on('click', '.thickbox-preview', function () {
			...
			bulkmail.util.ajax('get_file_list', {
				slug: slug
			}, function (response) {
				if (response.success) {
					var html = '';
					$.each(response.files, function (file, data) {
						...
						if ('index.html' != file && 'notification.html' != file) {
							html += '<a href="" class="thickbox-remove-file bulkmail-icon" data-slug="' + response.slug + '" data-file="' + file + '"></a>';
						}
		...
		.on('click', 'a.update', function () {
			...
			if (confirm(bulkmail.l10n.templates.update_note)) {
				$(this).closest('.bulkmail-box').addClass('loading');
				return true;
			}
			return false;
		})
		.on('click', 'a.deletion', function () {
			...
			if (confirm(bulkmail.util.sprintf(bulkmail.l10n.templates.confirm_delete, '"' + $(this).data('name') + '"'))) {
				$(this).closest('.bulkmail-box').addClass('loading');
				return true;
			}
			return false;
	...
	bulkmail.$.document
		...
		.on('click', '.thickbox-remove-file', function () {
			...
			if (confirm(bulkmail.util.sprintf(bulkmail.l10n.templates.delete_template_file, file, slug))) {
	...
	function uploader_init() {
		...
		uploader.bind('UploadProgress', function (up, file) {
			uploadinfo.html(bulkmail.util.sprintf(bulkmail.l10n.templates.uploading, file.percent + '%'));
		});
	...
	function get_template_html(href) {
		...
		$('#templateeditor').addClass('loading');
		bulkmail.util.ajax('get_template_html', {
			href: href
		}, function (response) {
			loader.hide();
			$('#templateeditor').removeClass('loading');
			...
			if (!codemirror) {
				...
				codemirror = bulkmail.util.CodeMirror.fromTextArea(templatecontent.get(0), {
					mode: mixedMode,
					tabMode: "indent",
					lineNumbers: true,
					autofocus: true
			...
			if (response.file != 'index.html' && response.file != 'notification.html') html += '<a class="remove-file bulkmail-icon" data-file="' + response.slug + '/' + response.file + '" data-confirm="' + bulkmail.util.sprintf(bulkmail.l10n.templates.delete_template_file, selected.label, selected.name) + '"></a>';
