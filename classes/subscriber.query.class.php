...
class BulkmailSubscriberQuery {
	...
	public function run( $args = array(), $campaign_id = null ) {
		...
		$name_order = bulkmail_option( 'name_order' );
		$joins      = array();
		$wheres     = array();
		...
		$this->args = apply_filters( 'bulkmail_subscriber_query_args', $this->args, $campaign_id );

		$cache_key = 'query_' . md5( serialize( $this->args ) );

		if ( $result = bulkmail_cache_get( $cache_key ) ) {
			return $result;
		}

		if ( $this->args['lists'] !== false ) {
			$join = "LEFT JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS lists_subscribers ON subscribers.ID = lists_subscribers.subscriber_id";
		...
		if ( $this->args['queue'] || $this->args['queue__not_in'] ) {
			$join = "LEFT JOIN {$wpdb->prefix}bulkmail_queue AS queue ON subscribers.ID = queue.subscriber_id";
		...
		if ( $this->args['unsubscribe'] || $this->args['unsubscribe__not_in'] ) {
			$join = "LEFT JOIN {$wpdb->prefix}bulkmail_actions AS actions_unsubscribe ON actions_unsubscribe.type = 4 AND subscribers.ID = actions_unsubscribe.subscriber_id";
		...
		if ( ! empty( $meta_and_fields ) ) {
			...
			foreach ( $meta_and_fields as $field ) {
				...
				if ( in_array( $field, array( 'lat', 'lng' ) ) ) {
					...
					$joins[] = "LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS `meta_$field` ON `meta_$field`.subscriber_id = subscribers.ID AND `meta_$field`.meta_key = 'coords'";

				} elseif ( in_array( $field, $this->custom_fields ) ) {

					$joins[] = "LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_fields AS `field_$field` ON `field_$field`.subscriber_id = subscribers.ID AND `field_$field`.meta_key = '$field'";
					...
					$joins[] = "LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS `meta_$field` ON `meta_$field`.subscriber_id = subscribers.ID AND `meta_$field`.meta_key = '$field'";
				}
			}
		}
			...
			foreach ( $this->args['conditions'] as $i => $and_conditions ) {
				...
				foreach ( $and_conditions as $j => $condition ) {
					...
					if ( in_array( $field, array( 'lat', 'lng' ) ) ) {
						...
						$joins[] = "LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS `meta_coords` ON `meta_coords`.subscriber_id = subscribers.ID AND `meta_coords`.meta_key = 'coords'";
						...
						if ( 'geo' == $field ) {
							...
							foreach ( $continents as $code => $continent ) {
								if ( ( $pos = array_search( $code, $value ) ) !== false ) {
									unset( $value[ $pos ] );
									$value = array_merge( $value, bulkmail( 'geo' )->get_continent_members( $code ) );
								}
							}
					...
					if ( ! in_array( $field, $this->action_fields ) ) {
						...
						if ( $field == '_lists__in' ) {
							...
							$sub_cond[] = "subscribers.ID IN ( SELECT subscriber_id FROM {$wpdb->prefix}bulkmail_lists_subscribers WHERE list_id IN (" . implode( ',', array_filter( $value, 'is_numeric' ) ) . ') )';

						} elseif ( $field == '_lists__not_in' ) {

							$sub_cond[] = "subscribers.ID NOT IN ( SELECT subscriber_id FROM {$wpdb->prefix}bulkmail_lists_subscribers WHERE list_id IN (" . implode( ',', array_filter( $value, 'is_numeric' ) ) . ') )';

						} elseif ( 0 === strpos( $field, '_sent' ) ) {

							$join = "LEFT JOIN {$wpdb->prefix}bulkmail_actions AS $alias ON $alias.type = 1 AND subscribers.ID = $alias.subscriber_id";
							...
							$join = "LEFT JOIN {$wpdb->prefix}bulkmail_actions AS $alias ON $alias.type = 2 AND subscribers.ID = $alias.subscriber_id";
							...
							$join = "LEFT JOIN {$wpdb->prefix}bulkmail_actions AS $alias ON $alias.type = 3 AND subscribers.ID = $alias.subscriber_id";
							...
							if ( ( '_click' === $field || '_click__not_in' === $field ) && $value && $value != -1 ) {
								...
								$joins[] = "LEFT JOIN {$wpdb->prefix}bulkmail_links AS {$alias}{$field} ON {$alias}{$field}.link IN ('" . implode( "','", $value ) . "')";
							}
		...
		$from = "FROM {$wpdb->prefix}bulkmail_subscribers AS subscribers";
		...
		$sql  = apply_filters( 'bulkmail_subscriber_query_sql_select', $select, $this->args, $campaign_id ) . "\n";
		$sql .= ' ' . apply_filters( 'bulkmail_subscriber_query_sql_from', $from, $this->args, $campaign_id ) . "\n";
		$sql .= ' ' . apply_filters( 'bulkmail_subscriber_query_sql_join', $join, $this->args, $campaign_id ) . "\n";
		$sql .= ' ' . apply_filters( 'bulkmail_subscriber_query_sql_where', $where, $this->args, $campaign_id ) . "\n";
		$sql .= ' ' . apply_filters( 'bulkmail_subscriber_query_sql_group', $group, $this->args, $campaign_id ) . "\n";
		$sql .= ' ' . apply_filters( 'bulkmail_subscriber_query_sql_having', $having, $this->args, $campaign_id ) . "\n";
		$sql .= ' ' . apply_filters( 'bulkmail_subscriber_query_sql_order', $order, $this->args, $campaign_id ) . "\n";
		$sql .= ' ' . apply_filters( 'bulkmail_subscriber_query_sql_limit', $limit, $this->args, $campaign_id );

		$sql = trim( $sql );

		// legacy filter
		$sql = apply_filters( 'bulkmail_campaign_get_subscribers_by_list_sql', $sql );

		$sql = apply_filters( 'bulkmail_subscriber_query_sql', $sql, $this->args, $campaign_id );
		...
		bulkmail_cache_set( $cache_key, $result );
	...
	private function get_custom_fields() {
		$custom_fields = bulkmail()->get_custom_fields( true );
	...
	private function get_meta_fields() {
		$meta_fields = bulkmail( 'subscribers' )->get_meta_keys( true );

		return $meta_fields;
	}

	private function get_wp_user_meta() {
		$wp_user_meta = wp_parse_args( array( 'wp_user_level', 'wp_capabilities' ), bulkmail( 'helper' )->get_wpuser_meta_fields() );
