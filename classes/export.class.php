...
class BulkmailExport {
	...
	public function init() {
		...
		add_filter( 'wxr_export_skip_postmeta', array( &$this, 'add_bulkmail_data_to_export' ), 10, 3 );
	...
	public function import_post_data( $postdata, $post ) {
		...
		bulkmail_notice( __( 'Please make sure all your campaigns are imported correctly!', 'bulkmail' ), 'error', false, 'import_campaings' );
	...
	public function convert_old_campaign_ids( $post_id, $original_post_ID, $postdata, $post ) {
		...
		echo '<h4>';
		printf( __( 'Updating Bulkmail tables for Campaign %s:', 'bulkmail' ), '"<a href="' . admin_url( 'post.php?post=' . $post_id . '&action=edit' ) . '">' . $postdata['post_title'] . '</a>"' );
		echo '</h4>';
		...
		foreach ( $tables as $table ) {
			...
			if ( false !== ( $rows = $wpdb->query( $sql ) ) ) {
				printf( '..' . __( 'completed for %d rows.', 'bulkmail' ), $rows );
			}
			echo '<br>';
		}
	...
	public function set_temporary_export_meta( $args ) {
		...
		$sql = "INSERT INTO {$wpdb->postmeta} (post_id, meta_key, meta_value) SELECT ID AS post_id, '_bulkmail_temp_export_hook', '1' FROM {$wpdb->posts} WHERE post_type = 'newsletter'";
	...
	public function unset_temporary_export_meta() {
		...
		$sql = "DELETE FROM {$wpdb->postmeta} WHERE meta_key = '_bulkmail_temp_export_hook'";

		return $wpdb->query( $sql );

	}

	public function add_bulkmail_data_to_export( $skip_it, $meta_key, $meta ) {
		if ( '_bulkmail_temp_export_hook' == $meta_key ) :
			global $wpdb;

			$tables = array( 'actions', 'queue', 'subscriber_meta' );

			foreach ( $tables as $table ) {
				$sql = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}bulkmail_{$table} WHERE campaign_id = %d", $meta->post_id );
				...
				if ( ! empty( $data ) ) {
					printf(
						...
						wxr_cdata( '_bulkmail_table_data_' . $table ),
