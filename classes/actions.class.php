...
class BulkmailActions {
	...
	public function init() {
		...
		add_action( 'bulkmail_send', array( &$this, 'send' ), 10, 2 );
		add_action( 'bulkmail_open', array( &$this, 'open' ), 10, 3 );
		add_action( 'bulkmail_click', array( &$this, 'click' ), 10, 4 );
		add_action( 'bulkmail_unsubscribe', array( &$this, 'unsubscribe' ), 10, 3 );
		add_action( 'bulkmail_list_unsubscribe', array( &$this, 'list_unsubscribe' ), 10, 4 );
		add_action( 'bulkmail_bounce', array( &$this, 'bounce' ), 10, 3 );
		add_action( 'bulkmail_subscriber_error', array( &$this, 'error' ), 10, 3 );
		add_action( 'bulkmail_cron_cleanup', array( &$this, 'cleanup' ) );
	...
	public function get_fields( $fields = null, $where = null ) {
		...
		$sql = "SELECT $fields FROM {$wpdb->prefix}bulkmail_actions WHERE 1=1";
		if ( is_array( $where ) ) {
	...
	public function error( $subscriber_id, $campaign_id, $error = '' ) {
		...
		bulkmail( 'subscribers' )->update_meta( $subscriber_id, $campaign_id, 'error', $error );
	...
	private function add_subscriber_action( $args, $explicit = true ) {
		...
		if ( bulkmail_option( 'do_not_track' ) && isset( $_SERVER['HTTP_DNT'] ) && $_SERVER['HTTP_DNT'] == 1 ) {
			return;
		}
		...
		if ( $args['type'] == 2 && $explicit ) {
			...
			if ( $client = bulkmail_get_user_client() ) {
		...
		bulkmail( 'subscribers' )->update_meta( $args['subscriber_id'], $args['campaign_id'], $user_meta );
	...
	private function add( $args, $explicit = true ) {
		...
		$sql = "INSERT INTO {$wpdb->prefix}bulkmail_actions (" . implode( ', ', array_keys( $args ) ) . ')';
		...
		$sql = apply_filters( 'bulkmail_actions_add_sql', $sql, $args, $explicit );

		if ( false !== $wpdb->query( $sql ) ) {
			if ( $args['type'] != 1 && $explicit && isset( $args['subscriber_id'] ) ) {
				wp_schedule_single_event( time() + 120, 'bulkmail_update_rating', array( $args['subscriber_id'] ) );
			}
	...
	public function clear( $campaign_id, $subscribers ) {
		...
		foreach ( $chunks as $subscriber_chunk ) {
			...
			$sql = "DELETE a FROM {$wpdb->prefix}bulkmail_queue AS a WHERE a.campaign_id = %d AND a.sent = 0 AND a.subscriber_id NOT IN (" . implode( ',', $subscriber_chunk ) . ')';
	...
	public function cleanup() {
		...
		$wpdb->query( $wpdb->prepare( "DELETE b FROM {$wpdb->prefix}bulkmail_actions AS a LEFT JOIN {$wpdb->prefix}bulkmail_actions AS b ON a.campaign_id = b.campaign_id AND a.subscriber_id = b.subscriber_id AND a.link_id = b.link_id WHERE a.type = %d AND b.type = %d", 6, 5 ) );
	...
	public function get_by_campaign( $campaign_id = null, $action = null, $strict = false ) {
		...
		$action_counts = bulkmail_cache_get( $cache_key );
		if ( ! $action_counts ) {
			$action_counts = array();
		}
		...
		$sql = "SELECT a.post_id AS ID, a.meta_value AS parent_id FROM {$wpdb->postmeta} AS a WHERE a.meta_key = '_bulkmail_parent_id'";
		...
		// $sql = "SELECT a.campaign_id AS ID, type, COUNT(DISTINCT a.subscriber_id) AS count, SUM(a.count) AS total FROM {$wpdb->prefix}bulkmail_actions AS a";
		$sql = "SELECT a.campaign_id AS ID, type, COUNT( DISTINCT COALESCE( a.subscriber_id, 1) ) AS count, COUNT(DISTINCT a.subscriber_id) AS count_cleard, SUM(a.count) AS total FROM {$wpdb->prefix}bulkmail_actions AS a";
		...
		bulkmail_cache_set( $cache_key, $action_counts );
	...
	public function get_by_subscriber( $subscriber_id = null, $action = null, $strict = false ) {
		...
		$sql = "SELECT a.campaign_id, a.subscriber_id AS ID, type, COUNT(DISTINCT a.subscriber_id) AS count, SUM(a.count) AS total FROM {$wpdb->prefix}bulkmail_actions AS a";
	...
	public function get_by_list( $list_id = null, $action = null, $strict = false ) {
		...
		$sql = "SELECT b.list_id AS ID, type, COUNT(DISTINCT a.subscriber_id) AS count, SUM(a.count) AS total FROM {$wpdb->prefix}bulkmail_actions AS a";

		$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS b ON a.subscriber_id = b.subscriber_id WHERE a.campaign_id != 0";
	...
	public function get_dashboard_actions( $scale = 'days', $limit = 7, $offset = 0, $sets = null ) {
		...
		$sql .= ") t1 LEFT JOIN {$wpdb->prefix}bulkmail_actions AS a ON FROM_UNIXTIME(a.timestamp+$timeoffset, '%Y-%m-%d') = t1.date";
	...
	public function get_chronological_actions( $scale = 'days', $since = null, $desc = true ) {
		...
		if ( false === ( $actions = bulkmail_cache_get( 'chronological_actions_' . $scale . $since . $desc ) ) ) {
			...
			$sql = "SELECT FROM_UNIXTIME(a.timestamp+$timeoffset, '" . $timestring[ $scale ] . "') AS date, COUNT(FROM_UNIXTIME(a.timestamp+$timeoffset, '" . $timestring[ $scale ] . "')) AS count, a.type FROM {$wpdb->prefix}bulkmail_actions AS a";
			...
			$sql = "SELECT FROM_UNIXTIME(a.signup+$timeoffset, '" . $timestring[ $scale ] . "') AS date, COUNT(FROM_UNIXTIME(a.signup+$timeoffset, '" . $timestring[ $scale ] . "')) AS count FROM {$wpdb->prefix}bulkmail_subscribers AS a";
			...
			bulkmail_cache_add( 'chronological_actions_' . $scale . $since . $desc, $actions );
	...
	public function get_campaign_actions( $campaign_id, $subscriber_id = null, $action = null, $cache = true ) {
		...
		if ( false === ( $actions = bulkmail_cache_get( 'campaign_actions' ) ) ) {
			...
			$sql = "SELECT a.subscriber_id AS ID, type, COUNT(DISTINCT a.subscriber_id) AS count, SUM(a.count) AS total, a.timestamp, a.link_id, b.link FROM {$wpdb->prefix}bulkmail_actions AS a LEFT JOIN {$wpdb->prefix}bulkmail_links AS b ON b.ID = a.link_id WHERE a.campaign_id = %d";
			...
			if ( $cache ) {
				bulkmail_cache_add( 'campaign_actions', $actions );
			}
		}
	...
	public function get_clicked_links( $campaign_id ) {
		...
		if ( false === ( $clicked_links = bulkmail_cache_get( 'clicked_links_' . $campaign_id ) ) ) {

			if ( $parent_id = get_post_meta( $campaign_id, '_bulkmail_parent_id', true ) ) {
				$sql = "SELECT c.link, c.i, COUNT(*) AS clicks, SUM(a.count) AS total FROM {$wpdb->prefix}bulkmail_actions AS a LEFT JOIN {$wpdb->postmeta} AS b ON b.meta_key = '_bulkmail_parent_id' AND b.post_id = a.campaign_id LEFT JOIN {$wpdb->prefix}bulkmail_links AS c ON c.ID = a.link_id WHERE (a.campaign_id = %d OR b.meta_value = %d) AND a.type = 3 GROUP BY a.campaign_id, a.link_id ORDER BY c.i ASC, total DESC, clicks DESC";

				$sql = $wpdb->prepare( $sql, $campaign_id, $campaign_id );

			} else {
				$sql = "SELECT c.link, c.i, COUNT(*) AS clicks, SUM(a.count) AS total FROM {$wpdb->prefix}bulkmail_actions AS a LEFT JOIN {$wpdb->prefix}bulkmail_links AS c ON c.ID = a.link_id WHERE a.campaign_id = %d AND a.type = 3 GROUP BY a.campaign_id, a.link_id ORDER BY c.i ASC, total DESC, clicks DESC";
			...
			bulkmail_cache_add( 'clicked_links_' . $campaign_id, $clicked_links );
	...
	public function get_clients( $campaign_id ) {
		...
		if ( false === ( $clients = bulkmail_cache_get( 'clients_' . $campaign_id ) ) ) {

			$sql = "SELECT COUNT(DISTINCT a.subscriber_id) AS count, a.meta_value AS name, b.meta_value AS type, c.meta_value AS version FROM {$wpdb->prefix}bulkmail_subscriber_meta AS a LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS b ON a.subscriber_id = b.subscriber_id AND a.campaign_id = b.campaign_id LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS c ON a.subscriber_id = c.subscriber_id AND a.campaign_id = c.campaign_id WHERE a.meta_key = 'client' AND b.meta_key = 'clienttype' AND c.meta_key = 'clientversion' AND a.campaign_id = %d GROUP BY a.meta_value, c.meta_value ORDER BY count DESC";
			...
			bulkmail_cache_add( 'clients_' . $campaign_id, $clients );
	...
	public function get_environment( $campaign_id ) {
		...
		if ( false === ( $environment = bulkmail_cache_get( 'environment_' . $campaign_id ) ) ) {

			$sql = "SELECT COUNT(DISTINCT a.subscriber_id) AS count, a.meta_value AS type FROM {$wpdb->prefix}bulkmail_subscriber_meta AS a LEFT JOIN {$wpdb->prefix}bulkmail_actions AS b ON a.subscriber_id = b.subscriber_id AND a.campaign_id = b.campaign_id WHERE a.meta_key = 'clienttype' AND a.campaign_id = %d AND b.type = 2 GROUP BY a.meta_value ORDER BY count DESC";
			...
			bulkmail_cache_add( 'environment_' . $campaign_id, $environment );
	...
	public function get_error_list( $campaign_id ) {
		...
		if ( false === ( $error_list = bulkmail_cache_get( 'error_list_' . $campaign_id ) ) ) {

			$sql = "SELECT s.ID, s.email, a.timestamp, a.count, b.meta_value AS errormsg FROM {$wpdb->prefix}bulkmail_actions AS a LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS b ON a.subscriber_id = b.subscriber_id AND a.campaign_id = b.campaign_id LEFT JOIN {$wpdb->prefix}bulkmail_subscribers AS s ON s.ID = a.subscriber_id WHERE a.campaign_id = %d AND a.type = 7 AND b.meta_key = 'error' ORDER BY a.timestamp DESC";

			$error_list = $wpdb->get_results( $wpdb->prepare( $sql, $campaign_id ) );

			bulkmail_cache_add( 'error_list_' . $campaign_id, $error_list );
	...
	public function get_activity( $campaign_id = null, $subscriber_id = null, $limit = null, $exclude = null ) {
		...
		$sql = "SELECT p.post_title AS campaign_title, p.post_status AS campaign_status, a.*, b.link, error.meta_value AS error FROM {$wpdb->prefix}bulkmail_actions AS a LEFT JOIN {$wpdb->prefix}bulkmail_links AS b ON b.ID = a.link_id LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS error ON error.subscriber_id = a.subscriber_id AND error.campaign_id = a.campaign_id AND error.meta_key = 'error' LEFT JOIN {$wpdb->posts} AS p ON p.ID = a.campaign_id WHERE 1";
	...
	public function get_list_activity( $list_id = null, $limit = null, $exclude = null ) {
		...
		$sql = "SELECT p.post_title AS campaign_title, a.*, b.link FROM {$wpdb->prefix}bulkmail_actions AS a INNER JOIN (SELECT min(timestamp) as max_ts, type FROM {$wpdb->prefix}bulkmail_actions AS a LEFT JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS ab ON a.subscriber_id = ab.subscriber_id WHERE 1";

		if ( ! is_null( $list_id ) ) {
			$sql .= ' AND ab.list_id = ' . (int) $list_id;
		}

		$sql .= " GROUP BY type, link_id) AS a2 ON a.timestamp = a2.max_ts and a.type = a2.type LEFT JOIN {$wpdb->prefix}bulkmail_links AS b ON b.ID = a.link_id LEFT JOIN {$wpdb->posts} AS p ON p.ID = a.campaign_id LEFT JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS ab ON a.subscriber_id = ab.subscriber_id WHERE 1";
	...
	public function get_link_id( $link, $index = 0 ) {
		...
		if ( $id = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM {$wpdb->prefix}bulkmail_links WHERE `link` = %s AND `i` = %d LIMIT 1", $link, (int) $index ) ) ) {

			return (int) $id;

		} elseif ( $wpdb->query( $wpdb->prepare( "INSERT INTO {$wpdb->prefix}bulkmail_links (`link`, `i`) VALUES (%s, %d)", $link, $index ) ) ) {
