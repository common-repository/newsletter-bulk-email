...
class BulkmailQueue {
	...
	public function init() {
		...
		add_action( 'bulkmail_cron', array( &$this, 'update_status' ), 10 );
		add_action( 'bulkmail_cron', array( &$this, 'update' ), 20 );
		add_action( 'bulkmail_cron_cleanup', array( &$this, 'cleanup' ), 50 );

		add_action( 'bulkmail_cron_worker', array( &$this, 'update_status' ), 10 );
		add_action( 'bulkmail_cron_worker', array( &$this, 'update' ), 20 );
		add_action( 'bulkmail_cron_worker', array( &$this, 'progress' ), 50 );
		add_action( 'bulkmail_cron_worker', array( &$this, 'finish_campaigns' ), 100 );

		add_action( 'bulkmail_cron_autoresponder', array( &$this, 'autoresponder_timebased' ), 30 );
		add_action( 'bulkmail_cron_autoresponder', array( &$this, 'autoresponder_usertime' ), 30 );
		add_action( 'bulkmail_cron_autoresponder', array( &$this, 'autoresponder' ), 30 );

		add_action( 'bulkmail_update_queue', array( &$this, 'autoresponder' ), 30 );
		add_action( 'bulkmail_update_queue', array( &$this, 'update_status' ), 30 );
		add_action( 'bulkmail_update_queue', array( &$this, 'update' ), 30 );

		add_action( 'bulkmail_bounce', array( &$this, 'update_after_bounce' ), 10, 3 );

		// hooks to remove subscriber from the queue
		if ( ! defined( 'BULKEMAIL_DO_BULKIMPORT' ) ) {
			add_action( 'bulkmail_subscriber_change_status', array( &$this, 'subscriber_change_status' ), 10, 3 );
			add_action( 'bulkmail_unassign_lists', array( &$this, 'unassign_lists' ), 10, 3 );
			add_action( 'bulkmail_update_subscriber', array( &$this, 'update_subscriber' ), 10, 3 );
		}
	...
	public function add( $args ) {
		...
		$sql = "INSERT INTO {$wpdb->prefix}bulkmail_queue (" . implode( ', ', array_keys( $args ) ) . ')';
	...
	public function update_after_bounce( $subscriber_id, $campaign_id, $hard ) {
		...
		$now   = time();
		$delay = bulkmail_option( 'bounce_delay', 60 ) * 60;
	...
	public function bulk_add( $campaign_id, $subscribers, $timestamp = null, $priority = 10, $clear = false, $ignore_status = false, $reset = false, $options = false, $tags = false ) {
		...
		foreach ( $chunks as $insert ) {
			$sql = "INSERT INTO {$wpdb->prefix}bulkmail_queue (subscriber_id, campaign_id, added, timestamp, priority, count, ignore_status, options, tags) VALUES";
	...
	public function remove( $campaign_id = null, $subscribers = null, $requeued = false ) {
		...
		$sql = "DELETE a FROM {$wpdb->prefix}bulkmail_queue AS a WHERE 1";
	...
	public function clear( $campaign_id = null, $subscribers = array() ) {
		...
		$sql = "DELETE queue FROM {$wpdb->prefix}bulkmail_queue AS queue WHERE queue.sent = 0 AND queue.subscriber_id NOT IN (" . implode( ',', $subscribers ) . ')';
		if ( ! is_null( $campaign_id ) ) {
	...
	public function cleanup() {
		...
		$wpdb->query( "DELETE queue FROM {$wpdb->prefix}bulkmail_queue AS queue LEFT JOIN {$wpdb->prefix}bulkmail_actions AS b ON queue.subscriber_id = b.subscriber_id AND queue.campaign_id = b.campaign_id WHERE b.type = 5 AND queue.requeued = 1 AND queue.sent != 0" );

		// remove all entries from the queue where subscribers got queue certain autoresponder and are sent already.
		$wpdb->query( "DELETE queue FROM {$wpdb->prefix}bulkmail_queue AS queue LEFT JOIN {$wpdb->posts} AS p ON p.ID = queue.campaign_id AND p.post_status = 'autoresponder' WHERE sent != 0 AND sent < " . ( time() ) );

		// remove all entries from the queue where campaign has been removed
		$wpdb->query( "DELETE queue FROM {$wpdb->prefix}bulkmail_queue AS queue LEFT JOIN {$wpdb->posts} AS p ON p.ID = queue.campaign_id AND p.post_type = 'newsletter' WHERE p.ID IS NULL AND queue.campaign_id != 0" );
	...
	public function update_status() {
		...
		$campaigns = bulkmail( 'campaigns' )->get_campaigns( array( 'post_status' => array( 'queued' ) ) );
		...
		foreach ( $campaigns as $campaign ) {
			...
			$timestamp = bulkmail( 'campaigns' )->meta( $campaign->ID, 'timestamp' );
			$timezone  = bulkmail( 'campaigns' )->meta( $campaign->ID, 'timezone' );

			// change status to active 24h if user based timezone is enabled
			if ( $timestamp - $now <= ( $timezone ? 86400 : 0 ) ) {
				bulkmail( 'campaigns' )->change_status( $campaign, 'active' );
			}
		}
	}
	...
	public function update() {
		...
		$campaigns = bulkmail( 'campaigns' )->get_campaigns( array( 'post_status' => array( 'active' ) ) );
		...
		foreach ( $campaigns as $campaign ) {
			...
			if ( ! bulkmail( 'campaigns' )->get_unsent_subscribers( $campaign->ID, array( 1 ), false, true ) ) {
				continue;
			}
			...
			while ( count( $subscribers = bulkmail( 'campaigns' )->get_unsent_subscribers( $campaign->ID, array( 1 ), true, true, $limit, $offset ) ) ) {

				// get users timeoffsets
				if ( $timezone ) {
					$timestamp = bulkmail( 'subscribers' )->get_timeoffset_timestamps( $subscribers, $timestamp );
				}
	...
	public function autoresponder( $campaign_id = null, $force = false ) {
		...
		global $wpdb;
		static $bulkmail_autoresponder;
		if ( ! isset( $bulkmail_autoresponder ) ) {
			$bulkmail_autoresponder = array();
		}

		$campaigns = empty( $campaign_id ) ? bulkmail( 'campaigns' )->get_autoresponder() : array( bulkmail( 'campaigns' )->get( $campaign_id ) );
		...
		foreach ( $campaigns as $campaign ) {
			...
			if ( in_array( $campaign->ID, $bulkmail_autoresponder ) && ! $force ) {
				continue;
			}

			$bulkmail_autoresponder[] = $campaign->ID;
			...
			if ( is_numeric( $autoresponder_meta['unit'] ) ) {
				...
				bulkmail_notice( sprintf( 'Auto responder campaign %s has been deactivated caused by an old timeformat. Please update your campaign!', '<strong>"<a href="post.php?post=' . $campaign->ID . '&action=edit">' . $campaign->post_title . '</a>"</strong>' ), 'error', false, 'camp_error_' . $campaign->ID, $campaign->post_author );
				bulkmail( 'campaigns' )->update_meta( $campaign->ID, 'active', false );
				continue;
			}

			// time when user no longer get the campaign
			$grace_period  = apply_filters( 'bulkmail_autoresponder_grace_period', WEEK_IN_SECONDS, $campaign );
			$queue_upfront = 3600;

			if ( 'bulkmail_subscriber_insert' == $autoresponder_meta['action'] ) {

				$offset     = (int) $autoresponder_meta['amount'] . ' ' . strtoupper( $autoresponder_meta['unit'] );
				$list_based = bulkmail( 'campaigns' )->list_based_opt_out( $campaign->ID );
				...
				$query_args = apply_filters( 'bulkmail_autoresponder_hook_args', $query_args, $campaign->ID );

				$subscribers = bulkmail( 'subscribers' )->query( $query_args, $campaign->ID );
				...
				if ( ! empty( $subscribers ) ) {
					...
					if ( $timestamp - time() <= 0 ) {
						wp_schedule_single_event( $timestamp, 'bulkmail_cron_worker', array( $campaign->ID ) );
					}
				}
			} elseif ( 'bulkmail_subscriber_unsubscribed' == $autoresponder_meta['action'] ) {
			...
			} elseif ( 'bulkmail_autoresponder_followup' == $autoresponder_meta['action'] && $campaign->post_parent ) {
			...
			} elseif ( 'bulkmail_post_published' == $autoresponder_meta['action'] && $autoresponder_meta['post_type'] == 'rss' ) {

				if ( preg_match_all( '#<module[^>]*?data-rss="(.*?)".*?</module>#ms', $campaign->post_content, $hits ) ) {
					$feed_urls = array_unique( $hits[1] );

					foreach ( $feed_urls as $feed_url ) {
						$feeds = bulkmail( 'helper' )->get_feed_since( $autoresponder_meta['since'], $feed_url );
						...
						foreach ( $feeds as $i => $feed ) {
							...
							if ( ! ( $autoresponder_meta['post_count_status'] % ( $autoresponder_meta['post_count'] + 1 ) ) ) {
								...
								if ( $new_id = bulkmail( 'campaigns' )->autoresponder_to_campaign( $campaign->ID, $send_offset, $autoresponder_meta['issue'], false, $index_offset ) ) {

									$created++;
									$new_campaign = bulkmail( 'campaigns' )->get( $new_id );
									$autoresponder_meta['issue']++;

									bulkmail_notice( sprintf( __( 'New campaign %1$s has been created and is going to be sent on %2$s.', 'bulkmail' ), '<strong>"<a href="post.php?post=' . esc_attr( $new_campaign->ID ) . '&action=edit">' . esc_html( $new_campaign->post_title ) . '</a>"</strong>', '<strong>' . date( bulkmail( 'helper' )->timeformat(), $now + $send_offset + $timeoffset ) . '</strong>' ), 'info', true );
					...
					bulkmail( 'campaigns' )->update_meta( $campaign->ID, 'autoresponder', $autoresponder_meta );
				}
			}
		}
	...
	public function autoresponder_timebased( $campaign_id = null, $force = false ) {
		...
		foreach ( $campaigns as $campaign ) {
			...
			if ( 'bulkmail_autoresponder_timebased' != $autoresponder_meta['action'] ) {
				continue;
			}
			...
			if ( isset( $autoresponder_meta['endschedule'] ) && $autoresponder_meta['endtimestamp'] && $autoresponder_meta['endtimestamp'] - $now < 0 ) {
				...
				bulkmail( 'campaigns' )->update_meta( $campaign->ID, 'active', false );
				bulkmail_notice( sprintf( esc_html__( 'Auto responder campaign %s has been finished and is deactivated!', 'bulkmail' ), '<strong>"<a href="post.php?post=' . $campaign->ID . '&action=edit">' . $campaign->post_title . '</a>"</strong>' ), 'success', false, 'autoresponder_' . $campaign_id, $campaign->post_author );
				continue;
			}
			...
			if ( $doit && $time_conditions ) {
				...
				if ( 'rss' == $autoresponder_meta['time_post_type'] ) {
					...
					if ( preg_match_all( '#<module[^>]*?data-rss="(.*?)".*?</module>#ms', $campaign->post_content, $hits ) ) {
						...
						foreach ( $feed_urls as $feed_url ) {
							if ( $posts = bulkmail( 'helper' )->get_feed_since( $new_content_since, $feed_url ) ) {
			...
			if ( $doit && $new_content_since ) {
				...
				$placeholder = bulkmail( 'placeholder', $campaign->post_content );
			...
			if ( $doit && $new_id = bulkmail( 'campaigns' )->autoresponder_to_campaign( $campaign->ID, $delay, $autoresponder_meta['issue']++ ) ) {

				$newCamp = bulkmail( 'campaigns' )->get( $new_id );
				bulkmail_notice( sprintf( esc_html__( 'New campaign %s has been created!', 'bulkmail' ), '<strong>"<a href="post.php?post=' . $newCamp->ID . '&action=edit">' . $newCamp->post_title . '</a>"</strong>' ), 'info', true, 'autoresponder_' . $campaign->ID, $campaign->post_author );
				...
				do_action( 'bulkmail_autoresponder_timebased', $campaign->ID, $new_id );
			...
			if ( $schedule_new ) {
				$nextdate = bulkmail( 'helper' )->get_next_date_in_future( $starttime, $autoresponder_meta['interval'], $autoresponder_meta['time_frame'], $autoresponder_meta['weekdays'] );

				bulkmail( 'campaigns' )->update_meta( $campaign->ID, 'timestamp', $nextdate );
	...
	public function autoresponder_usertime( $campaign_id = null, $force = false ) {
		...
		foreach ( $campaigns as $campaign ) {
			...
			if ( 'bulkmail_autoresponder_usertime' != $autoresponder_meta['action'] ) {
				continue;
			}
			...
			$date_fields = bulkmail()->get_custom_date_fields( true );

			if ( ! in_array( $autoresponder_meta['uservalue'], $date_fields ) ) {
				bulkmail_notice( sprintf( 'Auto responder campaign %s has been deactivated caused by a missing date field. Please update your campaign!', '<strong>"<a href="post.php?post=' . $campaign->ID . '&action=edit">' . $campaign->post_title . '</a>"</strong>' ), 'error', false, 'camp_error_' . $campaign->ID, $campaign->post_author );
				bulkmail( 'campaigns' )->update_meta( $campaign->ID, 'active', false );
				$this->remove( $campaign->ID );
				continue;
			}
			...
			$subscribers = bulkmail( 'subscribers' )->query(
				array(
			...
			foreach ( $subscribers as $subscriber ) {
				...
				if ( $nextdate - $now < 0 && ! $exact_date ) {
					$nextdate = bulkmail( 'helper' )->get_next_date_in_future( $nextdate, $autoresponder_meta['useramount'], $autoresponder_meta['userunit'] );
				}
			...
			if ( ! empty( $subscriber_ids ) ) {
				if ( $timezone_based ) {
					$timestamps = bulkmail( 'subscribers' )->get_timeoffset_timestamps( $subscriber_ids, $timestamps );
				}

				$this->bulk_add( $campaign->ID, $subscriber_ids, $timestamps, 15 );

				do_action( 'bulkmail_autoresponder_usertime', $campaign->ID, $subscriber_ids );
			}
		}
	}
	...
	public function finish_campaigns() {
		...
		$wpdb->query( "DELETE a FROM {$wpdb->prefix}bulkmail_queue AS a LEFT JOIN {$wpdb->prefix}bulkmail_subscribers AS b ON a.subscriber_id = b.ID WHERE (a.sent = 0 OR a.ignore_status = 1) AND b.status != 1 AND a.campaign_id != 0" );

		// select all active campaigns
		$sql = "SELECT posts.ID, queue.sent FROM {$wpdb->prefix}posts AS posts LEFT JOIN {$wpdb->prefix}bulkmail_queue AS queue ON posts.ID = queue.campaign_id LEFT JOIN {$wpdb->prefix}bulkmail_actions AS actions ON actions.subscriber_id = queue.subscriber_id AND actions.campaign_id = queue.campaign_id AND actions.type = 1 WHERE posts.post_status IN ('active') AND posts.post_type = 'newsletter' AND queue.requeued = 0 GROUP BY posts.ID HAVING SUM(queue.sent = 0) = 0 OR queue.sent IS NULL";
		...
		foreach ( $ids as $id ) {
			...
			bulkmail( 'campaigns' )->finish( $id );

		}

		// remove notifications which are sent over an hour ago
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}bulkmail_queue WHERE sent != 0 AND campaign_id = 0 AND sent < %d", ( time() - 3600 ) ) );
	...
	public function progress( $campaign_id = null, $process_id = 0 ) {
		...
		if ( ( $pid = bulkmail( 'cron' )->lock( $process_id ) ) !== true ) {

			echo '<h2>' . esc_html__( 'Cron Lock Enabled!', 'bulkmail' ) . '</h2>';
			$sec = isset( $last_hit['timestamp'] ) ? ( round( time() - $last_hit['timestamp'] ) ) : 0;

			if ( is_user_logged_in() ) {
				echo '<p>' . esc_html__( 'Another process is currently running the cron process and you have been temporary blocked to prevent duplicate emails getting sent out.', 'bulkmail' ) . '</p>';

				echo '<p>' . sprintf( esc_html__( 'Read more about Cron Locks %s.', 'bulkmail' ), '<a href="https://emailmarketing.run/">' . esc_html__( 'here', 'bulkmail' ) . '</a>' );

				if ( $last_hit ) {
					echo '<p>' . sprintf(
						esc_html__( 'Cron Lock requested %s ago from:', 'bulkmail' ),
						'<strong>' . ( $sec > 60 ? human_time_diff( time() + $sec ) : sprintf( esc_html__( _n( '%d second', '%d seconds', $sec, 'bulkmail' ) ), $sec ) ) . '</strong>'
					) . '</p>';
			...
			// unlock?
			$unlock = apply_filters( 'mymail_unlock_cron', apply_filters( 'bulkmail_unlock_cron', false ) );
			if ( $unlock && $sec > max( 600, $unlock ) ) {
				// unlock automatically but with a minimum of 10 minutes
				bulkmail( 'cron' )->unlock( $process_id );
				echo '<h2>' . esc_html__( 'Cron Lock has been released!', 'bulkmail' ) . '</h2>';
			} else {
				return false;
			}
		}
		...
		update_option( 'bulkmail_cron_lasthit', $last_hit );
		...
		@set_time_limit( 0 );
		$send_at_once       = bulkmail_option( 'send_at_once' );
		$max_bounces        = bulkmail_option( 'bounce_attempts' );
		$max_execution_time = bulkmail_option( 'max_execution_time', 0 );

		$sent_this_turn       = 0;
		$send_delay           = bulkmail_option( 'send_delay', 0 ) / 1000;
		$mail_send_time       = 0;
		$MID                  = bulkmail_option( 'ID' );
		$unsubscribe_homepage = apply_filters( 'mymail_unsubscribe_link', apply_filters( 'bulkmail_unsubscribe_link', ( get_page( bulkmail_option( 'homepage' ) ) ? get_permalink( bulkmail_option( 'homepage' ) ) : get_bloginfo( 'url' ) ) ) );

		$campaign_errors = array();

		$to_send = $this->size( $microtime, $campaign_id );

		$queue_update_sql = "UPDATE {$wpdb->prefix}bulkmail_queue SET sent = %d, error = %d, priority = %d, count = %d WHERE subscriber_id = %d AND campaign_id = %d AND requeued = %d AND options = %s LIMIT 1";
		...
		if ( $to_send ) {
			...
			$sql .= " FROM {$wpdb->prefix}bulkmail_queue AS queue";
			$sql .= " LEFT JOIN {$wpdb->posts} AS posts ON posts.ID = queue.campaign_id";
			$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_subscribers AS subscribers ON subscribers.ID = queue.subscriber_id";
			...
			$sql .= ! bulkmail_option( 'split_campaigns' ) ? ', queue.campaign_id ASC' : '';
			...
			foreach ( $result as $i => $data ) {
				...
				if ( $max_execution_time && microtime( true ) - $globaltime > $max_execution_time - 1 ) {
					$this->cron_log( '', '&nbsp;<span class="error">' . esc_html__( 'timeout reached', 'bulkmail' ) . '</span>', '', '', '' );
					if ( ! $send_this_turn ) {
						bulkmail_notice( sprintf( esc_html__( 'Bulkmail is not able to send your campaign cause of a server timeout. Please increase the %1$s on the %2$s', 'bulkmail' ), '<strong>&quot;' . esc_html__( 'Max. Execution Time', 'bulkmail' ) . '&quot;</strong>', '<a href="edit.php?post_type=newsletter&page=bulkmail_settings&bulkmail_remove_notice=max_execution_time#delivery">' . esc_html__( 'settings page', 'bulkmail' ) . '</a>' ), 'error', false, 'max_execution_time' );
					}
				...
				$data = apply_filters( 'bulkmail_queue_campaign_subscriber_data', $data );

				if ( $data->campaign_id ) {

					if ( ! $data->_requeued ) {
						// prevent to send duplicates within one minute
						if ( $duplicate = $wpdb->get_results( $wpdb->prepare( "SELECT subscriber_id FROM {$wpdb->prefix}bulkmail_actions WHERE campaign_id = %d AND subscriber_id = %d AND type = %d && `timestamp` > %d", $data->campaign_id, $data->subscriber_id, 1, time() - 60 ) ) ) {
							$this->cron_log( '', '&nbsp;<span class="error">' . $data->subscriber_id . ' ' . $data->email . '<br>' . esc_html__( 'Prevent to send duplicate within one minute.', 'bulkmail' ) . '</span>', $data->campaign_id, $data->_count, '' );
							continue;
						}
					}
					...
					// regular campaign
					$result = bulkmail( 'campaigns' )->send( $data->campaign_id, $data->subscriber_id, null, false, true, $tags );
					...
					if ( $options = @unserialize( $data->_options ) ) {
						$result = bulkmail( 'notification' )->send( $data->subscriber_id, $options );
					} else {
						continue;
					}
				} else {
				...
				if ( ! is_wp_error( $result ) ) {
					...
					if ( $result->get_error_code() == 'user_error' ) {
						...
						if ( $error ) {
							do_action( 'bulkmail_subscriber_error', $data->subscriber_id, $data->campaign_id, $result->get_error_message() );

							bulkmail( 'subscribers' )->change_status( $data->subscriber_id, 4 );
						}
						...
						if ( $error ) {
							if ( isset( $options['template'] ) && $options['template'] ) {
								bulkmail_notice( sprintf( esc_html__( 'Notification %1$s has thrown an error: %2$s', 'bulkmail' ), '<strong>&quot;' . $options['template'] . '&quot;</strong>', '<strong>' . implode( '', $result->get_error_messages() ) ) . '</strong>', 'error', 7200, 'notification_error_' . $options['template'] );
							}

							do_action( 'bulkmail_notification_error', $data->subscriber_id, $result->get_error_message() );
						}

						// campaign_error
					} elseif ( $result->get_error_code() == 'error' ) {

						$campaign = bulkmail( 'campaigns' )->get( $data->campaign_id );

						if ( $campaign->post_status == 'autoresponder' ) {
							bulkmail_notice( sprintf( esc_html__( 'Autoresponder %1$s has caused sending error: %2$s', 'bulkmail' ), '<a href="post.php?post=' . $campaign->ID . '&action=edit"><strong>' . $campaign->post_title . '</strong></a>', '<strong>' . implode( '', $result->get_error_messages() ) ) . '</strong>', 'success', true, 'camp_error_' . $campaign->ID, $campaign->post_author );

						} else {

							if ( bulkmail_option( 'pause_campaigns' ) ) {
								bulkmail( 'campaigns' )->change_status( $campaign, 'paused' );
								bulkmail_notice( sprintf( esc_html__( 'Campaign %1$s has been paused cause of a sending error: %2$s', 'bulkmail' ), '<a href="post.php?post=' . $campaign->ID . '&action=edit"><strong>' . $campaign->post_title . '</strong></a>', '<strong>' . implode( '', $result->get_error_messages() ) ) . '</strong>', 'error', 7200, 'camp_error_' . $campaign->ID, $campaign->post_author );

							} else {
								bulkmail_notice( sprintf( esc_html__( 'Campaign %1$s has some delay cause of a sending error: %2$s', 'bulkmail' ), '<a href="post.php?post=' . $campaign->ID . '&action=edit"><strong>' . $campaign->post_title . '</strong></a>', '<strong>' . implode( '', $result->get_error_messages() ) ) . '</strong>', 'success', true, 'camp_error_' . $campaign->ID, $campaign->post_author );

							}
						}

						array_push( $campaign_errors, $data->campaign_id );
						do_action( 'bulkmail_campaign_error', $data->subscriber_id, $data->campaign_id, $result->get_error_message() );

						// system_error
					} elseif ( $result->get_error_code() == 'system_error' ) {

						array_push( $campaign_errors, $data->campaign_id );
						do_action( 'bulkmail_system_error', $data->subscriber_id, $data->campaign_id, $result->get_error_message() );
		...
		if ( $sent_this_turn ) {
			...
			bulkmail_remove_notice( 'max_execution_time' );
		...
		do_action( 'bulkmail_cron_finished' );
	...
	public function cron_log() {
		...
		global $bulkmail_cron_log, $bulkmail_cron_log_max_fields;

		if ( ! $bulkmail_cron_log ) {
			$bulkmail_cron_log = array();
		}

		if ( $a = func_get_args() ) {
			array_unshift( $a, microtime( true ) );
			$bulkmail_cron_log[]          = $a;
			$bulkmail_cron_log_max_fields = max( $bulkmail_cron_log_max_fields || 0, count( $a ) );
		} else {
			$bulkmail_cron_log_max_fields = 0;
			$bulkmail_cron_log[]          = array();
		}
	...
	public function show_cron_log() {
		...
		$i    = 1;
		foreach ( $bulkmail_cron_log as $logs ) {
			if ( empty( $logs ) ) {
				$i     = 1;
			...
			$html .= str_repeat( '<td>&nbsp;</td>', max( 0, ( $bulkmail_cron_log_max_fields + 2 ) - $j - 4 ) );
	...
	public function get_job_count( $campaign_id = null, $timestamp = null ) {
		...
		$sql = "SELECT COUNT(queue.subscriber_id) AS count FROM {$wpdb->prefix}bulkmail_queue AS queue WHERE queue.sent = 0 AND queue.timestamp > %d AND queue.campaign_id = %d";
