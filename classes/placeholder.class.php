...
class BulkmailPlaceholder {
	...
	public function set_campaign( $id ) {
		$this->campaignID = $id;
		$autoresponder    = bulkmail( 'campaigns' )->meta( $id, 'autoresponder' );
	...
	public function add_defaults( $campaign_id = null, $args = array() ) {
		...
		if ( $campaign_id ) {
			...
			if ( ! $meta ) {
				$meta = bulkmail( 'campaigns' )->meta_defaults();
			}
			...
			$defaults = wp_parse_args(
				array(
					...
					'lists'          => bulkmail( 'campaigns' )->get_formated_lists( $campaign_id ),
				),
				$defaults
			);
		...
		$this->add( apply_filters( 'bulkmail_placeholder_defaults', $args, $campaign_id ) );
	}
	...
	public function add_custom( $campaign_id, $args = array() ) {
		...
		$unsubscribelink = bulkmail()->get_unsubscribe_link( $campaign_id, $this->subscriberHash );
		$forwardlink     = bulkmail()->get_forward_link( $campaign_id );
		$profilelink     = bulkmail()->get_profile_link( $campaign_id, $this->subscriberHash );

		$defaults = array(
			'webversionlinktext' => bulkmail_text( 'webversion' ),
			'unsublinktext'      => bulkmail_text( 'unsubscribelink' ),
			'forwardlinktext'    => bulkmail_text( 'forward' ),
			'profilelinktext'    => bulkmail_text( 'profile' ),
		...
		$this->add( apply_filters( 'bulkmail_placeholder_custom', $args, $campaign_id, $this->subscriberID ) );
	}
	...
	public function do_placeholder( $removeunused = true, $placeholders = array(), $relative_to_absolute = false ) {
		...
		$this->add( bulkmail_option( 'custom_tags', array() ) );
		$this->add( bulkmail_option( 'tags', array() ) );
		...
		while ( false !== strpos( $this->content, '{' ) ) {
			...
			if ( strpos( $this->content, '<style' ) !== false && preg_match_all( '#(<style(>|[^<]+?>)([^<]+)<\/style>)#', $this->content, $styles ) ) {
				foreach ( $styles[0] as $style ) {
					$this->content  = str_replace( $style, '<!--Bulkmail:styleblock' . count( $this->styles ) . '-->', $this->content );
					$this->styles[] = $style;
				}
			}
		...
		if ( ! empty( $this->keeptag ) ) {
			foreach ( $this->keeptag as $i => $keeptag ) {
				$this->content = str_replace( '<!--Bulkmail:keeptag' . $i . '-->', $keeptag, $this->content );
			}
		}

		if ( ! empty( $this->styles ) ) {
			foreach ( $this->styles as $i => $style ) {
				$this->content = str_replace( '<!--Bulkmail:styleblock' . $i . '-->', $style, $this->content );
			}
		}
	...
	private function share_service( $url, $title = '' ) {
		...
		$social = implode( '|', apply_filters( 'mymail_share_services', apply_filters( 'bulkmail_share_services', $social ) ) );
	...
	private function get_social_service( $service, $url, $title = '', $fallback = '' ) {
		...
		if ( ! $this->social_services ) {
			$this->social_services = bulkmail( 'helper' )->social_services();
		}
		...
		$content = '<img alt="' . esc_attr( sprintf( esc_html__( 'Share this on %s', 'bulkmail' ), $this->social_services[ $service ]['name'] ) ) . '" src="' . BULKEMAIL_URI . 'assets/img/share/share_' . $service . '.png" style="display:inline;display:inline !important;" />';

		$content = apply_filters( 'mymail_share_button_' . $service, apply_filters( 'bulkmail_share_button_' . $service, $content ) );
	...
	private function rss() {
		...
		if ( preg_match_all( '#<module[^>]*?data-rss="(.*?)".*?</module>#ms', $this->content, $modules ) ) {
			...
			foreach ( $modules[0] as $i => $html ) {
				...
				// $last_feed_timestamp = bulkmail( 'helper' )->new_feed_since( $this->rss_timestamp, $feed_url );
				$feeds = bulkmail( 'helper' )->get_feed_since( $this->rss_timestamp, $feed_url );

				if ( ! empty( $feeds ) ) {
					$replace        = '';
					$id             = md5( $feed_url );
					$temp_post_type = 'bulkmail_rss_' . $id;

					if ( ! isset( $this->rss[ $id ] ) ) {
						$this->rss[ 'bulkmail_get_last_post_' . $temp_post_type ] = $feed_url;
						add_filter( 'bulkmail_get_last_post_' . $temp_post_type, array( &$this, 'rss_replace' ), 5, 6 );
						if ( $i == 0 ) {
					...
					$replace            = str_replace( 'bulkmail_image_placeholder&amp;tag=rss', 'bulkmail_image_placeholder&amp;tag=' . $temp_post_type, $replace );
					$replace            = str_replace( 'bulkmail_image_placeholder&tag=rss', 'bulkmail_image_placeholder&tag=' . $temp_post_type, $replace );
			...
			if ( $first ) {
				...
				$this->content = str_replace( 'bulkmail_image_placeholder&amp;tag=rss', 'bulkmail_image_placeholder&amp;tag=' . $first, $this->content );
				$this->content = str_replace( 'bulkmail_image_placeholder&tag=rss', 'bulkmail_image_placeholder&tag=' . $first, $this->content );
			}
		}
	...
	public function rss_replace( $return, $args, $offset, $term_ids, $campaign_id, $subscriber_id ) {
		...
		return bulkmail( 'helper' )->feed( $feed_url, $offset );
		...
		if ( preg_match_all( '#<module[^>]*?data-tag="{(([a-z0-9_-]+):(-|~)?([\d]+)(;([0-9;,-]+))?)\}"(.*?)".*?</module>#ms', $this->content, $modules ) ) {
			...
			foreach ( $modules[0] as $i => $html ) {
				...
				if ( empty( $type ) ) {
					...
					if ( '~' == $type ) {
						$post = bulkmail()->get_random_post( $post_or_offset, $post_type, $term_ids, $this->last_post_args, $this->campaignID, $this->subscriberID );
					} else {
						$post = bulkmail()->get_last_post( $post_or_offset - 1 + $this->index_offset, $post_type, $term_ids, $this->last_post_args, $this->campaignID, $this->subscriberID );
					}
				}
	...
	public function conditions() {
		...
		if ( preg_match_all( '#<(module|single|multi)[^>]*?condition="([a-z0-9-_]+)([=!GLTE\^$]+)(.*?)".*?</(\\1)>#ms', $this->content, $conditions ) ) {
			...
			$subscriber = $this->subscriberID ? bulkmail( 'subscribers' )->get( $this->subscriberID, true ) : false;
	...
	private function replace_images( $relative_to_absolute = false ) {
		...
		// placeholder images
		if ( $count = preg_match_all( '#<(img|td|th|v:fill)([^>]*)(src|background)="(.*)\?action=bulkmail_image_placeholder([^"]+)"([^>]*)>#', $this->content, $hits ) ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				if ( isset( $query['tag'] ) ) {
					...
					$replace_to = bulkmail_cache_get( 'bulkmail_' . $querystring );
					...
					if ( false === $replace_to ) {
						...
						// not on RSS
						if ( $relative_to_absolute && 0 === strpos( $post_type, 'bulkmail_rss_' ) ) {
						...
						if ( $is_post ) {
							...
							$fallback_id           = isset( $extra[1] ) ? (int) $extra[1] : bulkmail_option( 'fallback_image' );
							$post_id_or_identifier = array_shift( $term_ids );
							$is_random             = 0 === strpos( $post_id_or_identifier, '~' );

							if ( $is_random ) {

								$post = bulkmail()->get_random_post( substr( $post_id_or_identifier, 1 ), $post_type, $term_ids, $this->last_post_args, $this->campaignID, $this->subscriberID );

							} elseif ( $post_id_or_identifier < 0 ) {

								$post = bulkmail()->get_last_post( abs( $post_id_or_identifier ) - 1 + $this->index_offset, $post_type, $term_ids, $this->last_post_args, $this->campaignID, $this->subscriberID );
							...
							$fallback_id = bulkmail_option( 'fallback_image' );
							$post        = null;
							$thumb_id    = null;
							$src         = apply_filters( 'bulkmail_image_placeholder', $query['tag'], $width, $height, $crop, $original, $this->campaignID, $this->subscriberID );
						...
						if ( ! $relative_to_absolute ) {
							...
							if ( ! empty( $post ) ) {
								...
								if ( empty( $org_src ) && isset( $post->post_image ) ) {
									if ( is_numeric( $post->post_image ) ) {
										...
										if ( $src = bulkmail( 'helper' )->get_meta_tags_from_url( $post->post_permalink, array( 'bulkmail:image', 'thumbnail', 'og:image', 'twitter:image' ) ) ) {
							...
							if ( ! empty( $org_src ) ) {
								...
								if ( $org_src[1] && $org_src[2] ) {
									...
									$img    = bulkmail( 'helper' )->create_image( $thumb_id, $org_src[0], $width, $height * $factor, $crop, $original );
								} else {
							...
							bulkmail_cache_set( 'bulkmail_' . $querystring, $replace_to );
					...
					if ( false !== $replace_to ) {
						$replace_to = apply_filters( 'bulkmail_replace_image', $replace_to, $search, $this->campaignID, $this->subscriberID );
	...
	private function get_post_types_to_replace() {
		if ( empty( $this->post_types ) ) {
			$this->post_types = bulkmail( 'helper' )->get_dynamic_post_types();
		}
		...
		if ( $count = preg_match_all( '#\{(!)?((' . $pts . ')_([^}]+):(-|~)?([\d]+)(;([0-9;,-]+))?)\}#i', $this->content, $hits ) ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				if ( empty( $type ) || $is_random ) {
					...
					$post = apply_filters( 'bulkmail_get_post_' . $post_type, $post, $post_id );
					...
					if ( empty( $post->post_excerpt ) ) {
						...
						$post->post_excerpt = bulkmail_remove_block_comments( $post->post_excerpt );
					}
		...
		if ( $count = preg_match_all( '#\{(!)?([a-z0-9-_]+):?([^\}|]+)?\|?([^\}]+)?\}#i', $this->content, $hits_fallback ) ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				if ( isset( $this->placeholder[ $search ] ) ) {
					...
					if ( $replace && '{preheader}' == $search && apply_filters( 'bulkmail_preview_text_fix', true ) ) {
					...
					// tag is a custom tag
				} elseif ( isset( $bulkmail_tags[ $tag ] ) && $this->replace_custom ) {
					$replace = call_user_func_array( $bulkmail_tags[ $tag ], array( $option, $fallback, $this->campaignID, $this->subscriberID ) );
				...
				$replace_to = apply_filters( 'bulkmail_replace_' . $tag, $replace, $option, $fallback, $this->campaignID, $this->subscriberID );
	...
	public function get_replace( $post, $what ) {
		...
		switch ( $what ) {
			...
			case 'modified':
			case 'modified_gmt':
				$replace_to = date( bulkmail( 'helper' )->dateformat(), strtotime( $post->{'post_' . $what} ) );
				break;
			case 'time':
				$what = 'date';
			case 'time_gmt':
			...
			case 'modified_time_gmt':
				...
				$replace_to = date( bulkmail( 'helper' )->timeformat(), strtotime( $post->{'post_' . $what} ) );
				break;
			case 'excerpt':
				if ( ! empty( $post->{'post_excerpt'} ) ) {
					$replace_to = wpautop( $post->{'post_excerpt'} );
				} else {
					$replace_to = bulkmail( 'helper' )->get_excerpt( $post_content );
				}
				break;
			case 'content':
			...
			case 'linkedin':
				...
				break;
			case 'image':
				$replace_to = '[' . ( sprintf( esc_html__( 'use the tag %s as url in the editbar', 'bulkmail' ), '"' . $hits[1][ $i ] . '"' ) ) . ']';
				break;
			default:
		...
		return apply_filters( 'bulkmail_replace_' . $post_type . '_' . $what, $replace_to, $post, $extra, $this->campaignID, $this->subscriberID );
	...
	private function get_last_tweet( $username, $fallback = '' ) {
		...
		if ( false === ( $tweet = get_transient( 'bulkmail_tweet_' . $username ) ) ) {

			$token           = bulkmail_option( 'twitter_token' );
			$token_secret    = bulkmail_option( 'twitter_token_secret' );
			$consumer_key    = bulkmail_option( 'twitter_consumer_key' );
			$consumer_secret = bulkmail_option( 'twitter_consumer_secret' );

			if ( ! $token || ! $token_secret || ! $consumer_key || ! $consumer_secret ) {

				return esc_html__( 'Please enter your Twitter application credentials on the settings page', 'bulkmail' );

			}

			require_once BULKEMAIL_DIR . 'classes/libs/twitter.class.php';
			...
			set_transient( 'bulkmail_tweet_' . $username, $tweet, 60 * bulkmail_option( 'tweet_cache_time' ) );
		}
	...
	private function get_keep_tags() {
		if ( is_null( $this->keeptags ) ) {
			$this->keeptags = array_unique( (array) apply_filters( 'bulkmail_keep_tags', array() ) );
		}
