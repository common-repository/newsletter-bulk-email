...
class BulkmailMail {
	...
	private function __construct() {
		...
		$this->deliverymethod = bulkmail_option( 'deliverymethod', 'simple' );

		$this->dkim = bulkmail_option( 'dkim' );

		require_once BULKEMAIL_DIR . 'classes/mail.helper.class.php';
		$this->mailer = new bulkmail_mail_helper( true );

		if ( $this->deliverymethod == 'smtp' ) {

			$this->mailer->IsSMTP();

			$this->mailer->Host     = bulkmail_option( 'smtp_host' );
			$this->mailer->Port     = bulkmail_option( 'smtp_port', 25 );
			$this->mailer->Timeout  = bulkmail_option( 'smtp_timeout', 10 );
			$this->mailer->SMTPAuth = ! ! bulkmail_option( 'smtp_auth', false );
			if ( $this->mailer->SMTPAuth ) {
				$this->mailer->AuthType = bulkmail_option( 'smtp_auth', '' );
				$this->mailer->Username = bulkmail_option( 'smtp_user' );
				$this->mailer->Password = bulkmail_option( 'smtp_pwd' );
			}
			$this->mailer->SMTPSecure    = bulkmail_option( 'smtp_secure', '' );
			$this->mailer->SMTPKeepAlive = true;

			add_action( 'bulkmail_presend', array( &$this, 'pre_send' ), 1, 10 );
			add_action( 'bulkmail_dosend', array( &$this, 'do_send' ), 1, 10 );

		} elseif ( $this->deliverymethod == 'simple' ) {

			$method = bulkmail_option( 'simplemethod' );

			if ( $method == 'sendmail' ) {
				$this->mailer->Sendmail = bulkmail_option( 'sendmail_path' );
		...
		do_action( 'bulkmail_initsend', $this );

		if ( $this->dkim ) {
			$this->mailer->DKIM_selector = bulkmail_option( 'dkim_selector' );
			$this->mailer->DKIM_domain   = bulkmail_option( 'dkim_domain' );

			$folder = BULKEMAIL_UPLOAD_DIR . '/dkim';

			$this->mailer->DKIM_private    = $folder . '/' . bulkmail_option( 'dkim_private_hash' ) . '.pem';
			$this->mailer->DKIM_passphrase = bulkmail_option( 'dkim_passphrase' );
			$this->mailer->DKIM_identity   = bulkmail_option( 'dkim_identity' );
		...
		$this->from      = bulkmail_option( 'from' );
		$this->from_name = bulkmail_option( 'from_name' );

		$this->send_limit = bulkmail_option( 'send_limit' );
		...
		$this->subscriber_errors = apply_filters( 'mymail_subscriber_errors', apply_filters( 'bulkmail_subscriber_errors', $subscriber_errors ) );
		$system_errors           = array(
			'Not in Time Frame',
		);
		$this->system_errors     = apply_filters( 'bulkmail_system_errors', $system_errors );

		if ( ! get_transient( '_bulkmail_send_period_timeout' ) ) {
			set_transient( '_bulkmail_send_period_timeout', true, bulkmail_option( 'send_period' ) * 3600 );
		} else {
			$this->sent_within_period = get_transient( '_bulkmail_send_period' );
		...
		if ( $this->sentlimitreached ) {
			$msg = sprintf( esc_html__( 'Sent limit of %1$s reached! You have to wait %2$s before you can send more mails!', 'bulkmail' ), '<strong>' . $this->send_limit . '</strong>', '<strong>' . human_time_diff( get_option( '_transient_timeout__bulkmail_send_period_timeout' ) ) . '</strong>' );
			bulkmail_notice( $msg, 'error', false, 'dailylimit' );
	...
	public function __set( $name, $value ) {
		switch ( $name ) {
			case 'mailer':
				break;
			default:
				$this->{$name} = apply_filters( "bulkmail_mail_set_{$name}", $value );
		}
	}
	...
	public function set_campaign( $id ) {
		...
		$this->baselink   = bulkmail()->get_base_link( (int) $id );
	}
	...
	public function send_notification( $content, $headline = null, $replace = array(), $force = false, $file = 'notification.html', $template = null ) {
		...
		if ( $template && $file ) {
			$template_obj  = bulkmail( 'template', $template, $file );
		...
		$placeholder = bulkmail( 'placeholder', $this->content );
		...
		$this->content = bulkmail( 'helper' )->prepare_content( $this->content );
		if ( apply_filters( 'bulkmail_inline_css', true ) ) {
			$content = bulkmail( 'helper' )->inline_css( $this->content );
		}
		...
		$this->embed_images       = bulkmail_option( 'embed_images' );
	...
	public function send() {
		...
		do_action( 'bulkmail_presend', $this );
		if ( ! $this->pre_send ) {
			return false;
		}

		do_action( 'bulkmail_dosend', $this );

		if ( $this->sent ) {

			$this->sent_within_period++;
			$this->sentlimitreached = $this->sent_within_period >= $this->send_limit;
			set_transient( '_bulkmail_send_period', $this->sent_within_period );
	...
	public function pre_send() {
		...
		try {
			...
			if ( $this->embed_images ) {
				...
				$this->mailer->msgHTML( $this->content, trailingslashit( dirname( BULKEMAIL_UPLOAD_DIR ) ) );
			} else {
				$this->mailer->Body = $this->mailer->normalizeBreaks( $this->content );
			}

			$this->mailer->AltBody = $this->mailer->normalizeBreaks( ! empty( $this->plaintext ) ? $this->plaintext : bulkmail( 'helper' )->plain_text( $this->content ) );
			...
			$this->mailer->messageID = sprintf(
				'<%s@%s>',
				$this->messageID . '-' . $this->hash . '-' . $this->campaignID . '-' . bulkmail_option( 'ID' ),
				$this->hostname
			);
	...
	public function get_errors( $format = '' ) {
		...
		switch ( $format ) {
			case 'ul':
			case 'ol':
				$html = '<' . $format . ' class="bulkmail-mail-error">';
			...
			case 'br':
				$format = '<br>';
			default:
				$html = '<span class="bulkmail-mail-error">';
				foreach ( $messages as $msg ) {
					$html .= $format . $msg . "\n";
				}
				$html  .= '</span>';
	...
	public function make_img_relative( $html ) {
		$html = str_replace( ' src="' . trailingslashit( dirname( BULKEMAIL_UPLOAD_URI ) ), ' src="', $html );
		return $html;
	}
