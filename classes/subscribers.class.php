...
class BulkmailSubscribers {
	...
	public function init() {
		...
		add_action( 'bulkmail_cron_worker', array( &$this, 'send_confirmations' ) );
		add_action( 'bulkmail_subscriber_subscribed', array( &$this, 'remove_pending_confirmations' ) );

		add_action( 'bulkmail_subscriber_notification', array( &$this, 'subscriber_delayed_notification' ) );
		add_action( 'bulkmail_unsubscribe_notification', array( &$this, 'subscriber_delayed_unsubscribe_notification' ) );
		...
		add_action( 'bulkmail_update_rating', array( &$this, 'update_rating' ) );
	...
	public function admin_menu() {
		...
		$page = add_submenu_page( 'edit.php?post_type=newsletter', esc_html__( 'Subscribers', 'bulkmail' ), esc_html__( 'Subscribers', 'bulkmail' ), 'bulkmail_edit_subscribers', 'bulkmail_subscribers', array( &$this, 'view_subscribers' ) );
	...
	public function script_styles() {
		...
		if ( isset( $_GET['ID'] ) || isset( $_GET['new'] ) ) :
			...
			wp_enqueue_style( 'bulkmail-subscriber-detail', BULKEMAIL_URI . 'assets/css/subscriber-style' . $suffix . '.css', array(), BULKEMAIL_VERSION );
			wp_enqueue_script( 'bulkmail-subscriber-detail', BULKEMAIL_URI . 'assets/js/subscriber-script' . $suffix . '.js', array( 'bulkmail-script' ), BULKEMAIL_VERSION, true );
			...
			bulkmail_localize_script(
				...
				array(
					...
					'invalid_email' => esc_html__( 'This isn\'t a valid email address!', 'bulkmail' ),
					'email_exists'  => esc_html__( 'This email address already exists!', 'bulkmail' ),
				)
			);

		else :

			wp_enqueue_style( 'bulkmail-subscribers-table', BULKEMAIL_URI . 'assets/css/subscribers-table-style' . $suffix . '.css', array(), BULKEMAIL_VERSION );
			wp_enqueue_script( 'bulkmail-subscribers-table', BULKEMAIL_URI . 'assets/js/subscribers-table-script' . $suffix . '.js', array( 'bulkmail-script' ), BULKEMAIL_VERSION, true );
			bulkmail_localize_script(
				'subscribers',
				array(
					'onbeforeunload' => esc_html__( 'Bulk process in progress!', 'bulkmail' ),
					'initprogess'    => sprintf( esc_html__( 'processing page %d', 'bulkmail' ), 1 ),
				)
			);
	...
	public function get_columns() {
		...
		$columns['lists']  = esc_html__( 'Lists', 'bulkmail' );
		$columns['emails'] = esc_html__( 'Emails', 'bulkmail' );
		$columns['status'] = esc_html__( 'Status', 'bulkmail' );
		$columns['signup'] = esc_html__( 'Subscribed', 'bulkmail' );
	...
	public function bulk_actions() {
		...
		if ( isset( $_POST['all_subscribers'] ) && $_POST['all_subscribers'] ) {
			...
			$subscriber_ids = bulkmail( 'subscribers' )->query(
				wp_parse_args(
					$args,
					array(
						'status'     => $status,
		...
		switch ( $action ) {
			...
			case 'subscribed':
				if ( $count = $this->change_status( $subscriber_ids, 1 ) ) {
					$success_message = sprintf( esc_html__( '%d Subscribers have been subscribed', 'bulkmail' ), $count );
				}
				break;

			case 'unsubscribed':
				if ( $count = $this->change_status( $subscriber_ids, 2 ) ) {
					$success_message = sprintf( esc_html__( '%d Subscribers have been unsubscribed', 'bulkmail' ), $count );
				}
				break;

			case 'pending':
				if ( $count = $this->change_status( $subscriber_ids, 0 ) ) {
					$success_message = sprintf( esc_html__( '%d Subscribers have been set to pending', 'bulkmail' ), $count );
				}
				break;

			case 'send_campaign':
				$listid = bulkmail( 'lists' )->add_segment();

				if ( $this->assign_lists( $subscriber_ids, $listid, false, true ) ) {
					$count           = count( $subscriber_ids );
					$success_message = sprintf( esc_html__( '%d Subscribers have been assigned to a new list', 'bulkmail' ), $coun );
				}
				break;

			case 'confirmation':
				if ( $count = $this->send_confirmations( $subscriber_ids, true, true ) ) {
					$success_message = sprintf( esc_html__( 'Confirmations sent to %d pending subscribers', 'bulkmail' ), $count );
				}
				break;
			...
			case 'verify':
				...
				if ( $unverfied ) {
					$error_message = sprintf( esc_html__( _n( '%s subscriber has not been verified!', '%s subscribers have not been verified!', $unverfied, 'bulkmail' ) ), $unverfied );
				}

				if ( $verfied ) {
					$success_message = sprintf( esc_html__( _n( '%s subscriber has been verified!', '%s subscribers have been verified!', $verfied, 'bulkmail' ) ), $verfied );
				}
			...
			default:
				if ( preg_match( '#^add_list_(\w+)#', $action, $match ) ) {
					...
					if ( $list = bulkmail( 'lists' )->get( $ids ) ) {
						$this->assign_lists( $subscriber_ids, $list->ID, false, true );
						$success_message = sprintf( esc_html__( '%1$d Subscribers added to list %2$s', 'bulkmail' ), count( $subscriber_ids ), '"<a href="edit.php?post_type=newsletter&page=bulkmail_lists&ID=' . $list->ID . '">' . $list->name . '</a>"' );
					}
				} elseif ( preg_match( '#^remove_list_(\w+)#', $action, $match ) ) {
					$ids = 'all' == $match[1] ? null : (int) $match[1];
					if ( $list = bulkmail( 'lists' )->get( $ids ) ) {
						$this->unassign_lists( $subscriber_ids, $list->ID, false, true );
						$success_message = sprintf( esc_html__( '%1$d Subscribers removed from list %2$s', 'bulkmail' ), count( $subscriber_ids ), '"<a href="edit.php?post_type=newsletter&page=bulkmail_lists&ID=' . $list->ID . '">' . $list->name . '</a>"' );
					}
				} elseif ( preg_match( '#^confirm_list_(\w+)#', $action, $match ) ) {
					$ids = 'all' == $match[1] ? null : (int) $match[1];
					if ( $list = bulkmail( 'lists' )->get( $ids ) ) {
						bulkmail( 'lists' )->confirm_subscribers( $list->ID, $subscriber_ids );
						$success_message = sprintf( esc_html__( '%1$d Subscribers confirmed to %2$s lists', 'bulkmail' ), count( $subscriber_ids ), count( $list ) );
					}
				} elseif ( preg_match( '#^unconfirm_list_(\w+)#', $action, $match ) ) {
					$ids = 'all' == $match[1] ? null : (int) $match[1];
					if ( $list = bulkmail( 'lists' )->get( $ids ) ) {
						bulkmail( 'lists' )->unconfirm_subscribers( $list->ID, $subscriber_ids );
						$success_message = sprintf( esc_html__( '%1$d Subscribers unconfirmed from %2$s lists', 'bulkmail' ), count( $subscriber_ids ), count( $list ) );
					}
				}
		...
		if ( $success_message ) {
			bulkmail_notice( $success_message . $message_postfix, 'success', true, 'subscriber_bulk_success', true, null, true );
		}

		if ( $error_message ) {
			bulkmail_notice( $error_message . $message_postfix, 'error', true, 'subscriber_bulk_error', true, null, true );
		}
		...
		if ( isset( $is_ajax ) ) {
			...
			wp_send_json(
				array(
					...
					'total'           => $total,
					'page'            => $page,
					'message'         => $finished ? '<span>' . esc_html__( 'Finished!', 'bulkmail' ) . '</span>' : '<span title="' . esc_html__( 'Check the browser console for more info!', 'bulkmail' ) . '">' . sprintf( esc_html__( 'processing page %1$s of %2$s', 'bulkmail' ), number_format_i18n( $page + 1 ), number_format_i18n( ceil( $total / $limit ) ) ) . '&hellip;</span>',
	...
	public function edit_entry() {
		...
		if ( isset( $_GET['new'] ) && isset( $_GET['wp_user'] ) ) {
			...
			if ( is_wp_error( $subscriber_id ) ) {
				...
				bulkmail_notice( __( $subscriber_id->get_error_message(), 'bulkmail' ), 'error', true );
				wp_redirect( 'edit.php?post_type=newsletter&page=bulkmail_subscribers' );

			} else {

				bulkmail_notice( esc_html__( 'Subscriber added', 'bulkmail' ), 'success', true );
				do_action( 'bulkmail_subscriber_save', $subscriber_id );
				wp_redirect( 'edit.php?post_type=newsletter&page=bulkmail_subscribers&ID=' . $subscriber_id );
				exit;
		...
		if ( isset( $_POST['bulkmail_data'] ) ) {
			...
			if ( isset( $_POST['save'] ) ) :
				...
				$old_subscriber_data = $this->get( (int) $_POST['bulkmail_data']['ID'], true );

				if ( $is_new && ! current_user_can( 'bulkmail_add_subscribers' ) ) {
					wp_die( esc_html__( 'You are not allowed to add subscribers!', 'bulkmail' ) );
				}

				if ( ! $is_new && ! current_user_can( 'bulkmail_edit_subscribers' ) ) {
					wp_die( esc_html__( 'You are not allowed to edit subscribers!', 'bulkmail' ) );
				}

				$entry = (object) stripslashes_deep( $_POST['bulkmail_data'] );
				...
				if ( is_wp_error( $subscriber_id ) ) {
					...
					switch ( $subscriber_id->get_error_code() ) {
						case 'email_exists':
							...
							$msg = sprintf( esc_html__( '%1$s already exists. %2$s', 'bulkmail' ), '<strong>&quot;' . $subscriber->email . '&quot;</strong>', '<a href="edit.php?post_type=newsletter&page=bulkmail_subscribers&ID=' . $subscriber->ID . '">' . esc_html__( 'Edit this user', 'bulkmail' ) . '</a>' );
							break;
						default:
							$msg = $subscriber_id->get_error_message();
					}

					bulkmail_notice( $msg, 'error', true );

				} else {

					$subscriber = $this->get( $subscriber_id, true );

					if ( isset( $_POST['bulkmail_lists'] ) ) {
						$lists = array_filter( $_POST['bulkmail_lists'], 'is_numeric' );
					} else {
						$lists = array();
					}
					...
					if ( ! $old_subscriber_data || $subscriber->status != $old_subscriber_data->status ) {
						if ( bulkmail_option( 'list_based_opt_in' ) ) {
							if ( 1 == $subscriber->status ) {
								bulkmail( 'lists' )->confirm_subscribers( null, $subscriber->ID );
							} elseif ( 0 == $subscriber->status ) {
								bulkmail( 'lists' )->unconfirm_subscribers( null, $subscriber->ID );
							}
						}
					}

					bulkmail_notice( $is_new ? esc_html__( 'Subscriber added', 'bulkmail' ) : esc_html__( 'Subscriber saved', 'bulkmail' ), 'success', true );
					do_action( 'bulkmail_subscriber_save', $subscriber->ID );
					wp_redirect( 'edit.php?post_type=newsletter&page=bulkmail_subscribers&ID=' . $subscriber->ID );
					exit;

				} elseif ( isset( $_POST['delete'] ) ) :

					if ( ! current_user_can( 'bulkmail_delete_subscribers' ) ) {
						wp_die( esc_html__( 'You are not allowed to delete subscribers!', 'bulkmail' ) );
					}

					if ( $subscriber = $this->get( (int) $_POST['bulkmail_data']['ID'], true ) ) {
						$success = $this->remove( $subscriber->ID );
						if ( ! $success ) {
							bulkmail_notice( esc_html__( 'There was an error while deleting subscribers!', 'bulkmail' ), 'error', true );

						} else {
							bulkmail_notice( sprintf( esc_html__( 'Subscriber %s has been removed', 'bulkmail' ), '<strong>&quot;' . $subscriber->email . '&quot;</strong>' ), 'error', true, true );
							do_action( 'bulkmail_subscriber_delete', $subscriber->ID, $subscriber->email );
						}
			...
			elseif ( isset( $_POST['confirmation'] ) ) :
				...
				if ( $subscriber = $this->get( (int) $_POST['bulkmail_data']['ID'], true ) ) {
					if ( $this->send_confirmations( $subscriber->ID, true, true ) ) {
						bulkmail_notice( esc_html__( 'Confirmation has been sent', 'bulkmail' ), 'success', true );
					}
					wp_redirect( 'edit.php?post_type=newsletter&page=bulkmail_subscribers&ID=' . $subscriber->ID );
					exit;
				};
		...
		if ( isset( $_GET['resendcampaign'] ) ) {
			if ( ! current_user_can( 'publish_newsletters' ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'bulkmail-resend-campaign' ) ) {
				wp_die( esc_html__( 'You are not allowed to publish campaigns!', 'bulkmail' ) );
			}
			...
			if ( $subscriber_id && $campaign_id && $subscriber = $this->get( $subscriber_id, true ) ) {
				...
				if ( $campaign && bulkmail( 'campaigns' )->send( $campaign_id, $subscriber_id, true, true ) ) {
					bulkmail_notice( sprintf( esc_html__( 'Campaign %s has been sent', 'bulkmail' ), '<strong>&quot;' . $campaign->post_title . '&quot;</strong>' ), 'success', true );
					wp_redirect( 'edit.php?post_type=newsletter&page=bulkmail_subscribers&ID=' . $subscriber_id );
					exit;
				}
			}
	...
	public function sync_all_subscriber( $limit = null, $offset = 0 ) {
		...
		$sql = "SELECT ID FROM {$wpdb->prefix}bulkmail_subscribers WHERE wp_id != 0";
	...
	public function sync_subscriber( $subscriber_id ) {
		...
		// delete cache
		bulkmail_cache_delete( 'get_custom_fields_' . $subscriber_id );
		$subscriber = $this->get( $subscriber_id, true );
	...
	public function sync_all_wp_user( $limit = null, $offset = 0 ) {
		...
		$sql = "SELECT wp_id AS ID FROM {$wpdb->prefix}bulkmail_subscribers WHERE wp_id != 0";
		if ( ! is_null( $limit ) ) {
			$sql .= ' LIMIT ' . (int) $offset . ', ' . (int) $limit;
		}
	...
	public function view_subscribers() {
		...
		if ( isset( $_GET['ID'] ) || isset( $_GET['new'] ) ) :
			...
			include BULKEMAIL_DIR . 'views/subscribers/detail.php';

		else :

			include BULKEMAIL_DIR . 'views/subscribers/overview.php';
		...
		if ( isset( $meta->referer ) ) :
			...
			switch ( $meta->referer ) {
				case 'import':
					...
					<strong><?php esc_html_e( 'via', 'bulkmail' ); ?></strong> <span><?php printf( esc_html__( 'import on %s', 'bulkmail' ), date( $timeformat, $subscriber->added + $timeoffset ) ); ?></span>
							<?php
					break;
				case 'wpuser':
				...
				case '/wp-login.php?action=register':
					?>
					<strong><?php esc_html_e( 'via', 'bulkmail' ); ?></strong> <span><?php printf( esc_html__( 'WP user on %s', 'bulkmail' ), date( $timeformat, $subscriber->added + $timeoffset ) ); ?></span>
									<?php
					break;
				case 'backend':
					?>
					<strong><?php esc_html_e( 'via', 'bulkmail' ); ?></strong> <span><?php printf( esc_html__( 'Backend on %s', 'bulkmail' ), date( $timeformat, $subscriber->added + $timeoffset ) ); ?></span>
							<?php
					break;
				case 'extern':
					?>
					<strong><?php esc_html_e( 'via', 'bulkmail' ); ?></strong> <span><?php printf( esc_html__( 'an extern form on %s', 'bulkmail' ), date( $timeformat, $subscriber->added + $timeoffset ) ); ?></span>
							<?php
					break;
				case '/':
					?>
					<strong><?php esc_html_e( 'via', 'bulkmail' ); ?></strong> <span><?php printf( esc_html__( 'Homepage on %s', 'bulkmail' ), date( $timeformat, $subscriber->added + $timeoffset ) ); ?></span>
						<?php
					break;
				default:
					if ( preg_match( '#^wpcomment_(\d+)#', $meta->referer, $match ) ) :
						...
						<strong><?php esc_html_e( 'via', 'bulkmail' ); ?></strong> <span><?php printf( esc_html__( '%1$s on %2$s', 'bulkmail' ), '<a href="' . get_permalink( $comment->comment_post_ID ) . '#comment-' . $comment->comment_ID . '">' . esc_html__( 'Comment', 'bulkmail' ) . '</a>', date( $timeformat, $subscriber->added + $timeoffset ) ); ?></span>
					<?php elseif ( preg_match( '#^https?://#', $meta->referer, $match ) ) : ?>

					<strong><?php esc_html_e( 'via', 'bulkmail' ); ?></strong> <a href="<?php echo $meta->referer; ?>"><?php echo $meta->referer; ?></a>
					<?php else : ?>

					<strong><?php esc_html_e( 'via', 'bulkmail' ); ?></strong> <?php echo $meta->referer; ?>
			...
			if ( isset( $meta->form ) ) :
				if ( $form = bulkmail( 'forms' )->get( $meta->form, false, false ) ) :
					?>
			<br><strong><?php esc_html_e( 'Form', 'bulkmail' ); ?> #<?php echo $form->ID; ?>:</strong> <a href="<?php echo admin_url( 'edit.php?post_type=newsletter&page=bulkmail_forms&ID=' . $form->ID ); ?>"><?php echo esc_html( $form->name ); ?></a>
					<?php
			endif;
			endif;
	...
	public function edit_user_profile( $user ) {
		...
		include BULKEMAIL_DIR . 'views/subscribers/user_edit.php';
	}


	public function screen_options() {

		require_once BULKEMAIL_DIR . 'classes/subscribers.table.class.php';
		...
		add_screen_option(
			'per_page',
			array(
				'label'   => esc_html__( 'Subscribers', 'bulkmail' ),
				'default' => 50,
				'option'  => 'bulkmail_subscribers_per_page',
			)
		);
	...
	public function save_screen_options( $status, $option, $value ) {
		...
		if ( 'bulkmail_subscribers_per_page' == $option ) {
			update_user_option( get_current_user_id(), 'bulkmail_subscribers_per_page', (int) $value );
			return $value;
		}
	...
	public function admin_notices() {
		...
		if ( isset( $_GET['post_type'] ) && 'subscriber' == $_GET['post_type'] && isset( $_GET['post_status'] ) && 'error' == $_GET['post_status'] ) {
			...
			) . '">' . esc_html__( 'convert all subscribers with error status back to subscribed', 'bulkmail' ) . '</a></strong></p></div>';
		}
	}
	...
	public function remove_unassigned_meta() {
		...
		$wpdb->query( "DELETE a FROM {$wpdb->prefix}bulkmail_subscriber_fields AS a {$wpdb->prefix}bulkmail_subscribers AS s ON a.subscriber_id = s.ID WHERE s.ID IS NULL" );
		$wpdb->query( "DELETE a FROM {$wpdb->prefix}bulkmail_subscriber_meta AS a {$wpdb->prefix}bulkmail_subscribers AS s ON a.subscriber_id = s.ID WHERE s.ID IS NULL" );
		$wpdb->query( "DELETE a FROM {$wpdb->prefix}bulkmail_queue AS a {$wpdb->prefix}bulkmail_subscribers AS s ON a.subscriber_id = s.ID WHERE s.ID IS NULL" );
		$wpdb->query( "DELETE a FROM {$wpdb->prefix}bulkmail_lists_subscribers AS a {$wpdb->prefix}bulkmail_subscribers AS s ON a.subscriber_id = s.ID WHERE s.ID IS NULL" );
	...
	public function verify( $entry ) {
		...
		return apply_filters( 'mymail_verify_subscriber', apply_filters( 'bulkmail_verify_subscriber', (array) $entry ) );
	}
	...
	public function update( $entry, $overwrite = true, $merge = false, $subscriber_notification = false ) {
		...
		if ( isset( $entry['email'] ) ) {
			if ( ! bulkmail_is_email( $entry['email'] ) ) {
				return new WP_Error( 'invalid_email', esc_html__( 'invalid email address', 'bulkmail' ) );
			}
		...
		if ( is_wp_error( $entry ) ) {
			...
			return new WP_Error( 'not_verified', esc_html__( 'Subscriber failed verification', 'bulkmail' ) );
		}
		...
		$sql = "INSERT INTO {$wpdb->prefix}bulkmail_subscribers (" . implode( ', ', $usedfields ) . ')';
		...
		if ( false !== $wpdb->query( $wpdb->prepare( $sql, $data ) ) ) {
			...
			$bulkimport = defined( 'BULKEMAIL_DO_BULKIMPORT' ) && BULKEMAIL_DO_BULKIMPORT;

			if ( ! $bulkimport ) {
				bulkmail_cache_delete( 'subscriber_' . $subscriber_id );
				bulkmail_cache_delete( 'get_custom_fields_' . $subscriber_id );
				bulkmail_cache_delete( 'subscriber_meta_' . $subscriber_id . '0' );
			}

			if ( isset( $meta['ip'] ) && $meta['ip'] && 'unknown' !== ( $geo = bulkmail_ip2City( $meta['ip'] ) ) ) {
			...
			if ( isset( $meta['form'] ) ) {
				$form = bulkmail( 'forms' )->get( $meta['form'], false );
			...
			do_action( 'bulkmail_update_subscriber', $subscriber_id );
			...
			if ( $mysql_errno == 1062 ) {
				return new WP_Error( 'email_exists', sprintf( esc_html__( 'The email "%s" already exists.', 'bulkmail' ), $entry['email'] ) );
			}
	...
	public function add( $entry, $overwrite = false, $merge = false, $subscriber_notification = true ) {
		...
		if ( ! isset( $entry['email'] ) ) {
			return new WP_Error( 'email_missing', esc_html__( 'You must define an email address.', 'bulkmail' ) );
		}
		$entry = wp_parse_args(
			$entry,
			array(
		...
		if ( bulkmail_option( 'track_users' ) && isset( $entry['ip'] ) && $entry['ip'] !== false ) {
		...
		if ( ! is_wp_error( $subscriber_id ) ) {
			do_action( 'bulkmail_add_subscriber', $subscriber_id );
		}
		return $subscriber_id;
	...
	public function add_from_wp_user( $user_id = null, $userdata = array(), $merge = false, $subscriber_notification = true ) {
		...
		if ( empty( $user ) ) {
			return new WP_Error( 'no_user', esc_html__( 'User doesn\'t exist!', 'bulkmail' ) );
		}
		...
		if ( ! $merge && $subscriber_exists ) {
			return new WP_Error( 'subscriber_exists', esc_html__( 'Subscriber already exists', 'bulkmail' ) );
		}
		...
		if ( ! isset( $userdata['status'] ) ) {
			$form = bulkmail( 'forms' )->get( 1 );

			$userdata['status'] = ( $form->doubleoptin && bulkmail_option( 'register_other_confirmation' ) ) ? 0 : 1;
		}
	...
	public function add_custom_value( $subscriber_id, $key, $value = null, $clear = false ) {
		...
		$sql = "INSERT INTO {$wpdb->prefix}bulkmail_subscriber_fields
        (subscriber_id, meta_key, meta_value) VALUES ";

		$inserts             = array();
		$british_date_format = bulkmail( 'helper' )->dateformat() == 'd/m/Y';
	...
	public function remove_custom_value( $subscriber_id, $key = null ) {
		...
		$sql = "DELETE FROM {$wpdb->prefix}bulkmail_subscriber_fields WHERE subscriber_id = %d";
		if ( ! is_null( $key ) ) {
	...
	public function assign_lists( $subscriber_ids, $lists, $remove_old = false, $added = null ) {
		...
		return bulkmail( 'lists' )->assign_subscribers( $lists, $subscriber_ids, $remove_old, $added );
	...
	public function unassign_lists( $subscriber_ids, $lists = null, $not_list = null ) {
		...
		$sql = "DELETE FROM {$wpdb->prefix}bulkmail_lists_subscribers WHERE subscriber_id IN (" . implode( ', ', $subscriber_ids ) . ')';
		...
		if ( false !== $wpdb->query( $sql ) ) {
			do_action( 'bulkmail_unassign_lists', $subscriber_ids, $lists, $not_list );
	...
	public function remove( $subscriber_ids, $status = null, $remove_actions = false, $remove_meta = true ) {
		...
		if ( $statuses ) {
			$sql            = "SELECT subscribers.ID FROM {$wpdb->prefix}bulkmail_subscribers AS subscribers WHERE subscribers.ID IN (0," . implode( ',', $subscriber_ids ) . ')';
		...
		if ( $delete_wp_user = ( bulkmail_option( 'delete_wp_user' ) && current_user_can( 'delete_users' ) ) ) {

			// get all wp users except the current user
			$current_user_id = get_current_user_id();
			$sql             = "SELECT wp_id FROM {$wpdb->prefix}bulkmail_subscribers AS subscribers WHERE subscribers.wp_id != 0 AND subscribers.wp_id != %d AND subscribers.ID IN (" . $subscriber_ids_concat . ')';
		...
		$sql = 'DELETE subscribers,lists_subscribers,subscriber_fields,' . ( $remove_actions ? 'actions,' : '' ) . "subscriber_meta,queue FROM {$wpdb->prefix}bulkmail_subscribers AS subscribers LEFT JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS lists_subscribers ON ( subscribers.ID = lists_subscribers.subscriber_id ) LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_fields AS subscriber_fields ON ( subscribers.ID = subscriber_fields.subscriber_id ) LEFT JOIN {$wpdb->prefix}bulkmail_actions AS actions ON ( subscribers.ID = actions.subscriber_id ) LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS subscriber_meta ON ( subscribers.ID = subscriber_meta.subscriber_id ) LEFT JOIN {$wpdb->prefix}bulkmail_queue AS queue ON ( subscribers.ID = queue.subscriber_id ) WHERE subscribers.ID IN (" . $subscriber_ids_concat . ' )';

		$sql = 'DELETE subscribers,lists_subscribers,subscriber_fields,' . ( $remove_actions ? 'actions,' : '' ) . ( $remove_meta ? 'subscriber_meta,' : '' ) . "queue FROM {$wpdb->prefix}bulkmail_subscribers AS subscribers LEFT JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS lists_subscribers ON ( subscribers.ID = lists_subscribers.subscriber_id ) LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_fields AS subscriber_fields ON ( subscribers.ID = subscriber_fields.subscriber_id ) LEFT JOIN {$wpdb->prefix}bulkmail_actions AS actions ON ( subscribers.ID = actions.subscriber_id ) LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS subscriber_meta ON ( subscribers.ID = subscriber_meta.subscriber_id ) LEFT JOIN {$wpdb->prefix}bulkmail_queue AS queue ON ( subscribers.ID = queue.subscriber_id ) WHERE subscribers.ID IN (" . $subscriber_ids_concat . ' )';
		...
		if ( $success = ( false !== $wpdb->query( $sql ) ) ) {
			...
			if ( ! $remove_actions ) {
				$sql = "UPDATE {$wpdb->prefix}bulkmail_actions AS actions SET subscriber_id = NULL WHERE actions.subscriber_id IN (" . $subscriber_ids_concat . ')';

				$wpdb->query( $sql );
			}
			// anonymize subscriber_meta data
			if ( ! $remove_meta ) {
				$sql = "UPDATE {$wpdb->prefix}bulkmail_subscriber_meta AS subscriber_meta SET subscriber_id = NULL WHERE subscriber_meta.subscriber_id IN (" . $subscriber_ids_concat . ')';
	...
	public function delete_subscriber_from_wpuser( $user_id ) {
		...
		if ( bulkmail_option( 'delete_wp_subscriber' ) ) {
	...
	public function get_meta_keys( $keys_only = false ) {
		$meta_keys = array(
			'bounce'        => esc_html__( 'Bounce', 'bulkmail' ),
			'geo'           => esc_html__( 'Location', 'bulkmail' ),
			'coords'        => esc_html__( 'Coordinates', 'bulkmail' ),
			'client'        => esc_html__( 'Client', 'bulkmail' ),
			'clienttype'    => esc_html__( 'Type of Client', 'bulkmail' ),
			'clientversion' => esc_html__( 'Clientversion', 'bulkmail' ),
			'lang'          => esc_html__( 'Language', 'bulkmail' ),
			'ip'            => esc_html__( 'IP address', 'bulkmail' ),
			'confirmation'  => esc_html__( 'Confirmation', 'bulkmail' ),
			'error'         => esc_html__( 'Error', 'bulkmail' ),
			'referer'       => esc_html__( 'Referer', 'bulkmail' ),
			'timeoffset'    => esc_html__( 'Timeoffset to UTC', 'bulkmail' ),
			'form'          => esc_html__( 'Form', 'bulkmail' ),
			'unsubscribe'   => esc_html__( 'Unsubscribe', 'bulkmail' ),
			'gdpr'          => esc_html__( 'GDPR Timestamp', 'bulkmail' ),
			'tags'          => esc_html__( 'Tags', 'bulkmail' ),
			'formkey'       => esc_html__( 'Form Key', 'bulkmail' ),
		);
	...
	public function meta( $id, $key = null, $campaign_id = 0 ) {
		...
		if ( false === ( $meta = bulkmail_cache_get( 'subscriber_meta_' . $id . $campaign_id ) ) ) {

			$default = array_fill_keys( $this->get_meta_keys( true ), null );

			$sql = "SELECT a.* FROM {$wpdb->prefix}bulkmail_subscriber_meta AS a WHERE a.subscriber_id = %d AND a.campaign_id = %d";
			...
			bulkmail_cache_add( 'subscriber_meta_' . $id . $campaign_id, $meta );
	...
	public function update_meta( $id, $campaign_id = 0, $key, $value = null ) {
		...
		$sql = "INSERT INTO {$wpdb->prefix}bulkmail_subscriber_meta (subscriber_id, campaign_id, meta_key, meta_value)";
		...
		foreach ( $insert as $key => $value ) {
			...
			if ( ! $value && isset( $oldmeta[ $key ] ) ) {
				// delete that row
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}bulkmail_subscriber_meta WHERE subscriber_id = %d AND meta_key = %s", $id, $key ) );
				continue;
			}
		...
		if ( empty( $inserts ) || false !== $wpdb->query( $sql ) ) {
			...
			bulkmail_cache_delete( 'subscriber_meta_' . $id . $campaign_id );
	...
	public function update_rating( $ids ) {
		...
		foreach ( $ids as $id ) {
			$actions = bulkmail( 'actions' )->get_by_subscriber( $id, null, false, true );
			$rating  = 0.25;
			if ( $this->get_sent( $id ) ) {
			...
			$rating = (float) apply_filters( 'bulkmail_subscriber_rating', $rating, $id );
			$rating = (float) apply_filters( 'bulkmail_subscriber_rating_' . $id, $rating );
			$rating = max( 0.1, min( $rating, 1 ) );

			$sql = "UPDATE {$wpdb->prefix}bulkmail_subscribers AS a SET a.rating = %f WHERE a.ID = %d AND a.rating != %f";
	...
	public function wp_id( $ids = null ) {
		...
		if ( is_array( $ids ) ) {
			...
			foreach ( $ids as $id ) {
				$sql = "UPDATE {$wpdb->prefix}bulkmail_subscribers AS a LEFT JOIN {$wpdb->users} AS b ON a.email = b.user_email SET a.wp_id = b.ID WHERE a.ID = %d";
		...
		$sql = "UPDATE {$wpdb->prefix}bulkmail_subscribers AS a INNER JOIN {$wpdb->users} AS b ON a.email = b.user_email SET a.wp_id = b.ID";
		if ( false !== $wpdb->query( $sql ) ) {
	...
	public function get_count_by_status( $status = null ) {
		...
		if ( false === ( $counts = bulkmail_cache_get( 'get_count_by_status' ) ) ) {

			$sql = "SELECT status, COUNT( a.ID ) AS count FROM {$wpdb->prefix}bulkmail_subscribers AS a GROUP BY status";

			$sql = apply_filters( 'bulkmail_subscribers_get_count_by_status', $sql );
			...
			bulkmail_cache_add( 'get_count_by_status', $counts );
	...
	public function get_sent( $id, $total = false ) {
		...
		return bulkmail( 'actions' )->get_by_subscriber( $id, 'sent' . ( $total ? '_total' : '' ), true );
	...
	public function get_sent_campaigns( $id, $ids_only = false ) {
		...
		$sql = "SELECT p.post_title AS campaign_title, a.* FROM {$wpdb->prefix}bulkmail_actions AS a LEFT JOIN {$wpdb->posts} AS p ON p.ID = a.campaign_id WHERE a.subscriber_id = %d AND a.type = 1";
	...
	public function get_opens( $id, $total = false ) {
		...
		return bulkmail( 'actions' )->get_by_subscriber( $id, 'opens' . ( $total ? '_total' : '' ), true );
	...
	public function get_opened_campaigns( $id, $ids_only = false ) {
		...
		$sql = "SELECT p.post_title AS campaign_title, a.* FROM {$wpdb->prefix}bulkmail_actions AS a LEFT JOIN {$wpdb->postmeta} AS c ON c.post_id = a.campaign_id AND c.meta_key = '_bulkmail_autoresponder' LEFT JOIN {$wpdb->posts} AS p ON p.ID = a.campaign_id WHERE a.subscriber_id = %d AND c.meta_value = '' AND a.type = 2";
	...
	public function get_clicks( $id, $total = false ) {
		...
		return bulkmail( 'actions' )->get_by_subscriber( $id, 'clicks' . ( $total ? '_total' : '' ), true );
	...
	public function get_clicked_campaigns( $id, $ids_only = false ) {
		...
		$sql = "SELECT p.post_title AS campaign_title, a.* FROM {$wpdb->prefix}bulkmail_actions AS a LEFT JOIN {$wpdb->postmeta} AS c ON c.post_id = a.campaign_id AND c.meta_key = '_bulkmail_autoresponder' LEFT JOIN {$wpdb->posts} AS p ON p.ID = a.campaign_id WHERE a.subscriber_id = %d AND c.meta_value = '' AND a.type = 3";
	...
	public function get_activity( $id, $limit = null, $exclude = null ) {
		...
		return bulkmail( 'actions' )->get_activity( null, $id, $limit, $exclude );
	...
	public function get_clients( $id ) {
		...
		$sql = "SELECT COUNT(a.meta_value) AS count, a.meta_value AS name, b.meta_value AS type FROM {$wpdb->prefix}bulkmail_subscriber_meta AS a LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS b ON a.subscriber_id = b.subscriber_id AND a.campaign_id = b.campaign_id WHERE a.meta_key = 'client' AND b.meta_key = 'clienttype' AND a.subscriber_id = %d AND a.campaign_id != 0 GROUP BY a.meta_value ORDER BY count DESC";
	...
	public function get_lists( $id, $ids_only = false ) {
		...
		$cache = bulkmail_cache_get( 'subscribers_lists' );
		if ( isset( $cache[ $id ] ) ) {
			return $cache[ $id ];
		}

		$sql = "SELECT a.*, ab.added AS confirmed FROM {$wpdb->prefix}bulkmail_lists AS a LEFT JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS ab ON a.ID = ab.list_id WHERE ab.subscriber_id = %d";
	...
	private function unsubscribe_by_type( $type, $value, $campaign_id = null, $status = null ) {
		...
		if ( bulkmail( 'campaigns' )->list_based_opt_out( $campaign_id ) ) {

			$lists = bulkmail( 'campaigns' )->get_lists( $campaign_id, true );
			if ( $this->unassign_lists( $subscriber->ID, $lists ) ) {
				...
				do_action( 'bulkmail_list_unsubscribe', $subscriber->ID, $campaign_id, $lists, $status );
		...
		if ( $this->change_status( $subscriber->ID, 2 ) ) {
			...
			do_action( 'bulkmail_unsubscribe', $subscriber->ID, $campaign_id, $status );
	...
	public function subscriber_notification( $id, $timestamp = null ) {
		...
		if ( defined( 'BULKEMAIL_DO_BULKIMPORT' ) && BULKEMAIL_DO_BULKIMPORT ) {
			return false;
		}

		if ( ! bulkmail_option( 'subscriber_notification' ) || ! bulkmail_option( 'subscriber_notification_receviers' ) ) {
			return false;
		}

		if ( $delay = bulkmail_option( 'subscriber_notification_delay' ) ) {

			$timestamp = bulkmail( 'helper' )->get_timestamp_by_string( $delay );
			...
			return ( ! wp_next_scheduled( 'bulkmail_subscriber_notification' ) )
			...
			return bulkmail( 'notification' )->add(
				$timestamp,
				array(
	...
	public function subscriber_delayed_notification() {
		...
		if ( ! bulkmail_option( 'subscriber_notification' ) || ! bulkmail_option( 'subscriber_notification_receviers' ) || ! bulkmail_option( 'subscriber_notification_delay' ) ) {
			return false;
		}
	...
	public function subscriber_unsubscribe_notification( $id, $timestamp = null, $lists = null ) {
		...
		if ( ! bulkmail_option( 'unsubscribe_notification' ) || ! bulkmail_option( 'unsubscribe_notification_receviers' ) ) {
			return false;
		}

		if ( $delay = bulkmail_option( 'unsubscribe_notification_delay' ) ) {
			...
			return ( ! wp_next_scheduled( 'bulkmail_unsubscribe_notification' ) )
	...
	public function subscriber_delayed_unsubscribe_notification() {
		...
		if ( ! bulkmail_option( 'unsubscribe_notification' ) || ! bulkmail_option( 'unsubscribe_notification_receviers' ) || ! bulkmail_option( 'unsubscribe_notification_delay' ) ) {
			return false;
		}
	...
	public function send_confirmations( $ids = null, $force = false, $now = false, $user_form_id = null ) {
		...
		$fallback_form_id = (int) $wpdb->get_var( "SELECT ID FROM {$wpdb->prefix}bulkmail_forms ORDER BY ID ASC LIMIT 1" );

		// get all pending subscribers which are not queued already
		$sql  = "SELECT subscribers.ID, subscribers.signup, IFNULL( confirmation.meta_value, 0 ) AS try, forms.resend, forms.resend_count, forms.resend_time, IFNULL( forms.ID, $fallback_form_id ) AS form_id, lists_subscribers.list_id FROM {$wpdb->prefix}bulkmail_subscribers AS subscribers";
		$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS lists_subscribers ON subscribers.ID = lists_subscribers.subscriber_id AND lists_subscribers.added = 0";
		$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS confirmation ON subscribers.ID = confirmation.subscriber_id AND confirmation.meta_key = 'confirmation'";
		$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS form ON subscribers.ID = form.subscriber_id AND form.meta_key = 'form'";
		$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_queue AS queue ON subscribers.ID = queue.subscriber_id AND queue.campaign_id = 0";
		$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_forms AS forms ON forms.ID = IFNULL( form.meta_value, $fallback_form_id )";
	...
	public function remove_pending_confirmations( $subscriber_id = null ) {
		...
		$sql = "DELETE a,b FROM {$wpdb->prefix}bulkmail_queue AS a LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS b ON a.subscriber_id = b.subscriber_id AND b.meta_key = 'confirmation' WHERE a.campaign_id = 0 AND a.options LIKE '%s:8:\"template\";s:12:\"confirmation\";%'";
	...
	public function query( $args = array(), $campaign_id = null ) {
		...
		require_once BULKEMAIL_DIR . 'classes/subscriber.query.class.php';
		$query = BulkmailSubscriberQuery::get_instance();
	...
	public function get_current_user( $custom_fields = true ) {
		...
		if ( isset( $_COOKIE ) && isset( $_COOKIE['bulkmail'] ) ) {
			$hash       = $_COOKIE['bulkmail'];
			$subscriber = bulkmail( 'subscribers' )->get_by_hash( $hash, $custom_fields );
		}
	...
	private function get_by_type( $type, $value, $custom_fields = false ) {
		...
		if ( 'ID' == $type ) {
			$value = (int) $value;
			if ( $subscriber = bulkmail_cache_get( 'subscriber_' . $value ) ) {
				return $subscriber;
			}
			$type = esc_sql( $type );
		} elseif ( 'md5' == $type ) {
		...
		$sql = "SELECT * FROM {$wpdb->prefix}bulkmail_subscribers WHERE " . $type . " = '" . esc_sql( $value ) . "' LIMIT 1";

		$sql = apply_filters( 'bulkmail_subscribers_get_by_type_sql', $sql, $type, $value );
		...
		if ( $custom_fields ) {
			bulkmail_cache_set( 'subscriber_' . $subscriber->ID, $subscriber );
		}
	...
	public function get_custom_fields( $subscriber_id, $field = null ) {
		...
		if ( false === ( $custom_fields = bulkmail_cache_get( 'get_custom_fields_' . $subscriber_id ) ) ) {

			$custom_field_names = bulkmail()->get_custom_fields( true );
			...
			$sql = $wpdb->prepare( "SELECT meta_key, meta_value FROM {$wpdb->prefix}bulkmail_subscriber_fields WHERE subscriber_id = %d", $subscriber_id );
	...
	public function get_empty( $custom_fields = false ) {
		...
		$fields = wp_parse_args(
			...
			$wpdb->get_col( "DESCRIBE {$wpdb->prefix}bulkmail_subscribers" )
		);

		if ( ! $custom_fields ) {
			$fields = wp_parse_args( bulkmail()->get_custom_fields( true ), $fields );
		}
	...
	public function get_confirm_link( $id, $form_id = null, $list_ids = null ) {
		...
		$link = ( bulkmail( 'helper' )->using_permalinks() )
			? trailingslashit( $baselink ) . trailingslashit( 'bulkmail/' . $slug . '/' . $subscriber->hash . '/' . $form_id . $lists )
			: add_query_arg(
				array(
					'bulkmail_confirm' => '',
					'k'                => $subscriber->hash,
	...
	public function get_recipient_detail( $id, $campaign_id ) {
		...
		$actions = (object) bulkmail( 'actions' )->get_campaign_actions( $campaign_id, $id, null, false );
		...
		if ( $avatar ) :
			$html .= '<div class="user_image" title="' . esc_html__( 'Source', 'bulkmail' ) . ': Gravatar.com" data-email="' . $subscriber->email . '" style="background-image:url(' . $this->get_gravatar_uri( $subscriber->email, 240 ) . ')"></div>';
		endif;

		$html .= '<div class="receiver-detail-data ' . ( $avatar ? 'has-avatar' : '' ) . '">';
		$html .= '<h4>' . esc_html( $subscriber->fullname ? $subscriber->fullname : $subscriber->email ) . ' <a href="edit.php?post_type=newsletter&page=bulkmail_subscribers&ID=' . $subscriber->ID . '">' . esc_html__( 'edit', 'bulkmail' ) . '</a></h4>';
		$html .= '<ul>';

		$html .= '<li><label>' . esc_html__( 'sent', 'bulkmail' ) . ':</label> ' . ( $actions->sent ? date( $timeformat, $actions->sent + $timeoffset ) . ', ' . sprintf( esc_html__( '%s ago', 'bulkmail' ), human_time_diff( $actions->sent ) ) . ( $actions->sent_total > 1 ? ' <span class="count">(' . sprintf( esc_html__( '%d times', 'bulkmail' ), $actions->sent_total ) . ')</span>' : '' ) : esc_html__( 'not yet', 'bulkmail' ) );
		$html .= '<li><label>' . esc_html__( 'opens', 'bulkmail' ) . ':</label> ' . ( $actions->opens ? date( $timeformat, $actions->opens + $timeoffset ) . ', ' . sprintf( esc_html__( '%s ago', 'bulkmail' ), human_time_diff( $actions->opens ) ) . ( $actions->opens_total > 1 ? ' <span class="count">(' . sprintf( esc_html__( '%d times', 'bulkmail' ), $actions->opens_total ) . ')</span>' : '' ) : esc_html__( 'not yet', 'bulkmail' ) );

		$html .= '<p class="meta"><label>&nbsp;</label>';
		if ( $meta['client'] ) {
			$html .= sprintf( esc_html__( 'with %s', 'bulkmail' ), '<strong>' . $meta['client'] . '</strong>' );
		}

		if ( $meta['geo'] ) {
			$geo = explode( '|', $meta['geo'] );
			if ( $geo[1] ) {
				$html .= ' ' . sprintf( esc_html_x( 'in %1$s, %2$s', 'in [city], [country]', 'bulkmail' ), '<strong>' . $geo[1] . '</strong>', '<span class="bulkmail-flag flag-' . strtolower( $geo[0] ) . '"></span> <strong>' . $geo[0] . '</strong>' );
			}
		}
		$html .= '</p>';
		...
		if ( $actions->clicks ) {
			...
			foreach ( $actions->clicks as $link => $count ) {
				$html .= '<li class=""><a href="' . $link . '" class="external clicked-link">' . $link . '</a> <span class="count">(' . sprintf( esc_html__( _n( '%s click', '%s clicks', (int) $count, 'bulkmail' ) ), $count ) . ')</span></li>';
			}
			$html .= '</ul></li>';
		}

		if ( $actions->unsubscribes ) {
			$message = bulkmail( 'helper' )->get_unsubscribe_message( $this->meta( $id, 'unsubscribe', $campaign_id ) );
			$html   .= '<li>' . sprintf( esc_html__( 'Unsubscribes on %s', 'bulkmail' ), date( $timeformat, $actions->unsubscribes + $timeoffset ) . ', ' . sprintf( esc_html__( '%s ago', 'bulkmail' ), human_time_diff( $actions->unsubscribes ) ) ) . '<br>' . esc_html( $message ) . '</li>';
		}

		if ( $actions->bounces ) {
			$message = bulkmail( 'helper' )->get_bounce_message( $this->meta( $id, 'bounce', $campaign_id ) );
			$html   .= '<li>';
			if ( $actions->softbounces_total ) :
				$html .= '<label class="red">' . sprintf( esc_html__( _n( '%s soft bounce', '%s soft bounces', $actions->softbounces_total, 'bulkmail' ) ), $actions->softbounces_total ) . '</label>';
			endif;
			$html .= '<strong class="red">' . sprintf( esc_html__( 'Hard bounced at %s', 'bulkmail' ), date( $timeformat, $actions->bounces + $timeoffset ) . ', ' . sprintf( esc_html__( '%s ago', 'bulkmail' ), human_time_diff( $actions->bounces ) ) ) . '</strong><br>' . esc_html( $message );
			$html .= '</li>';
		} elseif ( $actions->softbounces ) {
			$message = bulkmail( 'helper' )->get_bounce_message( $this->meta( $id, 'bounce', $campaign_id ) );
			$html   .= '<li><label class="red">' . sprintf( esc_html__( _n( '%s soft bounce', '%s soft bounces', $actions->softbounces_total, 'bulkmail' ) ), $actions->softbounces_total ) . '</label><br>' . esc_html( $message ) . '</li>';
		}
	...
	public function user_register( $user_id ) {
		...
		if ( ! apply_filters( 'mymail_user_register', apply_filters( 'bulkmail_user_register', true ) ) ) {
			return;
		}
		...
		if ( $is_register ) {
			...
			if ( ! bulkmail_option( 'register_signup' ) ) {
				return false;
			}

			// stop if not option
			if ( ! isset( $_POST['bulkmail_user_newsletter_signup'] ) ) {
				return false;
			}

			$status = bulkmail_option( 'register_signup_confirmation' ) ? 0 : 1;

			$referer = 'wp-login.php';

		} else {

			if ( ! bulkmail_option( 'register_other' ) ) {
				return false;
			}

			$status = bulkmail_option( 'register_other_confirmation' ) ? 0 : 1;

			$roles = bulkmail_option( 'register_other_roles', array() );
		...
		$lists = $is_register ? bulkmail_option( 'register_signup_lists', array() ) : bulkmail_option( 'register_other_lists', array() );

		$subscriber_id = $this->add_from_wp_user(
			$user_id,
			array(
				'status'  => $status,
				'referer' => apply_filters( 'mymail_user_register_referer', apply_filters( 'bulkmail_user_register_referer', $referer ) ),
				'_lists'  => $lists,
			),
			true
		);
	...
	public function register_form() {
		...
		$output = '<p><label for="bulkmail_user_newsletter_signup"><input name="bulkmail_user_newsletter_signup" type="checkbox" id="bulkmail_user_newsletter_signup" value="1" ' . checked( bulkmail_option( 'register_signup_checked' ), true, false ) . ' />' . bulkmail_text( 'newsletter_signup' ) . '</label></p>';

		echo apply_filters( 'bulkmail_register_form_signup_field', $output ) . "\n";
	...
	public function comment_form_checkbox() {
		...
		if ( ! bulkmail_option( 'register_comment_form' ) ) {
			return;
		}
		...
		$field .= '<label for="bulkmail_newsletter_signup"><input name="newsletter_signup" type="checkbox" id="bulkmail_newsletter_signup" value="1" ' . checked( bulkmail_option( 'register_comment_form_checked' ), true, false ) . '/> ' . bulkmail_text( 'newsletter_signup' ) . '</label>';
		$field .= '</p>';
	...
	public function comment_post( $comment_id, $comment_approved ) {
		...
		if ( isset( $_POST['newsletter_signup'] ) ) {
			...
			if ( in_array( $comment_approved . '', bulkmail_option( 'register_comment_form_status', array() ) ) ) {

				$comment = get_comment( $comment_id );

				if ( $comment && ! $this->get_by_mail( $comment->comment_author_email ) ) {

					$lists = bulkmail_option( 'register_comment_form_lists', array() );
					$lists = apply_filters( 'mymail_comment_post_lists', apply_filters( 'bulkmail_comment_post_lists', $lists, $comment, $comment_approved ), $comment, $comment_approved );

					$status = bulkmail_option( 'register_comment_form_confirmation' ) ? 0 : 1;
					...
					$userdata = apply_filters( 'mymail_comment_post_userdata', apply_filters( 'bulkmail_comment_post_userdata', $userdata, $comment, $comment_approved ), $comment, $comment_approved );
	...
	public function wp_set_comment_status( $comment_id, $comment_status ) {
		...
		if ( ! bulkmail_option( 'register_comment_form' ) || ! in_array( $comment_status . '', array( '1', 'approve' ) ) ) {
			return false;
		}
	...
	public function bounce( $subscriber_id, $campaign_id, $hard = false, $status = null ) {
		...
		if ( $hard ) {
			...
			if ( $this->change_status( $subscriber->ID, $this->get_status_by_name( 'hardbounced' ) ) ) {
				do_action( 'bulkmail_bounce', $subscriber->ID, $campaign_id, true, $status );
				if ( $status ) {
		...
		if ( $bounce_attempts == 1 || $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}bulkmail_actions WHERE type = 5 AND subscriber_id = %d AND campaign_id = %d AND count >= %d LIMIT 1", $subscriber->ID, $campaign_id, $bounce_attempts ) ) ) {
		...
		// softbounce
		do_action( 'bulkmail_bounce', $subscriber->ID, $campaign_id, false, $status );
		if ( $status ) {
			$this->update_meta( $subscriber->ID, $campaign_id, 'bounce', $status );
		}
	...
	public function get_sent_mails( $id ) {
		...
		if ( false === ( $counts = bulkmail_cache_get( 'get_sent_mails' ) ) ) {

			$sql = "SELECT status, COUNT( * ) AS count FROM {$wpdb->prefix}bulkmail_subscribers GROUP BY status";
			...
			bulkmail_cache_add( 'get_sent_mails', $counts );
		}
	...
	public function get_gravatar_uri( $email, $size = 120 ) {
		...
		$default = 'https://static.bulkmail.co/user/user' . $id . '.gif';
	...
	public function get_status( $id = null, $nice = false ) {
		...
		if ( $nice ) {
			$statuses = array(
				esc_html__( 'Pending', 'bulkmail' ),
				esc_html__( 'Subscribed', 'bulkmail' ),
				esc_html__( 'Unsubscribed', 'bulkmail' ),
				esc_html__( 'Hardbounced', 'bulkmail' ),
				esc_html__( 'Error', 'bulkmail' ),
			);
	...
	public function get_timeoffset_timestamps( $subscriber_ids, $timestamps = null ) {
		...
		if ( is_array( $timestamps ) ) {
			$sql = "SELECT a.ID, IF(b.meta_value IS NULL,0,(-b.meta_value*3600+$timeoffset)) AS offset FROM {$wpdb->prefix}bulkmail_subscribers AS a LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS b ON a.ID = b.subscriber_id AND b.meta_key = 'timeoffset' WHERE a.ID IN (" . implode( ',', $subscriber_ids ) . ') ORDER BY a.ID ASC';
			...
			foreach ( $chunks as $subscriber_id_chunk ) {
				...
				$sql = "SELECT IF(b.meta_value IS NULL,0,(-b.meta_value*3600+$timeoffset))+$timestamps AS timestamps FROM {$wpdb->prefix}bulkmail_subscribers AS a LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS b ON a.ID = b.subscriber_id AND b.meta_key = 'timeoffset' WHERE a.ID IN (" . implode( ',', $subscriber_id_chunk ) . ') ORDER BY a.ID ASC';
	...
	public function change_status( $subscriber_ids, $new_status, $silent = false ) {
		...
		foreach ( $subscriber_ids as $subscriber_id ) {
			...
			if ( false !== $wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}bulkmail_subscribers SET status = %d, updated = %d WHERE ID = %d", $new_status, time(), $subscriber->ID ) ) ) {
				if ( ! $silent ) {
					do_action( 'bulkmail_subscriber_change_status', $new_status, $old_status, $subscriber );
				}

				if ( bulkmail_option( 'list_based_opt_in' ) ) {
					if ( 1 == $new_status ) {
						bulkmail( 'lists' )->confirm_subscribers( null, $subscriber_id );
					} elseif ( 0 == $new_status ) {
						bulkmail( 'lists' )->unconfirm_subscribers( null, $subscriber_id );
					}
				}
	...
	private function hash( $email ) {
		...
		$hash = md5( $email . bulkmail_option( 'ID', '' ) );
		return apply_filters( 'bulkmail_subscriber_hash', $hash, $org_email );
