...
class BulkmailBounceHandler {
	...
	public function __construct( $service = 'pop3' ) {
		...
		$this->bounce_delete = bulkmail_option( 'bounce_delete' );
		$this->MID = bulkmail_option( 'ID' );
	...
	public function connect( $server, $user, $pwd, $port = 110, $secure = false, $timeout = 60 ) {
		...
		require_once BULKEMAIL_DIR . 'classes/libs/PhpImap/__autoload.php';
	...
	public function process_bounces() {
		...
		require_once BULKEMAIL_DIR . 'classes/libs/bounce/bounce_driver.class.php';

		foreach ( $messages as $id => $message ) {

			preg_match( '#X-(Bulkmail|MyMail): ([a-f0-9]{32})#i', $message, $hash );
			preg_match( '#X-(Bulkmail|MyMail)-Campaign: (\d+)#i', $message, $camp );
			...
			$subscriber = bulkmail( 'subscribers' )->get_by_hash( $hash[2], false );
			$campaign = ! empty( $camp ) ? bulkmail( 'campaigns' )->get( (int) $camp[2] ) : null;
			...
			if ( $subscriber ) {
				...
				switch ( $action ) {
					...
					case 'unsubscribe':
						// unsubscribe
						bulkmail( 'subscribers' )->unsubscribe( $subscriber->ID, $campaign_id, 'list_unsubscribe' );
					break;
					case 'failed':
						// hardbounce
						bulkmail( 'subscribers' )->bounce( $subscriber->ID, $campaign_id, true, $status );
					break;

					case 'transient':
					default:
						// softbounce
						bulkmail( 'subscribers' )->bounce( $subscriber->ID, $campaign_id, false, $status );
	...
	protected function get_messages( $all = false ) {
		...
		foreach ( $mailsIds as $i => $id ) {
			...
			if ( $all || preg_match( '#X-(Bulkmail|MyMail)-ID: ' . preg_quote( $this->MID ) . '#i', $message ) ) {
...
class BulkmailBounceLegacyHandler extends BulkmailBounceHandler {
