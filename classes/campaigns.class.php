...
class BulkmailCampaigns {

	private $defaultTemplate = 'bulkmail';
	private $template;
	...
	public function __construct() {
		...
		if ( $hooks = get_option( 'bulkmail_hooks', false ) ) {
	...
	public function init() {
		...
		add_action( 'bulkmail_finish_campaign', array( &$this, 'remove_revisions' ) );

		add_action( 'bulkmail_auto_post_thumbnail', array( &$this, 'get_post_thumbnail' ), 10, 1 );
...
//		add_action( 'admin_menu', array( &$this, 'autoresponder_menu' ), 20 );
		...
		add_filter( 'bulkmail_campaign_action', array( &$this, 'trigger_campaign_action' ), 10, 2 );
	...
	public function autoresponder_hook( $campaign_id, $subscriber_ids = null, $args = array() ) {
		...
		if ( ! $meta['active'] || $meta['autoresponder']['action'] != 'bulkmail_autoresponder_hook' ) {
			return;
		}
		...
		$query_args = apply_filters( 'bulkmail_autoresponder_hook_args', $query_args, $campaign_id, $subscriber_ids, $args );
		$query_args = apply_filters( 'bulkmail_autoresponder_hook_args_' . current_filter(), $query_args, $campaign_id, $subscriber_ids, $args );

		$query_args['return_ids'] = true;

		$subscribers = bulkmail( 'subscribers' )->query( $query_args, $campaign_id );
		...
		bulkmail( 'queue' )->bulk_add( $campaign_id, $subscribers, $timestamp, $priority, $clear, $ignore_status, $reset, $options, $tags );

		// handle instant delivery
		if ( $timestamp - time() <= 0 ) {
			wp_schedule_single_event( $timestamp, 'bulkmail_cron_worker', array( $campaign_id ) );
		}
	}
	...
	public function register_post_type() {
		...
		$single           = $is_autoresponder ? esc_html__( 'Autoresponder', 'bulkmail' ) : esc_html__( 'Campaign', 'bulkmail' );
		$plural           = $is_autoresponder ? esc_html__( 'Autoresponders', 'bulkmail' ) : esc_html__( 'Campaigns', 'bulkmail' );

		$color = '#a0a5aa';
		if ( is_admin() && ( isset( $_GET['post_type'] ) && 'newsletter' == $_GET['post_type'] || isset( $_GET['page'] ) && 'bulkmail_dashboard' == $_GET['page'] ) ) {
			$color = '#ffffff';
		...
		register_post_type(
			...
			array(
				...
				'labels'               => array(
					...
					'add_new'               => sprintf( esc_html__( 'New %s', 'bulkmail' ), $single ),
					'add_new_item'          => esc_html__( 'Create A New Campaign', 'bulkmail' ),
					'edit_item'             => sprintf( esc_html__( 'Edit %s', 'bulkmail' ), $single ),
					'new_item'              => sprintf( esc_html__( 'New %s', 'bulkmail' ), $single ),
					'all_items'             => esc_html__( 'All Campaigns', 'bulkmail' ),
					'view_item'             => esc_html__( 'View Newsletter', 'bulkmail' ),
					'search_items'          => sprintf( esc_html__( 'Search %s', 'bulkmail' ), $plural ),
					'not_found'             => sprintf( esc_html__( 'No %s found', 'bulkmail' ), $single ),
					'not_found_in_trash'    => sprintf( esc_html__( 'No %s found in Trash', 'bulkmail' ), $single ),
					'parent_item_colon'     => '',
					'menu_name'             => esc_html__( 'Newsletter & Bulk Email Sender', 'bulkmail' ),
					'filter_items_list'     => esc_html__( 'Filter Newsletter list', 'bulkmail' ),
					'items_list_navigation' => esc_html__( 'Newsletter list navigation', 'bulkmail' ),
					'items_list'            => esc_html__( 'Newsletter list', 'bulkmail' ),
				),
				...
				'map_meta_cap'         => true,
				'has_archive'          => bulkmail_option( 'hasarchive', false ) ? bulkmail_option( 'archive_slug', false ) : false,
				'hierarchical'         => $is_autoresponder,
				'rewrite'              => array(
					'with_front' => false,
					'slug'       => bulkmail_option( 'slug', 'newsletter' ),
				),
				'supports'             => array(
					'title',
					'thumbnail',
	...
	public function register_post_status() {
		...
		register_post_status(
			'paused',
			array(
				'label'       => esc_html__( 'Paused', 'bulkmail' ),
				'public'      => true,
				'label_count' => _n_noop( esc_html__( 'Paused', 'bulkmail' ) . ' <span class="count">(%s)</span>', esc_html__( 'Paused', 'bulkmail' ) . ' <span class="count">(%s)</span>' ),
			)
		);
		...
		register_post_status(
			'active',
			array(
				'label'       => esc_html__( 'Active', 'bulkmail' ),
				'public'      => true,
				'label_count' => _n_noop( esc_html__( 'Active', 'bulkmail' ) . ' <span class="count">(%s)</span>', esc_html__( 'Active', 'bulkmail' ) . ' <span class="count">(%s)</span>' ),
			)
		);
		...
		register_post_status(
			'queued',
			array(
				'label'       => esc_html__( 'Queued', 'bulkmail' ),
				'public'      => true,
				'label_count' => _n_noop( esc_html__( 'Queued', 'bulkmail' ) . ' <span class="count">(%s)</span>', esc_html__( 'Queued', 'bulkmail' ) . ' <span class="count">(%s)</span>' ),
			)
		);
		...
		register_post_status(
			'finished',
			array(
				'label'       => esc_html__( 'Finished', 'bulkmail' ),
				'public'      => true,
				'label_count' => _n_noop( esc_html__( 'Finished', 'bulkmail' ) . ' <span class="count">(%s)</span>', esc_html__( 'Finished', 'bulkmail' ) . ' <span class="count">(%s)</span>' ),
			)
		);
		...
		register_post_status(
			'autoresponder',
			array(
				'label'                  => esc_html__( 'Autoresponder', 'bulkmail' ),
				'public'                 => ! is_admin(),
				'exclude_from_search'    => true,
				'show_in_admin_all_list' => false,
				'label_count'            => _n_noop( esc_html__( 'Autoresponder', 'bulkmail' ) . ' <span class="count">(%s)</span>', esc_html__( 'Autoresponders', 'bulkmail' ) . ' <span class="count">(%s)</span>' ),
			)
		);
	...
	public function meta_boxes() {
		...
		global $post;
		add_meta_box( 'bulkmail_details', esc_html__( 'Details', 'bulkmail' ), array( &$this, 'newsletter_details' ), 'newsletter', 'normal', 'high' );
		add_meta_box( 'bulkmail_template', ( ! in_array( $post->post_status, array( 'active', 'finished' ) ) && ! isset( $_GET['showstats'] ) ) ? esc_html__( 'Template', 'bulkmail' ) : esc_html__( 'Clickmap', 'bulkmail' ), array( &$this, 'newsletter_template' ), 'newsletter', 'normal', 'high' );
		add_meta_box( 'bulkmail_submitdiv', esc_html__( 'Save', 'bulkmail' ), array( &$this, 'newsletter_submit' ), 'newsletter', 'side', 'high' );
		add_meta_box( 'bulkmail_delivery', esc_html__( 'Delivery', 'bulkmail' ), array( &$this, 'newsletter_delivery' ), 'newsletter', 'side', 'high' );
		add_meta_box( 'bulkmail_receivers', esc_html__( 'Receivers', 'bulkmail' ), array( &$this, 'newsletter_receivers' ), 'newsletter', 'side', 'high' );
		add_meta_box( 'bulkmail_options', esc_html__( 'Options', 'bulkmail' ), array( &$this, 'newsletter_options' ), 'newsletter', 'side', 'high' );
		add_meta_box( 'bulkmail_attachments', esc_html__( 'Attachment', 'bulkmail' ), array( &$this, 'newsletter_attachment' ), 'newsletter', 'side', 'low' );
	...
	public function display_post_states( $post_states, $post ) {
		...
		if ( $post->post_type == 'newsletter' ) {
			...
			if ( ! $this->meta( $post->ID, 'webversion' ) ) {
				$post_states['bulkmail_no_webversion'] = esc_html__( 'Private', 'bulkmail' );
			}
		}
	...
	public function autoresponder_menu() {
		...
		if ( current_user_can( 'edit_newsletters' ) ) {
			$submenu['edit.php?post_type=newsletter'][] = array(
				esc_html__( 'Autoresponder', 'bulkmail' ),
				'bulkmail_edit_autoresponders',
	...
	public function newsletter_details() {
		global $post;
		global $post_id;
		include BULKEMAIL_DIR . 'views/newsletter/details.php';
	}
	...
	public function newsletter_template() {
		global $post;
		global $post_id;
		include BULKEMAIL_DIR . 'views/newsletter/template.php';
	}
	...
	public function newsletter_delivery() {
		global $post;
		global $post_id;
		include BULKEMAIL_DIR . 'views/newsletter/delivery.php';
	}
	...
	public function newsletter_receivers() {
		global $post;
		global $post_id;
		include BULKEMAIL_DIR . 'views/newsletter/receivers.php';
	}
	...
	public function newsletter_options() {
		global $post;
		global $post_id;
		include BULKEMAIL_DIR . 'views/newsletter/options.php';
	}
	...
	public function newsletter_attachment() {
		global $post;
		global $post_id;
		include BULKEMAIL_DIR . 'views/newsletter/attachment.php';
	}
	...
	public function newsletter_submit( $post ) {
		...
		include BULKEMAIL_DIR . 'views/newsletter/submit.php';
	}
	...
	public function edit_hook() {
		...
		if ( isset( $_GET['post_type'] ) && 'newsletter' == $_GET['post_type'] && ! isset( $_GET['page'] ) ) {
			...
			// duplicate campaign
			if ( isset( $_GET['duplicate'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'bulkmail_duplicate_nonce' ) ) {
				...
				if ( ( current_user_can( 'duplicate_newsletters' ) && get_current_user_id() != $post->post_author ) && ! current_user_can( 'duplicate_others_newsletters' ) ) {
					wp_die( esc_html__( 'You are not allowed to duplicate this campaign.', 'bulkmail' ) );
				} else {
				...
				// pause campaign
			} elseif ( isset( $_GET['pause'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'bulkmail_pause_nonce' ) ) {
				$id = (int) $_GET['pause'];
				if ( ! current_user_can( 'publish_newsletters', $id ) ) {
					wp_die( esc_html__( 'You are not allowed to pause this campaign.', 'bulkmail' ) );
				} else {
					$this->pause( $id );
				}

				// continue/start campaign
			} elseif ( isset( $_GET['start'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'bulkmail_start_nonce' ) ) {
				$id = (int) $_GET['start'];
				if ( ! current_user_can( 'publish_newsletters', $id ) ) {
					wp_die( esc_html__( 'You are not allowed to start this campaign.', 'bulkmail' ) );
				} else {
					$this->start( $id );
				}

				// resume campaign
			} elseif ( isset( $_GET['resume'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'bulkmail_start_nonce' ) ) {
				$id = (int) $_GET['resume'];
				if ( ! current_user_can( 'publish_newsletters', $id ) ) {
					wp_die( esc_html__( 'You are not allowed to resume this campaign.', 'bulkmail' ) );
				} else {
					$this->resume( $id );
				}

				// finish campaign
			} elseif ( isset( $_GET['finish'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'bulkmail_finish_nonce' ) ) {
				$id = (int) $_GET['finish'];
				if ( ! current_user_can( 'publish_newsletters', $id ) ) {
					wp_die( esc_html__( 'You are not allowed to finish this campaign.', 'bulkmail' ) );
				} else {
					$this->finish( $id );
				}

				// activate autoresponder
			} elseif ( isset( $_GET['activate'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'bulkmail_activate_nonce' ) ) {
				$id = (int) $_GET['activate'];
				if ( ! current_user_can( 'publish_newsletters', $id ) ) {
					wp_die( esc_html__( 'You are not allowed to activate this campaign.', 'bulkmail' ) );
				} else {
					$this->activate( $id );
				}

				// deactivate autoresponder
			} elseif ( isset( $_GET['deactivate'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'bulkmail_deactivate_nonce' ) ) {
				$id = (int) $_GET['deactivate'];
				if ( ! current_user_can( 'publish_newsletters', $id ) ) {
					wp_die( esc_html__( 'You are not allowed to deactivate this campaign.', 'bulkmail' ) );
				} else {
					$this->deactivate( $id );
				}
			}
	...
	public function notice() {
		...
		switch ( $post->post_status ) {
			case 'finished':
				...
				$msg        = sprintf( esc_html__( 'This Campaign was sent on %s', 'bulkmail' ), '<span class="nowrap">' . date( $timeformat, $this->meta( $post->ID, 'finished' ) + $timeoffset ) . '</span>' );
				break;
			case 'queued':
				$msg = esc_html__( 'This Campaign is currently in the queue', 'bulkmail' );
				break;
			case 'active':
				$msg = esc_html__( 'This Campaign is currently progressing', 'bulkmail' );
				break;
			case 'paused':
				$msg = esc_html__( 'This Campaign has been paused', 'bulkmail' );
				break;
		}
	...
	public function updated_messages( $messages ) {
		...
		$messages['newsletter'] = array(
			0  => '',
			1  => sprintf( esc_html__( 'Campaign updated. %s', 'bulkmail' ), '<a href="' . esc_url( get_permalink( $post_id ) ) . '">' . esc_html__( 'View Newsletter', 'bulkmail' ) . '</a>' ),
			2  => sprintf( esc_html__( 'Template changed. %1$s', 'bulkmail' ), '<a href="' . remove_query_arg( 'message', bulkmail_get_referer() ) . '">' . esc_html__( 'Go back', 'bulkmail' ) . '</a>' ),
			3  => esc_html__( 'Template saved', 'bulkmail' ),
			4  => esc_html__( 'Campaign updated.', 'bulkmail' ),
			5  => isset( $_GET['revision'] ) ? sprintf( esc_html__( 'Campaign restored to revision from %s', 'bulkmail' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6  => sprintf( esc_html__( 'Campaign published. %s', 'bulkmail' ), '<a href="' . esc_url( get_permalink( $post_id ) ) . '">' . esc_html__( 'View Newsletter', 'bulkmail' ) . '</a>' ),
			7  => esc_html__( 'Campaign saved.', 'bulkmail' ),
			8  => sprintf( esc_html__( 'Campaign submitted. %s', 'bulkmail' ), '<a href="' . esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_id ) ) ) . '" target="_blank" rel="noopener">' . esc_html__( 'Preview Newsletter', 'bulkmail' ) . '</a>' ),
			9  => esc_html__( 'Campaign scheduled.', 'bulkmail' ),
			10 => esc_html__( 'Campaign draft updated.', 'bulkmail' ),
		);
	...
	public function columns( $columns ) {
		...
		$columns = array(
			...
			'title'   => esc_html__( 'Name', 'bulkmail' ),
			'status'  => esc_html__( 'Status', 'bulkmail' ),
			'total'   => esc_html__( 'Total', 'bulkmail' ),
			'open'    => esc_html__( 'Open', 'bulkmail' ),
			'click'   => esc_html__( 'Clicks', 'bulkmail' ),
			'unsubs'  => esc_html__( 'Unsubscribes', 'bulkmail' ),
			'bounces' => esc_html__( 'Bounces', 'bulkmail' ),
		...
		switch ( $column ) {
			...
			case 'status':
				...
				if ( ! in_array( $post->post_status, array( 'pending', 'auto-draft' ) ) ) {
					...
					switch ( $status ) {
						case 'paused':
							echo '<span class="bulkmail-icon paused"></span> ';
							esc_html_e( 'Paused', 'bulkmail' );
							if ( $totals = $this->get_totals( $post->ID ) ) {
								...
								echo '<br><span class="bulkmail-icon no-receiver"></span> ' . esc_html__( 'no receivers!', 'bulkmail' );
							}
							break;
						case 'active':
							if ( $totals = $this->get_totals( $post->ID ) ) {
								$sent = $this->get_sent( $post->ID );
								echo '<span class="bulkmail-icon progressing"></span> ' . ( $sent == $totals ? esc_html__( 'completing job', 'bulkmail' ) : esc_html__( 'progressing', 'bulkmail' ) ) . '&hellip;' . ( $meta['timezone'] ? ' <span class="timezonebased"  title="' . esc_html__( 'This campaign is based on subscribers timezone and probably will take up to 24 hours', 'bulkmail' ) . '">24h</span>' : '' );
								$p  = $totals ? round( $sent / $totals * 100 ) : 0;
								$pg = sprintf( esc_html__( '%1$s of %2$s sent', 'bulkmail' ), number_format_i18n( $sent ), number_format_i18n( $totals ) );
								echo "<br><div class='campaign-progress'><span class='bar' style='width:$p%'><span>&nbsp;$pg</span></span><span>&nbsp;$pg</span><var>$p%</var></div>";
							} elseif ( is_null( $totals ) ) {
							} else {
								echo '<span class="bulkmail-icon no-receiver"></span> ' . esc_html__( 'no receivers!', 'bulkmail' );
							}
							echo '<div class="campaign-status"></div>';
							break;
						case 'queued':
							echo '<span class="bulkmail-icon queued"></span> ';
							if ( $meta['timezone'] && $timestamp - $now < 86400 ) :
								$sub       = $this->get_unsent_subscribers( $post->ID, array( 1 ), true );
								$timestamp = min( bulkmail( 'subscribers' )->get_timeoffset_timestamps( $sub, $timestamp ) );
								endif;
							printf( esc_html__( 'starts in %s', 'bulkmail' ), ( $timestamp - $now > 60 ) ? human_time_diff( $timestamp ) : esc_html__( 'less than a minute', 'bulkmail' ) );
							echo $meta['timezone'] ? ' <span class="timezonebased"  title="' . esc_attr__( 'This campaign is based on subscribers timezone and probably will take up to 24 hours', 'bulkmail' ) . '">24h</span>' : '';
							echo "<br><span class='nonessential'>(" . date( $timeformat, $timestamp + $timeoffset ) . ')</span>';
							echo '<div class="campaign-status"></div>';
							break;
						case 'finished':
							echo '<span class="bulkmail-icon finished"></span> ' . esc_html__( 'Finished', 'bulkmail' );
							echo '<br><span class="nonessential">(' . date( $timeformat, $meta['finished'] + $timeoffset ) . ')</span>';
							break;
						case 'draft':
							echo '<span class="bulkmail-icon draft"></span> ' . $wp_post_statuses['draft']->label;
							break;
						case 'trash':
							echo $wp_post_statuses['trash']->label;
							break;
						case 'autoresponder':
							include BULKEMAIL_DIR . 'includes/autoresponder.php';

							$autoresponder = $meta['autoresponder'];

							echo '<span class="bulkmail-icon ' . $active . '"></span> ' . ( $is_active ? esc_html__( 'active', 'bulkmail' ) : esc_html__( 'inactive', 'bulkmail' ) );
							echo '<br>';
							...
							if ( 'bulkmail_autoresponder_timebased' == $autoresponder['action'] ) {

								$pts = bulkmail( 'helper' )->get_post_types( true, 'objects' );

								if ( $meta['timestamp'] && $meta['timestamp'] - $now < 0 ) {
									bulkmail( 'queue' )->autoresponder_timebased( $post->ID );
								}

								printf(
									esc_html__( 'send every %1$s %2$s', 'bulkmail' ),
								...
								if ( $meta['timestamp'] ) {
									echo '<br>';
									printf(
										esc_html__( 'next campaign in %s', 'bulkmail' ),
										'<strong title="' . date( $timeformat, $meta['timestamp'] + $timeoffset ) . '">' . human_time_diff( $meta['timestamp'] ) . '</strong>'
									);
									echo ' &ndash; ' . sprintf( '#%s', '<strong title="' . sprintf( esc_html__( 'Next issue: %s', 'bulkmail' ), '#' . $autoresponder['issue'] ) . '">' . $autoresponder['issue'] . '</strong>' );
									if ( isset( $autoresponder['since'] ) && $autoresponder['since'] ) {
										echo '<br>' . esc_html__( 'only if new content is available.', 'bulkmail' );
									}
									if ( isset( $autoresponder['time_conditions'] ) ) {
										if ( $posts_required = max( 0, ( $autoresponder['time_post_count'] - $autoresponder['post_count_status'] ) ) ) {
											if ( 'rss' == $autoresponder['time_post_type'] ) {
												echo '<br>' . sprintf( esc_html__( 'requires %1$s more %2$s', 'bulkmail' ), ' <strong>' . $posts_required . '</strong>', ' <strong>' . esc_html__( 'RSS Feed', 'bulkmail' ) . '</strong>' );
											} else {
												echo '<br>' . sprintf( esc_html__( 'requires %1$s more %2$s', 'bulkmail' ), ' <strong>' . $posts_required . '</strong>', ' <strong>' . $pts[ $autoresponder['time_post_type'] ]->labels->name . '</strong>' );
											}
										}
									}
								}
								...
								if ( count( $autoresponder['weekdays'] ) < 7 ) {
									...
									echo '<br>';
									printf(
										esc_html_x( 'only on %s', 'only on [weekdays]', 'bulkmail' ),
										' <strong>' . implode( ', ', $days ) . '</strong>'
									);
								}
							} elseif ( 'bulkmail_autoresponder_usertime' == $autoresponder['action'] ) {

								$datefields = bulkmail()->get_custom_date_fields();

								if ( $autoresponder['userexactdate'] ) :

									printf(
										esc_html__( 'send %1$s %2$s %3$s', 'bulkmail' ),
										'<strong>' . $autoresponder['amount'] . '</strong>',
										'<strong>' . $time_frame_names[ $autoresponder['unit'] ] . '</strong>',
										( $autoresponder['before_after'] > 0 ? esc_html__( 'after', 'bulkmail' ) : esc_html__( 'before', 'bulkmail' ) )
									);

									echo ' ' . sprintf( esc_html__( 'the users %1$s value', 'bulkmail' ), ' <strong>' . ( isset( $datefields[ $autoresponder['uservalue'] ] ) ? $datefields[ $autoresponder['uservalue'] ]['name'] : $autoresponder['uservalue'] ) . '</strong>' );
								else :
									printf(
										esc_html__( 'send every %1$s %2$s', 'bulkmail' ),
									...
									echo ' ' . sprintf( esc_html__( 'based on the users %1$s value', 'bulkmail' ), ' <strong>' . ( isset( $datefields[ $autoresponder['uservalue'] ] ) ? $datefields[ $autoresponder['uservalue'] ]['name'] : $autoresponder['uservalue'] ) . '</strong>' );

								endif;

							} elseif ( 'bulkmail_autoresponder_followup' == $autoresponder['action'] ) {

								if ( $campaign = $this->get( $post->post_parent ) ) {
									$types = array(
										1 => esc_html__( 'has been sent', 'bulkmail' ),
										2 => esc_html__( 'has been opened', 'bulkmail' ),
										3 => esc_html__( 'has been clicked', 'bulkmail' ),
									);
									printf(
										esc_html__( 'send %1$s %2$s %3$s', 'bulkmail' ),
										( $autoresponder['amount'] ? '<strong>' . $autoresponder['amount'] . '</strong> ' . $bulkmail_autoresponder_info['units'][ $autoresponder['unit'] ] : esc_html__( 'immediately', 'bulkmail' ) ),
										esc_html__( 'after', 'bulkmail' ),
										' <strong><a href="post.php?post=' . $campaign->ID . '&action=edit">' . $campaign->post_title . '</a></strong> ' . $types[ $autoresponder['followup_action'] ]
									);

								} else {
									echo '<br><span class="bulkmail-icon warning"></span> ' . esc_html__( 'Campaign does not exist', 'bulkmail' );
								}
							} else {
								...
								printf(
									...
									( $autoresponder['amount'] ? '<strong>' . $autoresponder['amount'] . '</strong> ' . $bulkmail_autoresponder_info['units'][ $autoresponder['unit'] ] : esc_html__( 'immediately', 'bulkmail' ) ),
									esc_html__( 'after', 'bulkmail' ),
									' <strong>' . $bulkmail_autoresponder_info['actions'][ $autoresponder['action'] ]['label'] . '</strong>'
								);
							...
							if ( ( current_user_can( 'bulkmail_edit_autoresponders' ) && ( get_current_user_id() == $post->post_author || current_user_can( 'bulkmail_edit_others_autoresponders' ) ) ) ) {
								if ( $active != 'active' ) {
									$actions['activate'] = '<a class="start live-action" href="?post_type=newsletter&activate=' . $post->ID . ( isset( $_GET['post_status'] ) ? '&post_status=' . esc_attr( $_GET['post_status'] ) : '' ) . '&_wpnonce=' . wp_create_nonce( 'bulkmail_activate_nonce' ) . '" title="' . esc_attr__( 'activate', 'bulkmail' ) . '">' . esc_html__( 'activate', 'bulkmail' ) . '</a>&nbsp;';
								} else {
									$actions['deactivate'] = '<a class="start live-action" href="?post_type=newsletter&deactivate=' . $post->ID . ( isset( $_GET['post_status'] ) ? '&post_status=' . esc_attr( $_GET['post_status'] ) : '' ) . '&_wpnonce=' . wp_create_nonce( 'bulkmail_deactivate_nonce' ) . '" title="' . esc_attr__( 'deactivate', 'bulkmail' ) . '">' . esc_html__( 'deactivate', 'bulkmail' ) . '</a>&nbsp;';
								}
							}
				...
				if ( ! $meta['ignore_lists'] ) {
					...
					if ( ! empty( $lists ) ) {
						echo esc_html__( 'assigned lists', 'bulkmail' ) . ':<br>';
						foreach ( $lists as $i => $list ) {
							echo '<strong class="nowrap"><a href="edit.php?post_type=newsletter&page=bulkmail_lists&ID=' . $list->ID . '">' . $list->name . '</a></strong>';
							if ( $i + 1 < count( $lists ) ) {
								echo ', ';
							}
						}
						...
						if ( ! in_array( $post->post_status, array( 'finished' ) ) ) {
							echo '<br><span class="bulkmail-icon warning"></span> ' . esc_html__( 'no lists selected', 'bulkmail' );
						}
					}
					echo '<br>';
				}
				...
				if ( $meta['list_conditions'] ) {
					...
					echo esc_html__( 'only if', 'bulkmail' ) . '<br>';

					bulkmail( 'conditions' )->render( $meta['list_conditions'] );
				}
				echo '</div>';
				...
				if ( ( current_user_can( 'publish_newsletters' ) && get_current_user_id() == $post->post_author ) || current_user_can( 'edit_others_newsletters' ) ) {
					if ( $post->post_status == 'queued' ) {
						$actions['start'] = '<a class="start live-action" href="?post_type=newsletter&start=' . $post->ID . ( isset( $_GET['post_status'] ) ? '&post_status=' . esc_attr( $_GET['post_status'] ) : '' ) . '&_wpnonce=' . wp_create_nonce( 'bulkmail_start_nonce' ) . '" title="' . esc_attr__( 'Start Campaign now', 'bulkmail' ) . '">' . esc_html__( 'Start now', 'bulkmail' ) . '</a>';
					}
					if ( in_array( $post->post_status, array( 'active', 'queued' ) ) && $status != 'finished' ) {
						$actions['pause'] = '<a class="pause live-action" href="?post_type=newsletter&pause=' . $post->ID . ( isset( $_GET['post_status'] ) ? '&post_status=' . esc_attr( $_GET['post_status'] ) : '' ) . '&_wpnonce=' . wp_create_nonce( 'bulkmail_pause_nonce' ) . '" title="' . esc_attr__( 'Pause Campaign', 'bulkmail' ) . '">' . esc_html__( 'Pause', 'bulkmail' ) . '</a>';
					} elseif ( $post->post_status == 'paused' && $totals ) {
						if ( ! empty( $meta['timestamp'] ) && $sent ) {
							$actions['start'] = '<a class="start live-action" href="?post_type=newsletter&resume=' . $post->ID . ( isset( $_GET['post_status'] ) ? '&post_status=' . esc_attr( $_GET['post_status'] ) : '' ) . '&_wpnonce=' . wp_create_nonce( 'bulkmail_start_nonce' ) . '" title="' . esc_attr__( 'Resume Campaign', 'bulkmail' ) . '">' . esc_html__( 'Resume', 'bulkmail' ) . '</a>';
						} else {
							$actions['start'] = '<a class="start live-action" href="?post_type=newsletter&start=' . $post->ID . ( isset( $_GET['post_status'] ) ? '&post_status=' . esc_attr( $_GET['post_status'] ) : '' ) . '&_wpnonce=' . wp_create_nonce( 'bulkmail_start_nonce' ) . '" title="' . esc_attr__( 'Start Campaign', 'bulkmail' ) . '">' . esc_html__( 'Start', 'bulkmail' ) . '</a>';
						}
					}
					if ( in_array( $post->post_status, array( 'active', 'paused' ) ) && $totals && $sent ) {
						$actions['finish'] = '<a class="finish live-action" href="?post_type=newsletter&finish=' . $post->ID . ( isset( $_GET['post_status'] ) ? '&post_status=' . esc_attr( $_GET['post_status'] ) : '' ) . '&_wpnonce=' . wp_create_nonce( 'bulkmail_finish_nonce' ) . '" title="' . esc_attr__( 'Finish Campaign', 'bulkmail' ) . '">' . esc_html__( 'Finish', 'bulkmail' ) . '</a>';
					}
				}
				if ( ! empty( $actions ) ) {
			...
			case 'total':
				...
				if ( ! empty( $errors ) ) {
					echo '&nbsp;(<a href="edit.php?post_type=newsletter&page=bulkmail_subscribers&status=4" class="errors" title="' . sprintf( esc_html__( '%d emails have not been sent', 'bulkmail' ), $errors ) . '">+' . $errors . '</a>)';
				}
			...
			case 'open':
				if ( ! $this->meta( $post->ID, 'track_opens' ) ) {
					echo '<span class="bulkmail-icon-lock" title="' . esc_attr__( 'Tracking is disabled for this campaign!', 'default' ) . '"></span>';
					...
					echo "<br><span title='" . sprintf( esc_attr__( '%s of sent', 'bulkmail' ), $rate . '%' ) . "' class='nonessential'>";
					echo ' (' . $rate . '%)';
					echo '</span>';
				} else {
					echo '&ndash;';
			...
			case 'click':
				if ( ! $this->meta( $post->ID, 'track_clicks' ) ) {
					...
					if ( $rate ) {
						echo "<br><span class='nonessential'>(<span title='" . sprintf( esc_attr__( '%s of sent', 'bulkmail' ), $rate . '%' ) . "'>";
						echo '' . $rate . '%';
						echo '</span>|';
						echo "<span title='" . sprintf( esc_attr__( '%s of opens', 'bulkmail' ), $rate_a . '%' ) . "'>";
						echo '' . $rate_a . '%';
						echo '</span>)</span>';
					} else {
						echo "<br><span title='" . sprintf( esc_attr__( '%s of sent', 'bulkmail' ), $rate . '%' ) . "' class='nonessential'>";
			...
			case 'bounces':
				if ( in_array( $post->post_status, array( 'finished', 'active', 'paused', 'autoresponder' ) ) ) {
					...
					echo "<br><span title='" . sprintf( esc_attr__( '%s of totals', 'bulkmail' ), $rate . '%' ) . "' class='nonessential'>";
					echo ' (' . $rate . '%)';
					echo '</span>';
				} else {
					echo '&ndash;';
	...
	public function bulk_actions( $actions ) {
		...
		$actions['duplicate'] = esc_html__( 'Duplicate', 'bulkmail' );
		if ( isset( $_GET['post_status'] ) && 'autoresponder' == $_GET['post_status'] ) {
			$actions['activate']   = esc_html__( 'Activate', 'bulkmail' );
			$actions['deactivate'] = esc_html__( 'Deactivate', 'bulkmail' );
		} else {
			$actions['start']  = esc_html__( 'Start', 'bulkmail' );
			$actions['pause']  = esc_html__( 'Pause', 'bulkmail' );
			$actions['resume'] = esc_html__( 'Resume', 'bulkmail' );
			$actions['finish'] = esc_html__( 'Finish', 'bulkmail' );
		}
	...
	public function handle_bulk_actions() {
			...
			switch ( $action ) {
				...
				case 'delete':
					if ( current_user_can( 'bulkmail_delete_subscribers' ) ) {
						...
						if ( is_wp_error( $success ) ) {
							$error_message[] = sprintf( esc_html__( 'There was an error while deleting subscribers: %s', 'bulkmail' ), $success->get_error_message() );

						} elseif ( $success ) {
							$count           = count( $subscriber_ids );
							$error_message[] = sprintf( esc_html__( '%d Subscribers have been removed', 'bulkmail' ), $count );
						}
					}
					break;
				...
				case 'start':
					if ( ! current_user_can( 'publish_newsletters', $post_id ) ) {
						$error_message[] = esc_html__( 'You are not allowed to start this campaign.', 'bulkmail' );
					} else {
						$this->start( $post_id );
					}
					break;
				...
				case 'pause':
					if ( ! current_user_can( 'publish_newsletters', $post_id ) ) {
						$error_message[] = esc_html__( 'You are not allowed to pause this campaign.', 'bulkmail' );
					} else {
						$this->pause( $post_id );
					}
					break;
				...
				case 'resume':
					if ( ! current_user_can( 'publish_newsletters', $post_id ) ) {
						$error_message[] = esc_html__( 'You are not allowed to resume this campaign.', 'bulkmail' );
					} else {
						$this->resume( $post_id );
					}
					break;
				...
				case 'finish':
					if ( ! current_user_can( 'publish_newsletters', $post_id ) ) {
						$error_message[] = esc_html__( 'You are not allowed to finish this campaign.', 'bulkmail' );
					} else {
						$this->finish( $post_id );
					}
					break;
				...
				case 'activate':
					if ( ! current_user_can( 'publish_newsletters', $post_id ) ) {
						$error_message[] = esc_html__( 'You are not allowed to activate this campaign.', 'bulkmail' );
					} else {
				...
				case 'deactivate':
					if ( ! current_user_can( 'publish_newsletters', $post_id ) ) {
						$error_message[] = esc_html__( 'You are not allowed to deactivate this campaign.', 'bulkmail' );
					} else {
		...
		if ( ! empty( $success_message ) ) {
			bulkmail_notice( implode( ' ', $success_message ) . $message_postfix, 'success', true, 'campaigns_bulk_success', true, null, true );
		}

		if ( ! empty( $error_message ) ) {
			bulkmail_notice( implode( ' ', $error_message ) . $message_postfix, 'error', true, 'campaigns_bulk_error', true, null, true );
		}
	...
	public function quick_edit_btns( $actions, $campaign ) {
		...
		if ( ! in_array( $campaign->post_status, array( 'pending', 'auto-draft', 'trash', 'draft' ) ) ) {
			...
			if ( ( current_user_can( 'duplicate_newsletters' ) && get_current_user_id() == $campaign->post_author ) || current_user_can( 'duplicate_others_newsletters' ) ) {
				$actions['duplicate'] = '<a class="duplicate" href="?post_type=newsletter&duplicate=' . $campaign->ID . ( isset( $_GET['post_status'] ) ? '&post_status=' . $_GET['post_status'] : '' ) . '&_wpnonce=' . wp_create_nonce( 'bulkmail_duplicate_nonce' ) . '" title="' . sprintf( esc_html__( 'Duplicate Campaign %s', 'bulkmail' ), '&quot;' . $campaign->post_title . '&quot;' ) . '">' . esc_html__( 'Duplicate', 'bulkmail' ) . '</a>';
			}
			...
			if ( ( current_user_can( 'publish_newsletters' ) && get_current_user_id() == $campaign->post_author ) || current_user_can( 'edit_others_newsletters' ) ) {
				$actions['statistics'] = '<a class="statistics" href="post.php?post=' . $campaign->ID . '&action=edit&showstats=1" title="' . sprintf( esc_html__( 'See stats of Campaign %s', 'bulkmail' ), '&quot;' . $campaign->post_title . '&quot;' ) . '">' . esc_html__( 'Statistics', 'bulkmail' ) . '</a>';
			}

			if ( $parent_id = (int) $this->meta( $campaign->ID, 'parent_id' ) ) {
				$actions['autoresponder_link'] = '<a class="edit_base" href="post.php?post=' . $parent_id . '&action=edit">' . esc_html__( 'Edit base', 'bulkmail' ) . '</a>';
			}
		}
	...
	public function title( $title ) {
		return esc_html__( 'Enter Campaign Title here', 'bulkmail' );
	}
	...
	public function assets() {
		...
		if ( 'edit-newsletter' == $screen->id ) {
			wp_enqueue_script( 'bulkmail-overview', BULKEMAIL_URI . 'assets/js/overview-script' . $suffix . '.js', array( 'bulkmail-script' ), BULKEMAIL_VERSION, true );

			wp_enqueue_style( 'bulkmail-overview', BULKEMAIL_URI . 'assets/css/overview-style' . $suffix . '.css', array(), BULKEMAIL_VERSION );

			bulkmail_localize_script(
				'campaigns',
				array(
					'finish_campaign'  => esc_html__( 'Do you really like to finish this campaign?', 'bulkmail' ),
					'finish_campaigns' => esc_html__( 'Do you really like to finish selected campaigns?', 'bulkmail' ),
					'start_campaigns'  => esc_html__( 'Do you really like to start selected campaigns?', 'bulkmail' ),
				)
			);
			...
			add_action( 'dbx_post_sidebar', array( bulkmail( 'ajax' ), 'add_ajax_nonce' ) );
			...
			if ( isset( $_GET['template'] ) ) {
				...
				$this->set_template( bulkmail_option( 'default_template' ), $this->post_data['file'] );
			}
			...
			if ( in_array( $post->post_status, array( 'active', 'finished' ) ) || isset( $_GET['showstats'] ) ) {
				...
				wp_add_inline_style( 'bulkmail-newsletter', '#local-storage-notice{display:none !important}' );
				...
				wp_enqueue_script( 'bulkmail-emojipicker', BULKEMAIL_URI . 'assets/js/libs/emoji-button.js', array(), BULKEMAIL_VERSION );

				if ( user_can_richedit() ) {
					wp_enqueue_script( 'editor' );
				}

				wp_enqueue_style( 'jquery-ui-style', BULKEMAIL_URI . 'assets/css/libs/jquery-ui' . $suffix . '.css', array(), BULKEMAIL_VERSION );
				wp_enqueue_style( 'jquery-datepicker', BULKEMAIL_URI . 'assets/css/datepicker' . $suffix . '.css', array(), BULKEMAIL_VERSION );
			...
			wp_enqueue_script( 'bulkmail-campaign', BULKEMAIL_URI . 'assets/js/campaign-script' . $suffix . '.js', array( 'bulkmail-script' ), BULKEMAIL_VERSION, true );
			wp_enqueue_style( 'bulkmail-campaign', BULKEMAIL_URI . 'assets/css/campaign-style' . $suffix . '.css', array(), BULKEMAIL_VERSION );

			wp_enqueue_script( 'bulkmail-editbar', BULKEMAIL_URI . 'assets/js/editbar-script' . $suffix . '.js', array( 'bulkmail-campaign' ), BULKEMAIL_VERSION, true );
			wp_enqueue_style( 'bulkmail-editbar', BULKEMAIL_URI . 'assets/css/editbar-style' . $suffix . '.css', array(), BULKEMAIL_VERSION );

			wp_enqueue_script( 'bulkmail-optionbar', BULKEMAIL_URI . 'assets/js/optionbar-script' . $suffix . '.js', array( 'bulkmail-campaign' ), BULKEMAIL_VERSION, true );
			wp_enqueue_style( 'bulkmail-optionbar', BULKEMAIL_URI . 'assets/css/optionbar-style' . $suffix . '.css', array(), BULKEMAIL_VERSION );

			wp_enqueue_style( 'bulkmail-flags', BULKEMAIL_URI . 'assets/css/flags' . $suffix . '.css', array(), BULKEMAIL_VERSION );

			bulkmail_localize_script(
				'conditions',
				array(
					'next'          => esc_html__( 'next', 'bulkmail' ),
					'prev'          => esc_html__( 'prev', 'bulkmail' ),
			...
			bulkmail_localize_script(
				...
				array(
					...
					'or'                     => esc_html__( 'or', 'bulkmail' ),
					'move_module_up'         => esc_html__( 'Move module up', 'bulkmail' ),
					'move_module_down'       => esc_html__( 'Move module down', 'bulkmail' ),
					'duplicate_module'       => esc_html__( 'Duplicate module', 'bulkmail' ),
					'remove_module'          => esc_html__( 'Remove module', 'bulkmail' ),
					'remove_all_modules'     => esc_html__( 'Do you really like to remove all modules?', 'bulkmail' ),
					'save_template'          => esc_html__( 'Save Template File', 'bulkmail' ),
					'add_module'             => esc_html__( 'Add Module', 'bulkmail' ),
					'module'                 => esc_html__( 'Module %s', 'bulkmail' ),
					'codeview'               => esc_html__( 'Codeview', 'bulkmail' ),
					'module_label'           => esc_html__( 'Name of the module (click to edit)', 'bulkmail' ),
					'edit'                   => esc_html__( 'Edit', 'bulkmail' ),
					'click_to_edit'          => esc_html__( 'Click to edit %s', 'bulkmail' ),
					'click_to_add'           => esc_html__( 'Click to add %s', 'bulkmail' ),
					'auto'                   => esc_html_x( 'Auto', 'for the autoimporter', 'bulkmail' ),
					'add_button'             => esc_html__( 'Add button', 'bulkmail' ),
					'add_repeater'           => esc_html__( 'Add repeater', 'bulkmail' ),
					'remove_repeater'        => esc_html__( 'Remove repeater', 'bulkmail' ),
					'add_s'                  => esc_html__( 'Add %s', 'bulkmail' ),
					'remove_s'               => esc_html__( 'Remove %s', 'bulkmail' ),
					'curr_selected'          => esc_html__( 'Currently selected', 'bulkmail' ),
					'remove_btn'             => esc_html__( 'An empty link will remove this button! Continue?', 'bulkmail' ),
					'preview_for'            => esc_html__( 'Preview for %s', 'bulkmail' ),
					'preview'                => esc_html__( 'Preview', 'bulkmail' ),
					'read_more'              => esc_html__( 'Read more', 'bulkmail' ),
					'invalid_image'          => esc_html__( '%s does not contain a valid image', 'bulkmail' ),
					'for_area'               => esc_html__( 'Area %s', 'bulkmail' ),
					'enter_list_name'        => esc_html__( 'Enter name of the list', 'bulkmail' ),
					'create_list'            => esc_html_x( '%1$s of %2$s', '[recipientstype] of [campaignname]', 'bulkmail' ),
					'next'                   => esc_html__( 'next', 'bulkmail' ),
					'prev'                   => esc_html__( 'prev', 'bulkmail' ),
					'start_of_week'          => get_option( 'start_of_week' ),
					'day_names'              => $wp_locale->weekday,
					'day_names_min'          => array_values( $wp_locale->weekday_abbrev ),
					'month_names'            => array_values( $wp_locale->month ),
					'delete_colorschema'     => esc_html__( 'Delete this color schema?', 'bulkmail' ),
					'delete_colorschema_all' => esc_html__( 'Do you really like to delete all custom color schema for this template?', 'bulkmail' ),
					'yourscore'              => esc_html__( '%s out of 10', 'bulkmail' ),
					'yourscores'             => array(
						esc_html__( 'This mail will hardly see any inbox!', 'bulkmail' ),
						esc_html__( 'You have to make it better!', 'bulkmail' ),
						esc_html__( 'Many inboxes will refuse this mail!', 'bulkmail' ),
						esc_html__( 'Not bad at all. Improve it further!', 'bulkmail' ),
						esc_html__( 'Almost perfect!', 'bulkmail' ),
						esc_html__( 'Great! Your campaign is ready to send!', 'bulkmail' ),
					),
					'undosteps'              => bulkmail_option( 'undosteps', 10 ),
					'statuschanged'          => esc_html__( 'The status of this campaign has changed. Please reload the page or %s', 'bulkmail' ),
					'click_here'             => esc_html__( 'click here', 'bulkmail' ),
					'send_now'               => esc_html__( 'Do you really like to send this campaign now?', 'bulkmail' ),
					'select_image'           => esc_html__( 'Select Image', 'bulkmail' ),
					'add_attachment'         => esc_html__( 'Add Attachment', 'bulkmail' ),
					'edit_conditions'        => esc_html__( 'Edit Conditions', 'bulkmail' ),
					'remove_conditions'      => esc_html__( 'Do you really like to remove all conditions?', 'bulkmail' ),
					'ready'                  => esc_html__( 'ready!', 'bulkmail' ),
					'error'                  => esc_html__( 'error!', 'bulkmail' ),
					'error_occurs'           => esc_html__( 'An error occurs while uploading', 'bulkmail' ),
					'unsupported_format'     => esc_html__( 'Unsupported file format', 'bulkmail' ),
					'unknown_locations'      => esc_html__( '+ %d unknown locations', 'bulkmail' ),
				)
			);
	...
	public function inline_editor() {
		...
		global $wp_version;
		return apply_filters( 'bulkmail_inline_editor', version_compare( '4.6', $wp_version, '<=' ) );
	}
	...
	public function add_post_thumbnail_link( $content, $post_id ) {
		...
		if ( isset( $post ) && $post->post_type == 'newsletter' ) {
			...
			if ( in_array( $post->post_status, array( 'active', 'finished' ) ) || isset( $_GET['showstats'] ) ) {
				...
				$content .= '<p><label><input type="checkbox" name="auto_post_thumbnail" value="1" ' . checked( $meta, true, false ) . '> ' . esc_html__( 'Create Screenshot for Feature Image', 'bulkmail' ) . '</label></p>';

				$timestamp = wp_next_scheduled( 'bulkmail_auto_post_thumbnail', array( $post_id ) );
				if ( $timestamp + 2 >= time() ) {
					$content .= '<p class="description" title="' . esc_html__( 'Generating the screenshot may take a while. Please reload the page to update', 'bulkmail' ) . '"><span class="spinner"></span>' . esc_html__( 'Creating Screenshot', 'bulkmail' ) . '&hellip;</p>';
				}
			}
		}
	...
	public function wp_insert_post_data( $post, $postarr ) {
		...
		if ( $is_autosave && isset( $_POST['data']['bulkmaildata'] ) ) {

			parse_str( $_POST['data']['bulkmaildata'], $postdata );
			$postdata = $postdata['bulkmail_data'];

		} elseif ( isset( $_POST['bulkmail_data'] ) ) {

			$postdata = $_POST['bulkmail_data'];
		...
		$post['post_content'] = bulkmail()->sanitize_content( $post['post_content'], $postdata['head'] );

		$post['post_excerpt'] = ! empty( $postdata['autoplaintext'] )
			? bulkmail( 'helper' )->plain_text( $post['post_content'] )
			: $post['post_excerpt'];
		...
		if ( ! in_array( $post['post_status'], array( 'pending', 'draft', 'auto-draft', 'trash' ) ) ) {
			...
			$post['post_status'] = isset( $_POST['bulkmail_data']['active'] ) ? 'queued' : $post['post_status'];
		...
		if ( $post['post_status'] == 'autoresponder' && isset( $postdata['autoresponder'] ) && $postdata['autoresponder']['action'] != 'bulkmail_autoresponder_followup' ) {
			$post['post_parent'] = 0;
		}
	...
	public function save_campaign( $post_id, $post, $update = null ) {
		...
		if ( isset( $postdata ) ) {
				...
				foreach ( $postdata['attachments'] as $attachment_id ) {
					...
					if ( @is_file( $file ) ) {
						...
						if ( $total_size <= $max_size ) {
							...
							bulkmail_notice( sprintf( esc_html__( 'Attachments must not exceed the file size limit of %s!', 'bulkmail' ), '<strong>' . esc_html( size_format( $max_size ) ) . '</strong>' ), 'error', true );
						}
					} else {
						bulkmail_notice( esc_html__( 'Attachment doesn\'t exist or isn\'t readable!', 'bulkmail' ), 'error', true );
					}
				}
			} else {
				$meta['attachments'] = array();
			...
			if ( $meta['auto_post_thumbnail'] = isset( $_POST['auto_post_thumbnail'] ) ) {
				...
				wp_schedule_single_event( time(), 'bulkmail_auto_post_thumbnail', array( $post_id ) );

			} else {

				if ( $timestamp = wp_next_scheduled( 'bulkmail_auto_post_thumbnail', array( $post_id ) ) ) {
					wp_unschedule_event( $timestamp, 'bulkmail_auto_post_thumbnail', array( $post_id ) );
				}
			}
			...
			if ( $is_autoresponder ) {
				...
				if ( in_array( $autoresponder['action'], array( 'bulkmail_subscriber_insert', 'bulkmail_subscriber_unsubscribed' ) ) ) {
				...
				} elseif ( 'bulkmail_post_published' == $autoresponder['action'] ) {
				...
				if ( 'bulkmail_autoresponder_timebased' == $autoresponder['action'] ) {
					...
					$localtime = bulkmail( 'helper' )->get_next_date_in_future( $localtime - $timeoffset, 0, $autoresponder['time_frame'], $autoresponder['weekdays'] );
					...
					switch ( $autoresponder['followup_action'] ) {
						...
						case '2':
							if ( ! $this->meta( $parent_id, 'track_opens' ) ) {
								...
								bulkmail_notice( '<strong>' . sprintf( esc_html__( 'Tracking Opens is disabled in campaign %s! Please enable tracking or choose a different campaign.', 'bulkmail' ), '<a href="' . admin_url( 'post.php?post=' . $parent_campaign->ID . '&action=edit' ) . '">' . $parent_campaign->post_title . '</a>' ) . '</strong>', 'error', true );
							}
							break;
						// clicked
						case '3':
							if ( ! $this->meta( $_POST['parent_id'], 'track_clicks' ) ) {
								$parent_campaign = get_post( $parent_id );
								bulkmail_notice( sprintf( esc_html__( 'Tracking Clicks is disabled in campaign %s! Please enable tracking or choose a different campaign.', 'bulkmail' ), '<a href="' . admin_url( 'post.php?post=' . $parent_campaign->ID . '&action=edit' ) . '">' . $parent_campaign->post_title . '</a>' ), 'error', true );
							}
							break;
				...
				} elseif ( 'bulkmail_autoresponder_hook' == $autoresponder['action'] ) {

					$hooks = get_option( 'bulkmail_hooks', array() );
					if ( ! is_array( $hooks ) ) {
						$hooks = array();
					}
					...
					update_option( 'bulkmail_hooks', $hooks );
					...
					if ( empty( $autoresponder['hook'] ) ) {
						bulkmail_notice( esc_html__( 'Please define a hook which should trigger the campaign!', 'bulkmail' ), 'error', true );
					}
				} elseif ( 'bulkmail_post_published' == $autoresponder['action'] ) {
			...
			bulkmail_remove_notice( 'camp_error_' . $post_id );
		...
		if ( ! $is_autosave ) {
			...
			if ( in_array( $post->post_status, array( 'active', 'queued', 'autoresponder' ) ) && $now - $meta['timestamp'] > -3600 ) {
				...
				bulkmail( 'cron' )->update();

			}
			if ( in_array( $post->post_status, array( 'autoresponder' ) ) ) {

				switch ( $autoresponder['action'] ) {
					case 'bulkmail_autoresponder_usertime':
						bulkmail( 'queue' )->autoresponder_usertime( $post_id );
						break;
					case 'bulkmail_autoresponder_timebased':
						bulkmail( 'queue' )->autoresponder_timebased( $post_id );
						break;
					default:
						bulkmail( 'queue' )->autoresponder( $post_id );
	...
	public function list_based_opt_out( $id ) {
		...
		if ( ! bulkmail_options( 'list_based_opt_in' ) ) {
			return false;
		}
	...
	public function meta( $id, $key = null ) {
		...
		$meta = bulkmail_cache_get( $cache_key );
		if ( ! $meta ) {
			$meta = array();
		}
		...
		$sql = "SELECT post_id AS ID, meta_key, meta_value FROM {$wpdb->postmeta} WHERE meta_key LIKE '_bulkmail_%'";
		...
		foreach ( $result as $metadata ) {
			...
			$meta_key = str_replace( '_bulkmail_', '', $metadata->meta_key );
		...
		bulkmail_cache_set( $cache_key, $meta );
	...
	public function update_meta( $id, $key, $value = null ) {
		...
		foreach ( $_meta as $k => $v ) {
			...
			if ( $v == '' && ! in_array( $k, $nullvalues ) ) {
				delete_post_meta( $id, '_bulkmail_' . $k );
				...
				// emojis are urlencoded
				update_post_meta( $id, '_bulkmail_' . $k, rawurlencode( wp_unslash( $v ) ) );
			} else {
				update_post_meta( $id, '_bulkmail_' . $k, $v );
			}
		}
	...
	public function meta_defaults( $key = null ) {
		$defaults = array(
			...
			'finished'            => null,
			'active'              => null,
			'timezone'            => bulkmail_option( 'timezone' ),
			'sent'                => null,
			'error'               => null,
			'from_name'           => bulkmail_option( 'from_name' ),
			'from_email'          => bulkmail_option( 'from' ),
			'reply_to'            => bulkmail_option( 'reply_to' ),
			'subject'             => null,
			'preheader'           => null,
			'template'            => null,
			'file'                => null,
			...
			'head'                => null,
			'colors'              => null,
			'track_opens'         => bulkmail_option( 'track_opens' ),
			'track_clicks'        => bulkmail_option( 'track_clicks' ),
			'autoplaintext'       => true,
			'webversion'          => true,
	...
	public function pause( $id, $timestamp = null ) {
		...
		if ( ! $campaign ) {
			return new WP_Error( 'no_campaign', esc_html__( 'This campaign doesn\'t exists.', 'bulkmail' ) );
		}

		wp_clear_scheduled_hook( 'bulkmail_campaign_action', array( __FUNCTION__, $id ) );
		if ( ! is_null( $timestamp ) ) {
			return wp_schedule_single_event( (int) $timestamp, 'bulkmail_campaign_action', array( __FUNCTION__, $id ) );
		}
		...
		if ( $this->change_status( $campaign, 'paused' ) ) {
			do_action( 'bulkmail_campaign_pause', $id );
			return true;
		} else {
			return false;
		}
	...
	public function start( $id, $timestamp = null ) {
		...
		if ( $this->change_status( $campaign, $status ) ) {
			do_action( 'bulkmail_campaign_start', $id );
			bulkmail_remove_notice( 'camp_error_' . $id );
			return true;
	...
	public function finish( $id, $timestamp = null ) {
		...
		$placeholder = bulkmail( 'placeholder' );
		...
		do_action( 'bulkmail_finish_campaign', $id );

		bulkmail( 'queue' )->remove( $id );
	...
	public function duplicate( $id, $timestamp = null ) {
		...
		if ( $new_id ) {
			...
			do_action( 'bulkmail_campaign_duplicate', $id, $new_id );
	...
	public function autoresponder_to_campaign( $id, $delay = 0, $issue = '', $relative_to_absolute = true, $index_offset = 0 ) {
		...
		if ( $campaign->post_status != 'autoresponder' ) {
			return new WP_Error( 'no_autoresponder_campaign', esc_html__( 'This campaign is not an autoresponder.', 'bulkmail' ) );
		}
	...
	public function cleanup_after_delete( $id ) {
		...
		if ( $delete_action ) {
			$wpdb->query( $wpdb->prepare( "DELETE actions FROM {$wpdb->prefix}bulkmail_actions AS actions WHERE actions.campaign_id = %d", $id ) );

		} else {
			$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}bulkmail_actions AS actions SET actions.campaign_id = NULL WHERE actions.campaign_id = %d", $id ) );
		}
		// remove queue and subscriber meta
		$wpdb->query( $wpdb->prepare( "DELETE queue FROM {$wpdb->prefix}bulkmail_queue AS queue WHERE queue.campaign_id = %d", $id ) );
		$wpdb->query( $wpdb->prepare( "DELETE subscriber_meta FROM {$wpdb->prefix}bulkmail_subscriber_meta AS subscriber_meta WHERE subscriber_meta.campaign_id = %d", $id ) );

		// unassign existing parents
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->postmeta} WHERE meta_value = %d AND meta_key = '_bulkmail_parent_id'", $id ) );
	...
	public function get_lists( $id, $ids_only = false ) {
		...
		return bulkmail( 'lists' )->get( $list_ids, false );
	...
	public function get_autoresponder( $args = '', $type = null ) {
		...
		$args     = wp_parse_args( $args, $defaults );
		if ( ! is_null( $type ) ) {
			$args['meta_key']     = '_bulkmail_autoresponder';
			$args['meta_compare'] = 'LIKE';
			$args['meta_value']   = '"bulkmail_' . $type . '"';
		}
	...
	public function get_subscribers( $id = null, $statuses = null, $return_ids = false, $ignore_sent = false, $ignore_queue = false, $limit = null, $offset = 0, $returnsql = false ) {
		...
		return bulkmail( 'subscribers' )->query(
			array(
				'lists'         => $lists,
	...
	public function get_subscribers_by_lists( $lists = false, $conditions = null, $statuses = null, $return_ids = false, $ignore_sent = false, $ignore_queue = false, $limit = null, $offset = 0, $returnsql = false ) {
		...
		_deprecated_function( __FUNCTION__, '2.3', 'bulkmail(\'subscribers\')->query()' );
	...
	public function get_sent_subscribers( $id = null ) {
		...
		if ( false === ( $sent_subscribers = bulkmail_cache_get( 'sent_subscribers' ) ) ) {

			$sql = "SELECT a.campaign_id, a.subscriber_id FROM {$wpdb->prefix}bulkmail_actions AS a WHERE type = 1 ORDER BY a.timestamp ASC";
			...
			bulkmail_cache_set( 'sent_subscribers', $sent_subscribers );
	...
	private function get_action( $action, $id = null, $total = false ) {
		...
		return bulkmail( 'actions' )->get_by_campaign( $id, $action . ( $total ? '_total' : '' ) );
	...
	public function get_clicked_links( $id = null ) {
		...
		return bulkmail( 'actions' )->get_clicked_links( $id );
	...
	public function get_error_list( $id = null ) {
		...
		return bulkmail( 'actions' )->get_error_list( $id );
	...
	public function get_clients( $id = null ) {
		...
		return bulkmail( 'actions' )->get_clients( $id );
	...
	public function get_environment( $id = null ) {
		...
		return bulkmail( 'actions' )->get_environment( $id );
	...
	public function get_geo_data( $id ) {
		...
		$sql = "SELECT COUNT(DISTINCT a.subscriber_id) AS count, a.meta_value AS geo, b.meta_value AS coords FROM {$wpdb->prefix}bulkmail_subscriber_meta AS a LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS b ON a.subscriber_id = b.subscriber_id AND a.campaign_id = b.campaign_id AND b.meta_key = 'coords' WHERE a.meta_key = 'geo' AND a.campaign_id = %d AND a.meta_value != '|' GROUP BY a.meta_value ORDER BY count DESC";
		...
		foreach ( $result as $row ) {
			...
			if ( ! $row->coords ) {
				...
				$geo_data[ $geo[0] ][] = array(
					...
					$geo[1],
					(int) $row->count,
					$row->count . ' ' . esc_html__( _n( 'opened', 'opens', $row->count, 'bulkmail' ) ),
				);
			}
		}
	...
	public function create_list_from_option( $name, $campaign_id, $option = 'open', $countonly = false ) {
		...
		if ( ! current_user_can( 'bulkmail_edit_lists' ) ) {
			return false;
		}
		...
		if ( $countonly ) {
			...
			return bulkmail( 'subscribers' )->query( $args, $campaign->ID );
		}

		$args['return_ids'] = true;

		$subscribers = bulkmail( 'subscribers' )->query( $args, $campaign->ID );
		...
		$list = bulkmail( 'lists' )->add_segment(
			array(
				'name'        => $name,
				'description' => sprintf( esc_html_x( 'A segment of all %1$s of %2$s', 'segment of all [recipients] from campaign [campaign]', 'bulkmail' ), $options[ $option ], '"' . $campaign->post_title . '"' ),
	...
	public function get_recipients_part( $campaign_id, $parts = array( 'unopen', 'opens', 'clicks', 'unsubs', 'bounces' ), $page = 0, $orderby = 'sent', $order = 'ASC' ) {
		...
		$limit  = apply_filters( 'bulkmail_get_recipients_part', 1000 );
		...
		$fields = array(
			...
			'sent'        => esc_html__( 'Sent Date', 'bulkmail' ),
			'open'        => esc_html__( 'Open Date', 'bulkmail' ),
			'open_count'  => esc_html__( 'Open Count', 'bulkmail' ),
			'clicks'      => esc_html__( 'Click Date', 'bulkmail' ),
			'click_count' => esc_html__( 'Click Count', 'bulkmail' ),
			'unsubs'      => esc_html__( 'Unsubscribes', 'bulkmail' ),
			'bounces'     => esc_html__( 'Bounces', 'bulkmail' ),
		);
		...
		if ( ! $offset ) {
			$return .= '<div class="ajax-list-header filter-list"><label>' . esc_html__( 'Filter', 'bulkmail' ) . ': </label> ';
			if ( $track_opens ) {
				$return .= '<label><input type="checkbox" class="recipients-limit show-unopen" value="unopen" ' . checked( $unopen, true, false ) . '> ' . esc_html__( 'unopens', 'bulkmail' ) . ' </label> ';
				$return .= '<label><input type="checkbox" class="recipients-limit show-open" value="opens" ' . checked( $opens, true, false ) . '> ' . esc_html__( 'opens', 'bulkmail' ) . ' </label> ';
			}
			if ( $track_clicks ) {
				$return .= '<label><input type="checkbox" class="recipients-limit show-click" value="clicks"' . checked( $clicks, true, false ) . '> ' . esc_html__( 'clicks', 'bulkmail' ) . ' </label> ';
			}
			$return .= '<label><input type="checkbox" class="recipients-limit show-unsubscribes" value="unsubs"' . checked( $unsubs, true, false ) . '> ' . esc_html__( 'unsubscribes', 'bulkmail' ) . '</label> ';
			$return .= '<label><input type="checkbox" class="recipients-limit show-bounces" value="bounces"' . checked( $bounces, true, false ) . '> ' . esc_html__( 'bounces', 'bulkmail' ) . ' </label> ';
			$return .= '<label>' . esc_html__( 'order by', 'bulkmail' ) . ' ';
			$return .= '<select class="recipients-order">';
			foreach ( $fields as $field => $name ) {
				$return .= '<option value="' . $field . '" ' . selected( $field, $orderby, false ) . '>' . $name . '</option>';
			}
			$return .= '</select></label>';
			$return .= '<a title="' . esc_html__( 'order direction', 'bulkmail' ) . '" class="recipients-order bulkmail-icon ' . ( $order == 'ASC' ? 'asc' : 'desc' ) . '"></a>';
			$return .= '</div>';
		}
		...
		if ( ! $offset ) {
			...
			if ( $deleted ) {
				$return .= '<tr><td>&nbsp;</td><td colspan="8"><em>' . esc_html__( 'Deleted Subscribers are not listed.', 'bulkmail' ) . '</em></td></tr>';
			}
		}
		...
		foreach ( $subscribers as $i => $subscriber ) {
			...
			$return .= '<td title="' . esc_html__( 'sent', 'bulkmail' ) . '">' . ( $subscriber->sent ? str_replace( ' ', '&nbsp;', date( $timeformat, $subscriber->sent + $timeoffset ) ) : '&ndash;' ) . '</td>';
			$return .= '<td>' . ( isset( $subscriber->open_count ) && $subscriber->open_count ? '<span title="' . esc_html__( 'has opened', 'bulkmail' ) . '" class="bulkmail-icon bulkmail-icon-open"></span>' : '<span title="' . esc_html__( 'has not opened yet', 'bulkmail' ) . '" class="bulkmail-icon bulkmail-icon-unopen"></span>' ) . '</td>';
			$return .= '<td>' . ( isset( $subscriber->click_count_total ) && $subscriber->click_count_total ? sprintf( esc_html__( _n( '%s click', '%s clicks', $subscriber->click_count_total, 'bulkmail' ) ), $subscriber->click_count_total ) : '' ) . '</td>';
			$return .= '<td>' . ( isset( $subscriber->unsubs ) && $subscriber->unsubs ? '<span title="' . esc_html__( 'has unsubscribed', 'bulkmail' ) . '" class="bulkmail-icon bulkmail-icon-unsubscribe"></span>' : '' ) . '</td>';
			$return .= '<td>';
			$return .= ( isset( $subscriber->bounce_count ) ? '<span class="bounce-indicator bulkmail-icon bulkmail-icon-bounce ' . ( $subscriber->status == 3 ? 'hard' : 'soft' ) . '" title="' . sprintf( esc_html__( _n( '%s bounce', '%s bounces', $subscriber->bounce_count, 'bulkmail' ) ), $subscriber->bounce_count ) . '"></span>' : '' );
			$return .= ( $subscriber->status == 4 ) ? '<span class="bounce-indicator bulkmail-icon bulkmail-icon-bounce" title="' . esc_html__( 'an error occurred while sending to this receiver', 'bulkmail' ) . '">E</span>' : '';
			$return .= '</td>';
			$return .= '</tr>';
		...
		if ( $count && $limit == $subscribers_count ) {
			$return .= '<tr ' . ( $i % 2 ? ' class="alternate" ' : '' ) . '><td colspan="7"><a class="load-more-receivers button aligncenter" data-page="' . ( $page + 1 ) . '" data-types="' . implode( ',', $parts ) . '" data-order="' . $order . '" data-orderby="' . $orderby . '">' . esc_html__( 'load more recipients from this campaign', 'bulkmail' ) . '</a>' . '<span class="spinner"></span></td></tr>';
		}
	...
	public function get_recipients_part_sql( $campaign_id, $parts = array( 'unopen', 'opens', 'clicks', 'unsubs', 'bounces' ) ) {
		...
		$sql .= " FROM {$wpdb->prefix}bulkmail_subscribers AS a";

		$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_fields AS firstname ON a.ID = firstname.subscriber_id AND firstname.meta_key = 'firstname'";
		$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_fields AS lastname ON a.ID = lastname.subscriber_id AND lastname.meta_key = 'lastname'";

		$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_actions AS sent ON a.ID = sent.subscriber_id AND sent.type = 1";

		if ( $unopen || $opens ) {
			$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_actions AS open ON a.ID = open.subscriber_id AND open.type = 2 AND open.campaign_id = sent.campaign_id";
		}
		if ( $clicks ) {
			$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_actions AS click ON a.ID = click.subscriber_id AND click.type = 3 AND click.campaign_id = sent.campaign_id";
		}
		if ( $unsubs ) {
			$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_actions AS unsub ON a.ID = unsub.subscriber_id AND unsub.type = 4 AND unsub.campaign_id = sent.campaign_id";
		}
		if ( $bounces ) {
			$sql .= " LEFT JOIN {$wpdb->prefix}bulkmail_actions AS bounce ON a.ID = bounce.subscriber_id AND bounce.type IN (5,6) AND bounce.campaign_id = sent.campaign_id";
		}
	...
	public function heartbeat( $response, $data ) {
		...
		if ( is_wp_error( $cron_status ) ) {
			bulkmail_notice( $cron_status->get_error_message(), 'error', false, 'check_cron' );
		}

		switch ( $data['bulkmail']['page'] ) {

			case 'overview':
				if ( ! isset( $_POST['data']['bulkmail']['ids'] ) ) {
					break;
				}

				$ids    = array_filter( $_POST['data']['bulkmail']['ids'], 'is_numeric' );
				...
				foreach ( $ids as $id ) {
					...
					$sent_formatted = sprintf( esc_html__( '%1$s of %2$s sent', 'bulkmail' ), number_format_i18n( $sent ), number_format_i18n( $totals ) );
					if ( is_wp_error( $cron_status ) ) {
						$status_title = esc_html__( 'Sending Problem!', 'bulkmail' );
						if ( current_user_can( 'activate_plugins' ) ) {
							 $status_title .= ' <a href="' . admin_url( 'admin.php?page=bulkmail_tests&autostart' ) . '" class="button button-small">' . esc_html__( 'Self Test', 'bulkmail' ) . '</a>';
						}
					} else {
			...
			case 'edit':
				...
				if ( $geo_data = $this->get_geo_data( $post->ID ) ) :
					...
					foreach ( $countrycodes as $countrycode => $count ) {
						...
						$geolocation .= '<label title="' . bulkmail( 'geo' )->code2Country( $countrycode ) . '"><span class="big"><span class="bulkmail-flag-24 flag-' . strtolower( $countrycode ) . '"></span> ' . round( $count / $opens * 100, 2 ) . '%</span></label> ';
						if ( ++$i >= 5 ) {
							break;
						}
					}
		...
		$response['bulkmail'] = $return;

		// maybe change status
		bulkmail( 'queue' )->update();
		// maybe change status
		bulkmail( 'queue' )->update_status();
	...
	public function send_to_subscriber( $campaign_id, $subscriber_id, $track = null, $force = false, $log = false ) {
		...
		_deprecated_function( __FUNCTION__, '2.2', "bulkmail('campaigns')->send()" );
	...
	public function send( $campaign_id, $subscriber_id, $track = null, $force = false, $log = true, $tags = array() ) {
		...
		if ( ! $campaign || $campaign->post_type != 'newsletter' ) {
			return new WP_Error( 'wrong_post_type', esc_html__( 'wrong post type', 'bulkmail' ) );
		}

		$subscriber = bulkmail( 'subscribers' )->get( $subscriber_id, true );

		if ( ! $subscriber ) {
			return new WP_Error( 'no_subscriber', esc_html__( 'No subscriber found', 'bulkmail' ) );
		}

		if ( ! $force && $subscriber->status > 2 ) {
			return new WP_Error( 'user_unsubscribed', esc_html__( 'User has not subscribed', 'bulkmail' ) );
		}

		if ( ! $force && ! bulkmail( 'helper' )->in_timeframe() ) {
			return new WP_Error( 'system_error', esc_html__( 'Not in time frame', 'bulkmail' ) );
		}
		...
		if ( $mail->sentlimitreached ) {
			return new WP_Error( 'sendlimit_reached', sprintf( esc_html__( 'Sent limit of %1$s reached! You have to wait %2$s before you can send more mails!', 'bulkmail' ), bulkmail_option( 'send_limit' ), human_time_diff( get_option( '_transient_timeout__bulkmail_send_period_timeout' ) ) ) );
		}
		...
		$mail->bouncemail   = bulkmail_option( 'bounce' );
		$mail->preheader    = $campaign_meta['preheader'];
		$mail->embed_images = bulkmail_option( 'embed_images' );
		...
		if ( ! ( $content = bulkmail_cache_get( 'campaign_send_' . $campaign->ID ) ) ) {

			$content = bulkmail()->sanitize_content( $campaign->post_content, $campaign_meta['head'] );

			$content = bulkmail( 'helper' )->prepare_content( $content );
			if ( apply_filters( 'bulkmail_inline_css', true, $campaign->ID, $subscriber->ID ) ) {
				$content = bulkmail( 'helper' )->inline_css( $content );
			}

			bulkmail_cache_set( 'campaign_send_' . $campaign->ID, $content );
		...
		if ( $subscriber_tags = bulkmail( 'subscribers' )->meta( $subscriber->ID, 'tags', $campaign->ID ) ) {
		...
		if ( $track ) {
			...
			$content = bulkmail()->replace_links( $content, $subscriber->hash, $campaign->ID );
		...
		$mail->content = apply_filters( 'bulkmail_campaign_content', $content, $campaign, $subscriber );
		...
		$unsubscribelink = bulkmail()->get_unsubscribe_link( $campaign->ID );
		...
		if ( bulkmail_option( 'mail_opt_out' ) ) {
			...
			$listunsubscribe_body    = rawurlencode( "Please remove me from your list! {$subscriber->email} X-Bulkmail: {$subscriber->hash} X-Bulkmail-Campaign: {$campaign->ID} X-Bulkmail-ID: {$MID}" );

			$listunsubscribe[] = "<mailto:$listunsubscribe_mail?subject=$listunsubscribe_subject&body=$listunsubscribe_body>";
		}
		$listunsubscribe[] = '<' . bulkmail( 'frontpage' )->get_link( 'unsubscribe', $subscriber->hash, $campaign->ID ) . '>';

		$headers = array(
			'X-Bulkmail'          => $subscriber->hash,
			'X-Bulkmail-Campaign' => $campaign->ID,
			'X-Bulkmail-ID'       => $MID,
		...
		$mail->add_header( apply_filters( 'bulkmail_mail_headers', $headers, $campaign->ID, $subscriber->ID ) );

		$placeholder->set_content( $mail->subject );
		$mail->subject = $placeholder->get_content();

		if ( $placeholder->has_error() ) {
			return new WP_Error( 'error', sprintf( esc_html__( 'There was an error during replacing tags in this campaign! %s', 'bulkmail' ), '<br>' . implode( '<br>', $placeholder->get_error_messages() ) ) );
		}
		$result = $mail->send();

		if ( $result && ! is_wp_error( $result ) ) {
			if ( $log ) {
				do_action( 'bulkmail_send', $subscriber->ID, $campaign->ID, $result );
			}
		...
		if ( $mail->is_user_error() ) {
			if ( $log ) {
				do_action( 'bulkmail_subscriber_error', $subscriber->ID, $campaign->ID, $mail->last_error->getMessage() );
			}
		...
		if ( $mail->is_system_error() ) {
			if ( $log ) {
				do_action( 'bulkmail_system_error', $subscriber->ID, $campaign->ID, $mail->last_error->getMessage() );
			}
		...
		if ( $mail->last_error ) {
			if ( $log ) {
				do_action( 'bulkmail_campaign_error', $subscriber->ID, $campaign->ID, $mail->last_error->getMessage() );
			}

			return new WP_Error( 'error', $mail->last_error->getMessage() );
		}

		return new WP_Error( 'unknown', esc_html__( 'unknown', 'bulkmail' ) );
	...
	public function check_for_autoresponder( $new_status, $old_status, $post ) {
		...
		if ( get_post_meta( $post->ID, 'bulkmail_ignore', true ) ) {
			return;
		}

		$accepted_status = apply_filters( 'bulkmail_check_for_autoresponder_accepted_status', 'publish', $post );
		...
		// delete cache;
		bulkmail_cache_delete( 'get_last_post' );
		...
		foreach ( $campaigns as $campaign ) {
			...
			if ( 'bulkmail_post_published' == $meta['action'] ) {
				...
				if ( ! ( $meta['post_count_status'] % ( $meta['post_count'] + 1 ) ) ) {
					...
					if ( $new_id = $this->autoresponder_to_campaign( $campaign->ID, $send_offset, $meta['issue']++ ) ) {
						...
						bulkmail_notice( sprintf( esc_html__( 'New campaign %1$s has been created and is going to be sent in %2$s.', 'bulkmail' ), '<strong>"<a href="post.php?post=' . $new_campaign->ID . '&action=edit">' . $new_campaign->post_title . '</a>"</strong>', '<strong>' . date( bulkmail( 'helper' )->timeformat(), $now + $send_offset + $timeoffset ) . '</strong>' ), 'info', true );

						do_action( 'bulkmail_autoresponder_post_published', $campaign->ID, $new_id );
			...
			} elseif ( 'bulkmail_autoresponder_timebased' == $meta['action'] ) {
				...
				bulkmail( 'queue' )->autoresponder_timebased( $campaign->ID, true );
	...
	public function set_template( $slug, $file = 'index.html', $verify = false ) {
		...
		if ( $verify ) {
			...
			if ( ! is_dir( bulkmail( 'templates' )->path . '/' . $slug ) ) {
				$slug = bulkmail_option( 'default_template', $this->defaultTemplate );
			}
			if ( ! file_exists( bulkmail( 'templates' )->path . '/' . $slug . '/' . $file ) ) {
				$file = 'index.html';
			}
		}
		...
		$this->templateobj = bulkmail( 'template', $slug, $file );
	...
	public function get_post_thumbnail( $campaign_id ) {
		...
		if ( 307 == $code ) {
			wp_schedule_single_event( time() + 10, 'bulkmail_auto_post_thumbnail', array( $campaign_id ) );
		}
		...
		$filename = apply_filters( 'mymail_post_thumbnail_filename', apply_filters( 'bulkmail_post_thumbnail_filename', 'newsletter-' . $campaign_id, $campaign ), $campaign ) . '.jpg';
		...
		$attachment = array(
			...
			'post_title'     => apply_filters( 'mymail_post_thumbnail_title', apply_filters( 'bulkmail_post_thumbnail_title', $campaign->post_title, $campaign ), $campaign ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
	...
	public function get_template_by_slug( $slug, $file = 'index.html', $modules = true, $editorstyle = false ) {
		...
		$template = bulkmail( 'template', $slug, $file );
	...
	private function iframe_script_styles() {
		...
		wp_register_style( 'bulkmail-icons', BULKEMAIL_URI . 'assets/css/icons' . $suffix . '.css', array(), BULKEMAIL_VERSION );
		wp_register_style( 'bulkmail-editor-style', BULKEMAIL_URI . 'assets/css/editor-style' . $suffix . '.css', array( 'bulkmail-icons' ), BULKEMAIL_VERSION );
		wp_register_script( 'bulkmail-editor-script', BULKEMAIL_URI . 'assets/js/editor-script' . $suffix . '.js', array( 'jquery' ), BULKEMAIL_VERSION );
		...
		$bulkmaildata = array(
			...
			'url'        => BULKEMAIL_URI,
			'inline'     => $inline,
			'codeview'   => current_user_can( 'bulkmail_see_codeview' ),
			'datefields' => array_merge( array( 'added', 'updated', 'signup', 'confirm' ), bulkmail()->get_custom_date_fields( true ) ),
			'_wpnonce'   => wp_create_nonce( 'bulkmail_nonce' ),
			'isrtl'      => is_rtl(),
			'plupload'   => array(
				'runtimes'            => 'html5,flash',
				'browse_button'       => 'bulkmail-editorimage-upload-button',
				...
				'filters'             => array(
					array(
						'title'      => esc_html__( 'Image files', 'bulkmail' ),
				...
				'multipart_params'    => array(
					'action'   => 'bulkmail_editor_image_upload_handler',
		...
		if ( $inline ) {
			$toolbar1 = (string) apply_filters( 'mymail_editor_toolbar1', apply_filters( 'bulkmail_editor_toolbar1', 'bold,italic,underline,strikethrough,|,bulkmail_mce_button,|,forecolor,backcolor,|,undo,redo,|,link,unlink,|,removeformat,|,bulkmail_remove_element' ) );
			$toolbar2 = (string) apply_filters( 'mymail_editor_toolbar2', apply_filters( 'bulkmail_editor_toolbar2', 'bullist,numlist,|,alignleft,aligncenter,alignright,alignjustify' ) );
			$toolbar3 = (string) apply_filters( 'mymail_editor_toolbar3', apply_filters( 'bulkmail_editor_toolbar3', '' ) );

			$single_toolbar1 = (string) apply_filters( 'bulkmail_editor_single_toolbar1', 'bold,italic,underline,strikethrough,|,bulkmail_mce_button,|,forecolor,backcolor,|,link,unlink,|,removeformat,|,bulkmail_remove_element' );
			$single_toolbar2 = (string) apply_filters( 'bulkmail_editor_single_toolbar2', '' );
			$single_toolbar3 = (string) apply_filters( 'bulkmail_editor_single_toolbar3', '' );

			$bulkmaildata['tinymce'] = array(
				'args'   => apply_filters(
					'bulkmail_editor_tinymce_args',
					array(
						'cache_suffix'           => 'bulkmail-mce-' . BULKEMAIL_VERSION,
						'hidden_input'           => false,
						...
						'skin_url'               => BULKEMAIL_URI . 'assets/css/tinymce',
		...
		wp_localize_script( 'bulkmail-editor-script', 'bulkmaildata', $bulkmaildata );

		wp_register_script( 'bulkmail-tinymce', includes_url( 'js/tinymce/' ) . 'tinymce.min.js', array(), false, true );
		wp_register_script( 'bulkmail-tinymce-compat', includes_url( 'js/tinymce/plugins/compat3x/' ) . 'plugin' . $suffix . '.js', array(), false, true );
		wp_register_style( 'bulkmail-wp-editor', includes_url( 'css/editor' . $suffix . '.css' ) );

		ob_start();

		if ( $inline ) {
			wp_print_styles( 'dashicons' );
			wp_print_styles( 'bulkmail-wp-editor' );
			wp_print_scripts( 'utils' );
			bulkmail( 'tinymce' )->editbar_translations();
			wp_print_scripts( 'bulkmail-tinymce' );
			wp_print_scripts( 'bulkmail-tinymce-compat' );
		}

		wp_print_styles( 'bulkmail-icons' );
		wp_print_styles( 'bulkmail-editor-style' );
		...
		wp_print_scripts( 'bulkmail-editor-script' );

		bulkmail( 'helper' )->get_bulkmail_styles( true );

		do_action( 'bulkmail_iframe_script_styles' );
	...
	public function iframe_body_stuff() {
		...
		do_action( 'bulkmail_iframe_body' );
