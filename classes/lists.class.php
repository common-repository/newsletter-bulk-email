...
class BulkmailLists {
	...
	public function admin_menu() {
		...
		$page = add_submenu_page( 'edit.php?post_type=newsletter', esc_html__( 'Lists', 'bulkmail' ), esc_html__( 'Lists', 'bulkmail' ), 'bulkmail_edit_lists', 'bulkmail_lists', array( &$this, 'view_lists' ) );
	...
	public function get_columns() {
		$columns = array(
			...
			'description' => esc_html__( 'Description', 'bulkmail' ),
	...
	public function view_lists() {
		...
		if ( isset( $_GET['ID'] ) || isset( $_GET['new'] ) ) :
			...
			wp_enqueue_style( 'bulkmail-list-detail', BULKEMAIL_URI . 'assets/css/list-style' . $suffix . '.css', array(), BULKEMAIL_VERSION );
			wp_enqueue_script( 'bulkmail-list-detail', BULKEMAIL_URI . 'assets/js/list-script' . $suffix . '.js', array( 'bulkmail-script' ), BULKEMAIL_VERSION, true );
			...
			include BULKEMAIL_DIR . 'views/lists/detail.php';

		else :

			wp_enqueue_style( 'bulkmail-lists-table', BULKEMAIL_URI . 'assets/css/lists-table-style' . $suffix . '.css', array(), BULKEMAIL_VERSION );

			include BULKEMAIL_DIR . 'views/lists/overview.php';
	...
	public function bulk_actions() {
		...
		switch ( $action ) {
			...
			case 'delete':
				if ( current_user_can( 'bulkmail_delete_lists' ) ) {

					if ( $this->remove( $_POST['lists'] ) ) {
						bulkmail_notice( sprintf( esc_html__( '%d Lists have been removed', 'bulkmail' ), count( $_POST['lists'] ) ), 'error', true );
					}
			...
			case 'delete_subscribers':
				if ( current_user_can( 'bulkmail_delete_lists' ) && current_user_can( 'bulkmail_delete_subscribers' ) ) {

					if ( $this->remove( $_POST['lists'], true ) ) {
						bulkmail_notice( sprintf( esc_html__( '%d Lists with subscribers have been removed', 'bulkmail' ), count( $_POST['lists'] ) ), 'error', true );
					}
			...
			case 'subscribe':
				if ( $count = $this->change_status( $_POST['lists'], 1 ) ) {
					...
					bulkmail_notice( esc_html__( 'Subscribers have been subscribed', 'bulkmail' ), 'error', true );
			...
			case 'unsubscribe':
				if ( $this->unsubscribe( $_POST['lists'] ) ) {
					...
					bulkmail_notice( esc_html__( 'Subscribers have been unsubscribed', 'bulkmail' ), 'error', true );
			...
			case 'merge':
				if ( count( $_POST['lists'] ) == 1 ) {
					...
					bulkmail_notice( esc_html__( 'Please selected at least two lists!', 'bulkmail' ), 'error', true );
					...
					bulkmail_notice( sprintf( esc_html__( 'Lists have been merged. Please update your %s if necessary!', 'bulkmail' ), '<a href="edit.php?post_type=newsletter">' . esc_html__( 'campaigns', 'bulkmail' ) . '</a>' ), 'success', true );
	...
	public function edit_entry() {
		...
		if ( isset( $_POST['bulkmail_data'] ) ) {
			...
			if ( isset( $_POST['save'] ) ) :
				...
				// sanitize input;
				$entry   = (object) ( array_intersect_key( stripslashes_deep( $_POST['bulkmail_data'] ), (array) $empty ) );
				...
				if ( is_wp_error( $list_id ) ) {
					...
					bulkmail_notice( isset( $urlparams['new'] ) ? esc_html__( 'List added', 'bulkmail' ) : esc_html__( 'List saved', 'bulkmail' ), 'success', true );
					do_action( 'bulkmail_list_save', $list_id );
					wp_redirect( 'edit.php?post_type=newsletter&page=bulkmail_lists&ID=' . $list->ID );
					exit;
				} elseif ( $_POST['delete'] || $_POST['delete_subscribers'] ) :

					if ( $list = $this->get( (int) $_POST['bulkmail_data']['ID'], false ) ) {

						$delete_subscribers = isset( $_POST['delete_subscribers'] );

						if ( $this->remove( $list->ID, $delete_subscribers ) ) {
							bulkmail_notice( sprintf( esc_html__( 'List %s has been removed', 'bulkmail' ), '<strong>&quot;' . $list->name . '&quot;</strong>' ), 'error', true );
							do_action( 'bulkmail_list_delete', $list->ID );
							wp_redirect( 'edit.php?post_type=newsletter&page=bulkmail_lists' );
							exit;
						}
					}
	...
	public function add_segment( $entry = array(), $overwrite = false, $subscriber_ids = null ) {
		...
		$id = get_option( 'bulkmail_list_segment_id', 1 );

		$segment = $this->get( $segment_id = get_option( 'bulkmail_list_segment_parent_id' ) );
		if ( ! $segment ) {
			$segment_id = $this->add(
				array(
					'name'        => esc_html__( 'Segments', 'bulkmail' ),
					'description' => esc_html__( 'contains all segments', 'bulkmail' ),
				),
				true
			);
			update_option( 'bulkmail_list_segment_parent_id', $segment_id );
		}

		$name = isset( $entry['name'] ) && ! empty( $entry['name'] ) ? $entry['name'] : sprintf( esc_html__( 'Segment #%d', 'bulkmail' ), $id++ );
		...
		$list_id = $this->add(
			wp_parse_args(
				...
				array(
					...
					'description' => sprintf( esc_html__( 'Segment created by %s', 'bulkmail' ), $current_user->display_name ),
				)
			),
			$overwrite,
			$subscriber_ids
		...
		if ( ! is_wp_error( $list_id ) ) {
			update_option( 'bulkmail_list_segment_id', $id );
			return $list_id;
		}
	...
	public function update( $entry, $overwrite = true, $subscriber_ids = null ) {
		...
		$entry = apply_filters( 'mymail_verify_list', apply_filters( 'bulkmail_verify_list', $entry ) );
		if ( is_wp_error( $entry ) ) {
			return $entry;
		} elseif ( $entry === false ) {
			return new WP_Error( 'not_verified', esc_html__( 'List failed verification', 'bulkmail' ) );
		}
		...
		if ( isset( $data['name'] ) && empty( $data['name'] ) ) {
			$data['name'] = esc_html__( 'undefined', 'bulkmail' );
		}

		$sql = "INSERT INTO {$wpdb->prefix}bulkmail_lists (" . implode( ', ', array_keys( $data ) ) . ')';
		...
		if ( false !== $wpdb->query( $sql ) ) {
			...
			do_action( 'bulkmail_update_list', $list_id );
	...
	public function add( $entry, $overwrite = false, $subscriber_ids = null ) {
		...
		add_action( 'bulkmail_update_list', array( &$this, 'update_forms' ) );
	...
	public function update_forms( $list_id ) {
		...
		foreach ( $forms as $form ) {
			...
			bulkmail( 'forms' )->assign_lists( $form->ID, $list_id );
		}
	...
	public function remove_from_forms( $list_id ) {
		...
		foreach ( $forms as $form ) {
			bulkmail( 'forms' )->unassign_lists( $form->ID, $list_id );
		}
	...
	public function confirm_subscribers( $ids, $subscriber_ids, $force = false ) {
		...
		$sql = "UPDATE {$wpdb->prefix}bulkmail_lists_subscribers SET added = %d WHERE 1=1";
	...
	public function assign_subscribers( $ids, $subscriber_ids, $remove_old = false, $added = null ) {
		...
		if ( is_null( $added ) ) {
			$added = bulkmail_option( 'list_based_opt_in' ) ? 0 : time();
		...
		$sql = "UPDATE {$wpdb->prefix}bulkmail_lists_subscribers AS l LEFT JOIN {$wpdb->prefix}bulkmail_subscribers AS s ON s.ID = l.subscriber_id SET l.added = s.confirm WHERE l.subscriber_id IN (" . implode( ', ', $subscriber_ids ) . ') AND l.added = 0 AND s.status != 0';
	...
	public function unassign_subscribers( $ids, $subscriber_ids ) {
		...
		foreach ( $chunks as $chunk ) {
			$sql = "DELETE FROM {$wpdb->prefix}bulkmail_lists_subscribers WHERE";
	...
	public function remove( $ids, $subscribers = false ) {
		...
		if ( $subscribers ) {
			$sql = "DELETE a,b,c,d,e,f FROM {$wpdb->prefix}bulkmail_subscribers AS a LEFT JOIN {$wpdb->prefix}bulkmail_lists_subscribers b ON a.ID = b.subscriber_id LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_fields c ON a.ID = c.subscriber_id LEFT JOIN {$wpdb->prefix}bulkmail_subscriber_meta AS d ON a.ID = d.subscriber_id LEFT JOIN {$wpdb->prefix}bulkmail_actions AS e ON a.ID = e.subscriber_id LEFT JOIN {$wpdb->prefix}bulkmail_queue AS f ON a.ID = f.subscriber_id WHERE b.list_id IN (" . implode( ', ', $ids ) . ')';

			$wpdb->query( $sql );
		}

		$sql = "DELETE a,b FROM {$wpdb->prefix}bulkmail_lists AS a LEFT JOIN {$wpdb->prefix}bulkmail_lists_subscribers b ON a.ID = b.list_id WHERE a.ID IN (" . implode( ', ', $ids ) . ')';
	...
	public function merge( $ids, $name = null ) {
		...
		$segment_id    = get_option( 'bulkmail_list_segment_id', 1 );
		$merge_list_id = get_option( 'bulkmail_merged_list_id', 1 );

		if ( is_null( $name ) ) {
			$name = sprintf( esc_html__( 'Merged List #%d', 'bulkmail' ), $merge_list_id );
		}
		...
		$new_id = $this->add(
			array(
				...
				'description' => esc_html__( 'A merged list of', 'bulkmail' ) . ":\n" . implode( ', ', $list_names ),
			)
		);
		...
		if ( ! is_wp_error( $new_id ) ) {
			...
			// move connections
			$sql = "UPDATE IGNORE {$wpdb->prefix}bulkmail_lists_subscribers SET list_id = %d, added = %d WHERE list_id IN (" . implode( ', ', $ids ) . ')';
			$wpdb->query( $wpdb->prepare( $sql, $new_id, $now ) );

			$this->remove( $ids, false );

			update_option( 'bulkmail_merged_list_id', ++$merge_list_id );
	...
	public function change_status( $ids, $new_status ) {
		...
		$sql = "UPDATE {$wpdb->prefix}bulkmail_subscribers AS a LEFT JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS b ON a.ID = b.subscriber_id  SET status = %d, updated = %d WHERE b.list_id IN (" . implode( ', ', array_filter( $ids, 'is_numeric' ) ) . ')';
	...
	public function move_subscribers( $from, $to, $added = false ) {
		...
		$sql = "UPDATE {$wpdb->prefix}bulkmail_lists_subscribers SET list_id = %d" . ( $added ? ', added = ' . time() : '' ) . " WHERE {$wpdb->prefix}bulkmail_lists_subscribers.list_id IN (" . implode( ', ', array_filter( $from, 'is_numeric' ) ) . ');';
	...
	public function get( $id = null, $status = null, $counts = false ) {
		...
		if ( false === ( $lists = bulkmail_cache_get( $key ) ) ) {
			...
			if ( is_null( $id ) ) {
				...
				if ( $counts ) {
					$sql = "SELECT a.*, COUNT(DISTINCT b.ID) AS subscribers, CASE WHEN a.parent_id = 0 THEN a.ID*10 ELSE a.parent_id*10+1 END AS _sort FROM {$wpdb->prefix}bulkmail_lists AS a LEFT JOIN ( {$wpdb->prefix}bulkmail_subscribers AS b INNER JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS ab ON b.ID = ab.subscriber_id AND b.status IN(" . implode( ', ', $statuses ) . ')';
					if ( ! in_array( 0, $statuses ) ) {
						$sql .= ' AND ab.added != 0';
					}
					$sql .= ') ON a.ID = ab.list_id GROUP BY a.ID ORDER BY _sort ASC';
				} else {
					$sql = "SELECT a.*, CASE WHEN a.parent_id = 0 THEN a.ID*10 ELSE a.parent_id*10+1 END AS _sort FROM {$wpdb->prefix}bulkmail_lists AS a ORDER BY _sort ASC";
				}

				$sql = apply_filters( 'bulkmail_list_get_sql', $sql, null, $statuses, $counts );
				...
				if ( $counts ) {
					$sql = "SELECT a.*, COUNT(DISTINCT b.ID) AS subscribers FROM {$wpdb->prefix}bulkmail_lists AS a LEFT JOIN ( {$wpdb->prefix}bulkmail_subscribers AS b INNER JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS ab ON b.ID = ab.subscriber_id AND b.status IN(" . implode( ', ', $statuses ) . ')';
					if ( ! in_array( 0, $statuses ) ) {
						$sql .= ' AND ab.added != 0';
					}
					$sql .= ') ON a.ID = ab.list_id WHERE a.ID = %d GROUP BY a.ID';
				} else {
					$sql = "SELECT a.* FROM {$wpdb->prefix}bulkmail_lists AS a WHERE a.ID = %d";
				}

				$sql = apply_filters( 'bulkmail_list_get_sql', $sql, $id, $statuses, $counts );
				...
				if ( ! empty( $ids ) ) {
					...
					if ( $counts ) {
						...
						$sql = "SELECT a.* FROM {$wpdb->prefix}bulkmail_lists AS a WHERE a.ID IN(" . implode( ', ', $ids ) . ')';
					}

					$sql   = apply_filters( 'bulkmail_list_get_sql', $sql, $ids, $statuses, $counts );
					$lists = $wpdb->get_results( $sql );
				}
			}

			bulkmail_cache_add( $key, $lists );
	...
	public function get_by_name( $name, $field = null, $status = 1 ) {
		...
		if ( false === ( $result = bulkmail_cache_get( $key ) ) ) {

			if ( ! is_null( $field ) && $field != 'subscribers' ) {

				$result = $wpdb->get_var( $wpdb->prepare( 'SELECT ' . esc_sql( $field ) . " FROM {$wpdb->prefix}bulkmail_lists WHERE (name = %s OR slug = %s) LIMIT 1", $name, $name ) );
	...
	public function count( $lists = null, $statuses = null ) {
		...
		$sql = "SELECT COUNT(DISTINCT a.ID) FROM {$wpdb->prefix}bulkmail_subscribers AS a LEFT JOIN ({$wpdb->prefix}bulkmail_lists AS b INNER JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS ab ON b.ID = ab.list_id) ON a.ID = ab.subscriber_id WHERE 1=1";
	...
	public function get_list_count() {
		...
		$sql = "SELECT COUNT( * ) AS count FROM {$wpdb->prefix}bulkmail_lists";
	...
	public function get_activity( $id, $limit = null, $exclude = null ) {
		...
		return bulkmail( 'actions' )->get_list_activity( $id, $limit, $exclude );
	...
	public function get_member_count( $list_id = null, $statuses = null ) {
		...
		if ( false === ( $list_counts = bulkmail_cache_get( $key ) ) ) {

			$sql = "SELECT a.ID, a.parent_id, COUNT(DISTINCT ab.subscriber_id) AS count FROM {$wpdb->prefix}bulkmail_lists AS a LEFT JOIN ({$wpdb->prefix}bulkmail_subscribers AS b INNER JOIN {$wpdb->prefix}bulkmail_lists_subscribers AS ab ON b.ID = ab.subscriber_id AND (ab.added != 0 OR b.status = 0)) ON a.ID = ab.list_id";
			...
			bulkmail_cache_add( $key, $list_counts );
	 ...
	 */
	public function print_it( $id = null, $status = null, $name = 'bulkmail_lists', $show_count = true, $checked = array(), $type = 'checkbox' ) {
		...
		if ( $lists = $this->get( $id, $status, ! ! $show_count ) ) {
			...
			if ( $type == 'checkbox' ) {
				...
				echo esc_html('<select class="widefat" multiple name="' . $name . '">');
				foreach ( $lists as $list ) {
			...
			if ( is_admin() ) {
				echo '<ul><li>' . esc_html__( 'No Lists found!', 'bulkmail' ) . '</li><li><a href="edit.php?post_type=newsletter&page=bulkmail_lists&new">' . esc_html__( 'Create a List now', 'bulkmail' ) . '</a></li></ul>';
			}
		}
	...
	private function get_action( $action, $id = null, $total = false ) {
		...
		return bulkmail( 'actions' )->get_by_list( $id, $action . ( $total ? '_total' : '' ) );
	...
	public function on_activate( $new ) {
		...
		if ( $new ) {
			$this->add(
				array(
					'name' => esc_html__( 'Default List', 'bulkmail' ),
				),
				false,
				get_current_user_id()
			);
