...
class BulkmailGeo {
	...
	public function __construct() {
		...
		add_action( 'bulkmail_location_update', array( &$this, 'maybe_update' ) );
		add_action( 'bulkmail_cron', array( &$this, 'maybe_set_cron' ) );
	...
	public function maybe_set_cron() {
		...
		if ( bulkmail_option( 'track_location' ) ) {
			if ( bulkmail_option( 'track_location_update' ) ) {
				$this->set_cron( 'daily' );
			}
		} else {
			$this->clear_cron();
	...
	public function set_cron( $type = 'single' ) {
		...
		if ( wp_next_scheduled( 'bulkmail_location_update' ) ) {
			return;
		}
		switch ( $type ) {
			case 'single':
				wp_schedule_single_event( time(), 'bulkmail_location_update' );
				break;
			case 'daily':
				wp_schedule_event( time(), 'daily', 'bulkmail_location_update' );
				break;
		}
	}
	...
	public function clear_cron() {
		if ( wp_next_scheduled( 'bulkmail_location_update' ) ) {
			wp_clear_scheduled_hook( 'bulkmail_location_update' );
		}
	}


	public function ip2Country() {
		require_once BULKEMAIL_DIR . 'classes/libs/Ip2Country.php';
		$ip2Country = new Ip2Country( $this->get_file_path( 'country' ) );

		return $ip2Country;
	}
	public function Ip2City() {
		require_once BULKEMAIL_DIR . 'classes/libs/Ip2City.php';
	...
	public function get_file_path( $which ) {
		...
		$folder = trailingslashit( apply_filters( 'bulkmail_location_db_folder', BULKEMAIL_UPLOAD_DIR ) );

		switch ( $which ) {
			case 'country':
			case 'countries':
				return apply_filters( 'bulkmail_location_db_file_country', $folder . 'GeoIPv6.dat' );
			case 'city':
			case 'cities':
				return apply_filters( 'bulkmail_location_db_file_city', $folder . 'GeoIPCity.dat' );
				break;
		}
		return false;
	}
	...
	public function get_continents( $european_union = false ) {
		...
		$continents = array(
			'_EU' => esc_html__( 'Europe', 'bulkmail' ),
			'_AS' => esc_html__( 'Asia/Pacific Region', 'bulkmail' ),
			'_NA' => esc_html__( 'North America', 'bulkmail' ),
			'_SA' => esc_html__( 'South America', 'bulkmail' ),
			'_AF' => esc_html__( 'Africa', 'bulkmail' ),
			'_OC' => esc_html__( 'Oceania/Australia', 'bulkmail' ),
		);

		if ( $european_union ) {
			$continents['_EN'] = esc_html__( 'European Union', 'bulkmail' );
		}
