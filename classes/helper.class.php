...
class BulkmailHelper {
	...
	public function create_image( $attach_id = null, $img_url = null, $width, $height = null, $crop = false, $original = false ) {
		...
		$image = apply_filters( 'bulkmail_pre_create_image', null, $attach_id, $img_url, $width, $height, $crop, $original );
		if ( ! is_null( $image ) ) {
			return $image;
		}
			...
			if ( file_exists( $img_url ) ) {
				...
				if ( ! file_exists( $actual_file_path ) ) {
					...
					if ( false !== strpos( $img_url, '//images.unsplash.com' ) ) {
						...
						$unsplash_args = apply_filters( 'bulkmail_create_image_unsplash_args', array(), $attach_id, $img_url, $width, $height, $crop );
		...
		if ( ! file_exists( $resized_img_path ) && file_exists( $actual_file_path ) ) {
			...
			if ( $meta_data && is_array( $meta_data ) ) {
				$size_id                        = '_bulkmail-' . $width . 'x' . $height . '|' . $crop;
	...
	public function get_wpuser_meta_fields( $force = false ) {
		...
		$cache_key = 'bulkmail_wpuser_meta_fields';
	...
	public function get_addons( $force = false ) {
		...
		if ( $force || false === ( $addons = get_transient( 'bulkmail_addons' ) ) ) {

			$url = 'https://static.bulkmail.co/v1/addons.json';
			...
			if ( is_wp_error( $response ) ) {
				set_transient( 'bulkmail_addons', $response, 360 );
				return $response;
			}

			$addons = json_decode( $response_body );
			set_transient( 'bulkmail_addons', $addons, DAY_IN_SECONDS );
		}
	...
	public function install_plugin( $plugin ) {
		...
		$title        = esc_html__( 'Plugin Install', 'bulkmail' );
		$parent_file  = 'plugins.php';
		$submenu_file = 'plugin-install.php';

		$title = sprintf( esc_html__( 'Installing Plugin: %s', 'bulkmail' ), $api->name . ' ' . $api->version );
	...
	public function link_query( $args = array(), $countonly = false ) {
		...
		foreach ( $posts as $post ) {
			if ( 'post' == $post->post_type ) {
				$info = mysql2date( esc_html__( 'Y/m/d', 'bulkmail' ), $post->post_date );
			} else {
	...
	public function get_social_links( $username = '', $only_with_username = false ) {
		...
		$links = apply_filters( 'bulkmail_get_social_links', $links );
	...
	public function get_post_term_dropdown( $post_type = 'post', $labels = true, $names = false, $values = array() ) {
		...
		foreach ( $taxonomies as $id => $taxonomy ) {
			...
			foreach ( $values[ $id ] as $term ) {
				$select  = '<select class="dynamic_embed_options_taxonomy check-for-posts" ' . ( $names ? 'name="bulkmail_data[autoresponder][terms][' . $id . '][]"' : '' ) . '>';
				$select .= '<option value="-1">' . sprintf( esc_html__( 'any %s', 'bulkmail' ), $taxonomy->labels->singular_name ) . '</option>';
				foreach ( $cats as $cat ) {
			...
			$tax .= implode( ' ' . esc_html__( 'or', 'bulkmail' ) . ' ', $selects );

			$tax .= '</span><div class="bulkmail-list-operator"><span class="operator-and">' . esc_html__( 'and', 'bulkmail' ) . '</span></div></div>';
	...
	public function social_services() {
		include BULKEMAIL_DIR . 'includes/social_services.php';

		return $bulkmail_social_services;
	...
	public function using_permalinks() {
		global $wp_rewrite;
		return apply_filters( 'bulkmail_using_permalinks', is_object( $wp_rewrite ) && $wp_rewrite->using_permalinks() );
	}
	...
	public function get_first_form_id() {
		global $wpdb;
		return (int) $wpdb->get_var( "SELECT ID FROM {$wpdb->prefix}bulkmail_forms ORDER BY ID ASC LIMIT 1" );
	}
	...
	public function notifcation_template_dropdown( $selected, $fieldname, $disabled = false ) {
		...
		$templatefiles = bulkmail( 'templates' )->get_files( bulkmail_option( 'default_template' ) );
		...
		<select name="<?php echo esc_attr( $fieldname ); ?>" <?php echo $disabled ? 'disabled' : ''; ?>>
				<option value="-1" <?php selected( -1 == $selected ); ?>><?php esc_html_e( 'Plain Text (no template file)', 'bulkmail' ); ?></option>
	...
	public function media_editor_link( $attachemnt_id, $fieldname, $size = 'thumbnail' ) {
		...
		if ( ! function_exists( 'wpview_media_sandbox_styles' ) ) { // since 4.0
			...
			<?php endif; ?>
			<label><?php esc_html_e( 'Image ID', 'bulkmail' ); ?>:
			...
			wp_enqueue_script( 'bulkmail-media-editor-link-script', BULKEMAIL_URI . 'assets/js/media-editor-link-script' . $suffix . '.js', array( 'jquery' ), BULKEMAIL_VERSION );
			...
			<div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>" title="<?php esc_attr_e( 'Change Image', 'bulkmail' ); ?>" data-title="<?php esc_attr_e( 'Add Image', 'bulkmail' ); ?>">
				<img class="media-editor-link-img"<?php echo $image_url ? ' src="' . esc_attr( $image_url ) . '"' : ''; ?>>
				<a class="media-editor-link-select button" href="#"><?php esc_html_e( 'Select Image', 'bulkmail' ); ?></a>
				<a class="media-editor-link-remove" href="#" title="<?php esc_attr_e( 'Remove Image', 'bulkmail' ); ?>">&#10005;</a>
	...
	public function dateformat() {
		...
		return apply_filters( 'bulkmail_dateformat', $format );
	...
	public function timeformat() {
		...
		return apply_filters( 'bulkmail_timeformat', $format );
	...
	public function get_bounce_message( $status, $original = null ) {
		...
		$res = apply_filters( 'bulkmail_get_bounce_message', null, $status, $original );
		if ( ! is_null( $res ) ) {
			return $res;
		}

		include BULKEMAIL_DIR . 'classes/libs/bounce/bounce_statuscodes.php';
	...
	public function get_unsubscribe_message( $status, $original = null ) {
		...
		$res = apply_filters( 'bulkmail_get_unsubscribe_message', null, $status, $original );
		if ( ! is_null( $res ) ) {
			return $res;
		}
		...
		switch ( $status ) {
			...
			case 'list_unsubscribe_list':
				return esc_html__( 'The user clicked on the unsubscribe option in the Mail application', 'bulkmail' );
			case 'link_unsubscribe':
			case 'link_unsubscribe_list':
				return esc_html__( 'The user clicked on an unsubscribe link in the campaign.', 'bulkmail' );
			case 'email_unsubscribe':
			case 'email_unsubscribe_list':
				return esc_html__( 'The user canceled the subscription via the website.', 'bulkmail' );
			case 'spam_complaint':
			case 'spam_complaint_list':
				return esc_html__( 'The user marked this message as Spam in the Mail application.', 'bulkmail' );
		}
	...
	public function prepare_content( $content ) {
		...
		// custom styles
		$content = $this->add_bulkmail_styles( $content );

		// handle shortcodes
		$content = $this->handle_shortcodes( $content );

		return apply_filters( 'bulkmail_prepare_content', $content );
	...
	public function handle_shortcodes( $org_content ) {
		...
		if ( ! ( $content = bulkmail_cache_get( $key ) ) ) {

			if ( ! ( apply_filters( 'bulkmail_strip_shortcodes', ! bulkmail_option( 'shortcodes' ) ) ) ) {
				$org_content = do_shortcode( $org_content );
			}
			if ( $shortcodes = apply_filters( 'bulkmail_strip_shortcode_tags', array_keys( $shortcode_tags ) ) ) {
			...
			$content = apply_filters( 'bulkmail_handle_shortcodes', $content );
			bulkmail_cache_set( $key, $content );
	...
	public function inline_css( $content ) {
		...
		foreach ( $comments[0] as $i => $comment ) {
			$content = str_replace( $comment, '<!--Bulkmail:html_comment_' . $i . '_' . $commentid . '-->', $content );
		}
		...
		if ( preg_match_all( '#<style([^><]*)>(.*?)</style>#is', $content, $originalstyles ) ) {
			...
			if ( $has_data_image = preg_match_all( '/url\(data:image.*\)/', $content, $data_images ) ) {
				foreach ( $data_images[0] as $i => $data_image ) {
					$content = str_replace( $data_image, '/*Bulkmail:html_data_image_' . $i . '*/', $content );
				}
			}

			require BULKEMAIL_DIR . 'classes/libs/InlineStyle/autoload.php';
			...
			if ( $has_data_image ) {
				foreach ( $data_images[0] as $i => $data_image ) {
					$content = str_replace( '/*Bulkmail:html_data_image_' . $i . '*/', $data_image, $content );
				}
			}
		...
		foreach ( $comments[0] as $i => $comment ) {
			$content = str_replace( '<!--Bulkmail:html_comment_' . $i . '_' . $commentid . '-->', $comment, $content );
		}
	...
	public function add_style( $callback, $args, $embed = false ) {
		...
		if ( ! isset( $bulkmail_mystyles ) ) {
			$bulkmail_mystyles = array();
		}
		if ( ! isset( $bulkmail_mystyles[ $type ] ) ) {
			$bulkmail_mystyles[ $type ] = array();
		}

		$bulkmail_mystyles[ $type ][] = call_user_func_array( $callback, $args );
	...
	public function get_bulkmail_styles( $echo = false ) {

		// custom styles
		global $bulkmail_mystyles;

		if ( ! did_action( 'bulkmail_add_style' ) ) {
			do_action( 'bulkmail_add_style' );
		}

		$bulkmail_styles = '';

		if ( $bulkmail_mystyles ) {
			foreach ( $bulkmail_mystyles as $type => $styles ) {
				foreach ( $styles as $style ) {
					$bulkmail_styles .= '<style type="text/css"' . ( 'embed' == $type ? ' data-embed' : '' ) . '>' . "\n" . $style . "\n" . '</style>' . "\n";
				}
			}
		}

		if ( ! $echo ) {
			return $bulkmail_styles;
		}
		echo $bulkmail_styles;
	}

	public function add_bulkmail_styles( $content ) {
		$content = str_replace( '</head>', $this->get_bulkmail_styles() . '</head>', $content );
		return $content;
	}
	...
	public function wp_print_embedded_scripts( $handle ) {
		...
		( @file_exists( $path . $wp_scripts->registered[ $handle ]->src ) )
			...
			: include str_replace( BULKEMAIL_URI, BULKEMAIL_DIR, $wp_scripts->registered[ $handle ]->src );
	...
	public function wp_print_embedded_styles( $handle ) {
		...
		( @file_exists( $path . $wp_styles->registered[ $handle ]->src ) )
			...
			: include str_replace( BULKEMAIL_URI, BULKEMAIL_DIR, $wp_styles->registered[ $handle ]->src );
		$output = ob_get_contents();
	...
	public function mkdir( $folder = '', $prevent_access = true ) {
		...
		if ( 0 === strrpos( $folder, ABSPATH ) ) {
			...
			$path = trailingslashit( trailingslashit( BULKEMAIL_UPLOAD_DIR ) . $folder );
		}
	...
	public function in_timeframe( $timestamp = null ) {
		...
		$from = bulkmail_option( 'time_frame_from', 0 );
		$to   = bulkmail_option( 'time_frame_to', 0 );
		$days = bulkmail_option( 'time_frame_day' );
		if ( is_null( $timestamp ) ) {
	...
	public function get_post_types( $public_only = true, $output = 'names', $exclude = array( 'attachment', 'newsletter' ) ) {
		...
		return apply_filters( 'bulkmail_post_types', $post_types, $output );
	...
	public function get_dynamic_post_types( $public_only = true, $output = 'names', $exclude = array( 'attachment', 'newsletter' ) ) {
		...
		return apply_filters( 'bulkmail_dynamic_post_types', $this->get_post_types( $public_only, $output, $exclude ), $output );
	...
	public function feed( $url, $item = null, $cache_duration = null ) {
		...
		if ( ! ( $posts = bulkmail_cache_get( 'feed_' . $feed_id ) ) ) {
			...
			if ( is_null( $cache_duration ) ) {
				$cache_duration = apply_filters( 'bulkmail_feed_cache_duration', 360 );
			}

			if ( ! $cache_duration || false === ( $body = get_transient( 'bulkmail_feed_' . $feed_id ) ) ) {

				$response = wp_remote_get( $url, array( 'timeout' => 10 ) );
				$code     = wp_remote_retrieve_response_code( $response );

				if ( $code != 200 ) {
					$response = new WP_Error( 'bulkmail-feed-error', sprintf( esc_html__( 'The server responded with error code %d.', 'bulkmail' ), $code ) );
				}

				if ( is_wp_error( $response ) ) {
					if ( ! is_admin() ) {
						bulkmail_notice( sprintf( esc_html__( 'There\'s a problem receiving the feed from `%1$s`: %2$s', 'bulkmail' ), $url, $response->get_error_message() ), 'error', $cache_duration, $feed_id );
					}
					return $response;
				}

				bulkmail_remove_notice( $feed_id );
				...
				set_transient( 'bulkmail_feed_' . $feed_id, $body, $cache_duration );
			...
			$feed->strip_htmltags( apply_filters( 'bulkmail_feed_strip_htmltags', $feed->strip_htmltags ) );
			$feed->strip_attributes( apply_filters( 'bulkmail_feed_strip_attributes', $feed->strip_attributes ) );

			$feed->init();
			$feed->set_output_encoding( get_option( 'blog_charset' ) );

			if ( $feed->error() ) {
				set_transient( 'bulkmail_feed_' . $feed_id, $body, 0 );
			...
			$max_items = apply_filters( 'bulkmail_feed_max_items', 100 );
			$max_items = @$feed->get_item_quantity( (int) $max_items );

			if ( $item >= $max_items ) {
				return new WP_Error( 'feed_to_short', sprintf( esc_html__( 'The feed only contains %d items', 'bulkmail' ), $max_items ) );
			}
			...
			bulkmail_cache_set( 'feed_' . $feed_id, $posts );
		}

		if ( ! is_null( $item ) ) {
			return isset( $posts[ $item ] ) ? $posts[ $item ] : new WP_Error( 'no_item', sprintf( esc_html__( 'Feed item #%d does not exist', 'bulkmail' ), $item ) );
		}
	...
	public function get_meta_tags_from_url( $url, $fields = null, $force = false ) {
		...
		$tags      = null;
		$cache_key = 'bulkmail_meta_tags_' . md5( $url );
	...
	public function get_excerpt( $org_string, $length = null, $more = null ) {
		...
		$excerpt = apply_filters( 'mymail_pre_get_excerpt', apply_filters( 'bulkmail_pre_get_excerpt', null, $org_string, $length, $more ), $org_string, $length, $more );
		if ( is_string( $excerpt ) ) {
			return $excerpt;
		}

		$stripped_string = bulkmail( 'helper' )->handle_shortcodes( $org_string );

		$string            = str_replace( "\n", '<!--Bulkmail:newline-->', $stripped_string );
		$string            = html_entity_decode( wp_trim_words( htmlentities( $string ), $length, $more ) );
		$maybe_broken_html = str_replace( '<!--Bulkmail:newline-->', "\n", $string );
		...
		return apply_filters( 'mymail_get_excerpt', apply_filters( 'bulkmail_get_excerpt', $excerpt, $org_string, $length, $more ), $org_string, $length, $more );
	...
	public function plain_text( $html, $linksonly = false ) {
		...
		$result = apply_filters( 'mymail_plain_text', apply_filters( 'bulkmail_plain_text', null, $html, $linksonly ), $html, $linksonly );
		if ( ! is_null( $result ) ) {
			return $result;
		}
		...
		if ( $linksonly ) {
			...
			require_once BULKEMAIL_DIR . 'classes/libs/class.html2text.php';
			$htmlconverter = new \BulkmailHtml2Text\Html2Text(
				$html,
				array(
					'width'    => 200,
					'do_links' => 'table',
	...
	public function dialog( $content, $args = array() ) {
		...
		if ( $is_file = is_file( BULKEMAIL_DIR . 'views/dialogs/' . basename( $content ) . '.php' ) ) {
			$content = BULKEMAIL_DIR . 'views/dialogs/' . basename( $content ) . '.php';
		}

		$defaults = array(
			'id'           => uniqid(),
			'button_label' => esc_html__( 'Ok, got it!', 'bulkmail' ),
			'classes'      => array(),
		);
		...
		$args['id'] = 'bulkmail-' . $args['id'];

		$suffix = SCRIPT_DEBUG ? '' : '.min';

		wp_enqueue_script( 'bulkmail-dialog', BULKEMAIL_URI . 'assets/js/dialog-script' . $suffix . '.js', array( 'bulkmail-script' ), BULKEMAIL_VERSION, true );
		wp_enqueue_style( 'bulkmail-dialog', BULKEMAIL_URI . 'assets/css/dialog-style' . $suffix . '.css', array(), BULKEMAIL_VERSION );

		?>
		<div id="<?php echo esc_attr( $args['id'] ); ?>" class="bulkmail-notification-dialog notification-dialog-wrap <?php echo esc_attr( $args['id'] ); ?> hidden <?php echo implode( ' ', $args['classes'] ); ?>">
	...
	public function unsplash( $command, $args = array() ) {
		...
		$key = sanitize_key( apply_filters( 'bulkmail_unsplash_client_id', 'ba3e2af91c8c44d00cb70fe6217dcf021f7350633c323876ffa561a1dfbfc25f' ) );
		...
		switch ( $command ) {
			...
			case 'download_url':
				...
				break;
			default:
				return new WP_Error( 'err', esc_html__( 'Command not supported', 'bulkmail' ) );
				break;
		}
		...
		$cache_key = 'bulkmail_unsplash_' . md5( $url );
