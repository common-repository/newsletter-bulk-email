...
class BulkmailTranslations {

	private $endpoint = 'https://emailmarketing.run/';
	...
	public function load() {
		if ( is_dir( BULKEMAIL_UPLOAD_DIR . '/languages' ) ) {
			$custom = BULKEMAIL_UPLOAD_DIR . '/languages/bulkmail-' . get_locale() . '.mo';
			if ( file_exists( $custom ) ) {
				load_textdomain( 'bulkmail', $custom );
			} else {
				load_plugin_textdomain( 'bulkmail' );
			}
		} else {
			load_plugin_textdomain( 'bulkmail' );
		}
	...
	public function get_translation_set( $force = false ) {
		...
		$object = get_option( 'bulkmail_translation' );
	...
	public function get_translation_data( $force = false ) {
		...
		if ( $force || ! $object || $now - $object['expires'] >= 0 ) {
			...
			if ( 'en_US' == $locale ) {
				update_option( 'bulkmail_translation', array( 'expires' => $now + DAY_IN_SECONDS ) );
				return false;
			}
			...
			$file    = 'bulkmail-' . $locale;
			$url     = $this->endpoint . '/api/projects/bulkmail';
			$package = $this->endpoint . '/api/get/bulkmail/' . $locale;
			...
			if ( empty( $body ) || 200 != wp_remote_retrieve_response_code( $response ) ) {
				...
				update_option( 'bulkmail_translation', $object );
				return false;
			}
			...
			if ( $translation_set && $lastmodified - $filemtime > 0 ) {
				$object['data'] = array(
					...
					'current'    => $filemtime,
					'package'    => $package,
					'autoupdate' => (bool) bulkmail_option( 'autoupdate' ),
				);
			}
	...
	public function on_activate( $new ) {
		...
		try {
			...
			bulkmail( 'settings' )->define_texts( true );
		} catch ( Exception $e ) {
		}
