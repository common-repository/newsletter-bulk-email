...
class BulkmailTests {
	...
	public function get_tests() {
		...
		return apply_filters( 'bulkmail_tests', $tests );
		...
		foreach ( array( 'error', 'warning', 'notice', 'success' ) as $type ) {
			...
			foreach ( $this->errors[ $type ] as $test_id => $test_errors ) {
				...
				foreach ( $test_errors as $i => $error ) {
					...
					$html .= '<div class="bulkmail-test-result bulkmail-test-is-' . $type . '"><h4>' . $name . ( $error['data']['link'] ? ' (<a class="bulkmail-test-result-link external" href="' . esc_url( $error['data']['link'] ) . '">' . esc_html__( 'More Info', 'bulkmail' ) . '</a>)' : '' ) . ' <a class="retest bulkmail-icon" href="' . add_query_arg( array( 'test' => $test_id ), admin_url( 'edit.php?post_type=newsletter&page=bulkmail_tests&autostart' ) ) . '">' . esc_html__( 'Test again', 'bulkmail' ) . '</a></h4><div class="bulkmail-test-result-more">' . nl2br( $error['msg'] ) . '</div></div>';
					if ( $type != 'success' ) {
	...
	private function test_php_version() {
		if ( version_compare( PHP_VERSION, '5.3' ) < 0 ) {
			$this->error( sprintf( 'Bulkmail requires PHP version 5.3 or higher. Your current version is %s. Please update or ask your hosting provider to help you updating.', PHP_VERSION ) );
		} elseif ( version_compare( PHP_VERSION, '7.0' ) < 0 ) {
			$this->notice( sprintf( 'Bulkmail recommends PHP version 7.0 or higher. Your current version is %s. Please update or ask your hosting provider to help you updating.', PHP_VERSION ) );
		} else {
	...
	private function test_wordpress_version() {
		...
		if ( version_compare( $current, '3.8' ) < 0 ) {
			$this->error( sprintf( 'Bulkmail requires WordPress version 3.8 or higher. Your current version is %s.', $current ) );
	...
	private function test_verfied_installation() {
		...
		if ( ! bulkmail()->is_verified() ) {
			$this->error( 'Your Bulkmail installation is not verified! Please register via your <a href="' . admin_url( 'admin.php?page=bulkmail_dashboard' ) . '">dashboard</a>.' );
		} else {
	...
	private function test_update_available() {
		...
		if ( $plugin_info->update ) {
			$this->warning( sprintf( 'A new version of Bulkmail is available! Please %1$s to version %2$s', '<a href="update.php?action=upgrade-plugin&plugin=' . urlencode( BULKEMAIL_SLUG ) . '&_wpnonce=' . wp_create_nonce( 'upgrade-plugin_' . BULKEMAIL_SLUG ) . '">update now</a>', $plugin_info->new_version ) );
		} else {
			$this->success( 'You are running the latest version of Bulkmail!' );
		}

	}
	private function test_content_directory() {
		$content_dir = BULKEMAIL_UPLOAD_DIR;
		...
		if ( ! is_dir( $content_dir ) || ! wp_is_writable( $content_dir ) ) {
			$this->warning( sprintf( 'Your Bulkmail content folder in %s is not writable.', '"' . $content_dir . '"' ) );
		} else {
			$this->success( sprintf( 'Your Bulkmail content folder in %s is writable.', '"' . $content_dir . '"' ) );
		}
	...
	private function test_default_template() {
		...
		$template_dir = trailingslashit( BULKEMAIL_UPLOAD_DIR ) . 'templates/' . $template;
		...
		if ( ! is_dir( $template_dir ) || ! wp_is_writable( $template_dir ) ) {
			$default_template_dir = trailingslashit( BULKEMAIL_UPLOAD_DIR ) . 'templates/' . $default;
			if ( ! is_dir( $default_template_dir ) || ! wp_is_writable( $default_template_dir ) ) {
				$result = bulkmail( 'templates' )->renew_default_template( $default );
				if ( is_wp_error( $result ) ) {
					...
					if ( bulkmail_update_option( 'default_template', $default ) ) {
	...
	private function test_deprecated_hooks() {
		...
		if ( ! empty( $hooks ) ) {
			...
			foreach ( $hooks as $hook ) {
				...
				$msg .= '<li><code>' . $hook . '</code> => <code>' . str_replace( 'mymail', 'bulkmail', $hook ) . '</code>';
	...
	private function _test_tinymce_access() {
		...
		if ( is_wp_error( $response ) ) {
			$this->warning( sprintf( 'The Bulkmail Editor requires TinyMCE and access to the file %1$s which seems to be blocked by your host. [%2$s]', '"' . $file . '"', $response->get_error_message() ) );
		} elseif ( $code != 200 ) {
			$this->warning( sprintf( 'The Bulkmail Editor requires TinyMCE and access to the file %1$s which seems to be blocked by your host. [Error Code %2$s]', '"' . $file . '"', $code ) );
		}

	}
	private function test_custom_language() {
		if ( file_exists( $custom = BULKEMAIL_UPLOAD_DIR . '/languages/bulkmail-' . get_locale() . '.mo' ) ) {
	...
	private function test_geo_database() {
		...
		if ( bulkmail_option( 'track_location' ) ) {
			...
			if ( ! empty( $warnings ) ) {
				$this->warning( implode( '<br>', $warnings ), admin_url( 'edit.php?post_type=newsletter&page=bulkmail_settings&bulkmail_remove_notice=newsletter_homepage#privacy' ) );
			}
		}
	}
	private function test_wp_debug() {
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			$this->failure( ( bulkmail_is_local() ? 'notice' : 'warning' ), 'WP_DEBUG is enabled and should be disabled on a production site.', 'https://codex.wordpress.org/WP_DEBUG' );
		}
	}
	private function test_dom_document_extension() {
		if ( ! class_exists( 'DOMDocument' ) ) {
			$this->error( 'Bulkmail requires the <a href="https://php.net/manual/en/class.domdocument.php" target="_blank" rel="noopener">DOMDocument</a> library.' );
		}
	}
	...
	private function test_database_structure() {
		...
		$set_charset = true;
		$result      = bulkmail()->dbstructure( false, true, $set_charset, false );
		...
		if ( true !== $result ) {
			$second_result = bulkmail()->dbstructure( false, true, $set_charset, false );
		...
		$wpdb->query( $wpdb->prepare( "ALTER TABLE {$wpdb->prefix}bulkmail_subscribers AUTO_INCREMENT = %d", 1 ) );
		$wpdb->query( $wpdb->prepare( "ALTER TABLE {$wpdb->prefix}bulkmail_forms AUTO_INCREMENT = %d", 1 ) );
		$wpdb->query( $wpdb->prepare( "ALTER TABLE {$wpdb->prefix}bulkmail_lists AUTO_INCREMENT = %d", 1 ) );
		$wpdb->query( $wpdb->prepare( "ALTER TABLE {$wpdb->prefix}bulkmail_links AUTO_INCREMENT = %d", 1 ) );
	...
	private function test_memory_limit() {
		...
		if ( $max < 128 ) {
			$this->warning( 'Your Memory Limit is ' . size_format( $max * 1048576 ) . ', Bulkmail recommends at least 128 MB' );
		} else {
			$this->success( 'Your Memory Limit is ' . size_format( $max * 1048576 ) );
		}
	}
	private function test_plugin_location() {
		if ( BULKEMAIL_SLUG != 'bulkmail/bulkmail.php' ) {
			$this->warning( 'You have changed the plugin location of Bulkmail. This can cause problems while updating the plugin.' );
		}
	}
	private function test_bulkmail_folder_in_root() {
		if ( file_exists( ABSPATH . 'bulkmail' ) || is_dir( ABSPATH . 'bulkmail' ) ) {
			$this->error( 'There\'s a file or folder called \'bulkmail\' in ' . ABSPATH . ' which causes a conflict with campaign links! Please remove or rename this folder.' );
		} else {
			$this->success( 'There\'s no file or folder called \'bulkmail\' in ' . ABSPATH . '.' );
		}
	}
	private function test_working_cron() {

		$cron_status = bulkmail( 'cron' )->check( true );

		if ( is_wp_error( $cron_status ) ) {
			switch ( $cron_status->get_error_code() ) {
				case 'cron_error':
					$this->error( $cron_status->get_error_message(), 'https://emailmarketing.run/' );
					break;
				default:
					$this->warning( $cron_status->get_error_message(), 'https://emailmarketing.run/' );
					break;
			}
		} else {
			if ( $last_hit = get_option( 'bulkmail_cron_lasthit' ) ) {
				$this->success( sprintf( esc_html__( 'Last hit was %s ago', 'bulkmail' ), human_time_diff( $last_hit['timestamp'] ) ) );
			}
		}
		return;
	}
	private function test_cron_lock() {
		...
		if ( ! bulkmail( 'cron' )->is_locked() ) {
			$this->warning( 'Cron Lock mechanism is not working with the current method.', 'https://emailmarketing.run/' );
		} else {
	...
	private function test_mail_throughput() {
		if ( $last_hit = get_option( 'bulkmail_cron_lasthit' ) ) {
			...
			$mails_per_sec = sprintf( esc_html__( _n( '%s mail per second', '%s mails per second', $mails_per_sec, 'bulkmail' ) ), $mails_per_sec );

			if ( $last_hit['mail'] > 1 ) {
				$this->warning( 'Your mail throughput is low. (' . $mails_per_sec . ')', 'https://emailmarketing.run/' );
			} else {
				$this->success( 'Your mail throughput is ok. (' . $mails_per_sec . ')', 'https://emailmarketing.run/' );
			}
		}
	}
	private function test_newsletter_homepage() {

		$hp = get_post( bulkmail_option( 'homepage' ) );

		if ( ! $hp || $hp->post_status == 'trash' ) {

			$this->error( sprintf( esc_html__( 'You haven\'t defined a homepage for the newsletter. This is required to make the subscription form work correctly. Please check the %1$s or %2$s.', 'bulkmail' ), '<a href="edit.php?post_type=newsletter&page=bulkmail_settings&bulkmail_remove_notice=newsletter_homepage#frontend">' . esc_html__( 'frontend settings page', 'bulkmail' ) . '</a>', '<a href="' . add_query_arg( 'bulkmail_create_homepage', wp_create_nonce( 'bulkmail_create_homepage' ), admin_url() ) . '">' . esc_html__( 'create it right now', 'bulkmail' ) . '</a>' ), 'https://emailmarketing.run/' );
			return;

		} elseif ( $hp->post_status != 'publish' ) {

			$this->error( sprintf( esc_html__( 'Your newsletter homepage is not visible. Please update %s.', 'bulkmail' ), '<a href="post.php?post=' . $hp->ID . '&action=edit&bulkmail_remove_notice=newsletter_homepage">' . esc_html__( 'this page', 'bulkmail' ) . '</a>' ), 'https://emailmarketing.run/' );
		...
		if ( ! preg_match( '#\[newsletter_signup\]#', $hp->post_content )
			...
			$this->error( sprintf( esc_html__( 'Your newsletter homepage is not setup correctly. Please update %s.', 'bulkmail' ), '<a href="post.php?post=' . $hp->ID . '&action=edit">' . esc_html__( 'this page', 'bulkmail' ) . '</a>' ), 'https://emailmarketing.run/' );

		}

		if ( preg_match( '#\[newsletter_signup_form id="?(\d+)"?#i', $hp->post_content, $matches ) ) {
			$form_id = (int) $matches[1];
			if ( ! bulkmail( 'forms' )->get( $form_id ) ) {
				$this->error( sprintf( esc_html__( 'The form with id %1$s doesn\'t exist. Please update %2$s.', 'bulkmail' ), $form_id . ' (<code>' . $matches[0] . ']</code>)', '<a href="post.php?post=' . $hp->ID . '&action=edit">' . esc_html__( 'this page', 'bulkmail' ) . '</a>' ), 'https://emailmarketing.run/' );
			}
		}
	...
	private function test_form_exist() {
		...
		if ( ! count( $forms ) ) {
			$this->error( sprintf( esc_html__( 'You have no form! Bulkmail requires at least one form for the newsletter homepage. %s.', 'bulkmail' ), '<a href="edit.php?post_type=newsletter&page=bulkmail_forms&new">' . esc_html__( 'Create a new form now', 'bulkmail' ) . '</a>' ) );
		}
	...
	private function test_update_server_connection() {
		...
		$response = wp_remote_post( apply_filters( 'bulkmail_updatecenter_endpoint', 'https://emailmarketing.run/' ) );
		$code     = wp_remote_retrieve_response_code( $response );

		if ( is_wp_error( $response ) ) {
			$this->error( $response->get_error_message() . ' - Please allow connection to emailmarketing.run!' );
	...
	private function test_mailfunction() {
		...
		$subject = 'This is a test mail from the Bulkmail Test page';
		$message = 'This test message can sent from ' . admin_url( 'edit.php?post_type=newsletter&page=bulkmail_tests' ) . ' and can get deleted.';
		...
		if ( ! $mail->send_notification( 'Sendtest', $message, array( 'notification' => '' ), false ) ) {
			...
			if ( false !== stripos( $error_message, 'smtp connect()' ) ) {
				$this->error( $msg . '<br>' . $error_message, 'https://emailmarketing.run/' );
		...
		if ( bulkmail_option( 'system_mail' ) ) {
	...
	public function wp_mail_failed( $error ) {
		...
		$msg           = 'You are not able to use <code>wp_mail()</code> with Bulkmail';
	...
	private function test_db_version() {
		if ( get_option( 'bulkmail_dbversion' ) != BULKEMAIL_DBVERSION ) {
			$this->error( 'Your current DB version is ' . get_option( 'bulkmail_dbversion' ) . ' and should be ' . BULKEMAIL_DBVERSION . '. Please visit the <a href="' . admin_url( 'admin.php?page=bulkmail_update' ) . '">update page</a> to run necessary updates.' );

		}
	}
	private function test_delivery_port() {
		if ( 'smtp' == bulkmail_option( 'deliverymethod' ) ) {
			$this->port_test( bulkmail_option( 'smtp_port' ), bulkmail_option( 'smtp_host' ), true );
		}
	}
	private function test_bounce_port() {

		if ( bulkmail_option( 'bounce_active' ) ) {
			$this->port_test( bulkmail_option( 'bounce_port' ), bulkmail_option( 'bounce_server' ), true );
		}
	}
	...
	private function test_permalink_structure() {
		...
		} elseif ( ! bulkmail()->check_link_structure() ) {
			$this->error( 'A problem with you permalink structure. Please check the slugs on the <a href="' . admin_url( 'admin.php?page=bulkmail_settings#frontend' ) . '">frontend tab</a>.' );
		}
	...
	private function port_test( $port, $domain, $strict = false ) {
		...
		$result = bulkmail( 'settings' )->check_port( $domain, $port );
