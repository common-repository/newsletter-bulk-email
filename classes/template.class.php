...
class BulkmailTemplate {
	...
	public function load_template( $slug = '' ) {
		...
		if ( 'mymail' == $slug && ! file_exists( $file ) ) {
			bulkmail( 'templates' )->renew_default_template( 'mymail' );
		}
		...
		if ( file_exists( $file ) ) {
			...
			$data = str_replace( '//dummy.newsletter-plugin.com/', '//dummy.bulkmail.co/', $data );
		} else {
		...
		if ( $logo_id = bulkmail_option( 'logo' ) ) {
			$xpath     = new DOMXPath( $doc );
			$logos     = $xpath->query( '//*/img[@label="Logo" or @label="logo" or @label="Your Logo"]' );
			$high_dpi  = bulkmail_option( 'high_dpi' ) ? 2 : 1;
			$logo_link = bulkmail_option( 'logo_link' );
			...
			foreach ( $logos as $logo ) {
				...
				$new_logo = bulkmail( 'helper' )->create_image( $logo_id, null, $width * $high_dpi, null, false );
		...
		if ( $services = bulkmail_option( 'services' ) ) {
			...
			if ( $buttons->length ) {
				...
				if ( ! is_dir( $base_path ) ) {
					$base_path = BULKEMAIL_DIR . 'templates/mymail/img/social/';
					$base_url  = BULKEMAIL_URI . 'templates/mymail/img/social/';
				}
				...
				foreach ( $services as $service => $username ) {
					...
					$url = bulkmail( 'helper' )->get_social_link( $service, $username );
					...
					$img->setAttribute( 'alt', esc_attr( sprintf( esc_html__( 'Share this on %s', 'bulkmail' ), ucwords( $service ) ) ) );
	...
	public function remove_template( $slug = '' ) {
		...
		if ( $wp_filesystem->delete( $this->templatepath, true ) ) {
			bulkmail( 'templates' )->remove_screenshot( $slug );
			return true;
		}
	...
	public function upload_template() {
		...
		$tempfolder = BULKEMAIL_UPLOAD_DIR . '/uploads';

		wp_mkdir_p( $tempfolder );

		return bulkmail( 'templates' )->unzip_template( $result['file'], $tempfolder );
	...
	public function create_new( $name, $content = '', $modules = true, $active = false, $overwrite = false ) {
		...
		if ( $name == esc_html__( 'Base', 'bulkmail' ) ) {
			$filename = 'index.html';
		}

		if ( $name == esc_html__( 'Notification', 'bulkmail' ) ) {
		...
		$content = bulkmail()->sanitize_content( $content );
	...
	public function get_module_list( $activeonly = false ) {
		...
		$labels = array(
			'full size image'          => esc_html_x( 'Full Size Image', 'common module name', 'bulkmail' ),
			'intro'                    => esc_html_x( 'Intro', 'common module name', 'bulkmail' ),
			'separator'                => esc_html_x( 'Separator', 'common module name', 'bulkmail' ),
			'separator with button'    => esc_html_x( 'Separator with button', 'common module name', 'bulkmail' ),
			'full size text invert'    => esc_html_x( 'Full Size Text Invert', 'common module name', 'bulkmail' ),
			'iphone promotion'         => esc_html_x( 'iPhone Promotion', 'common module name', 'bulkmail' ),
			'macbook promotion'        => esc_html_x( 'Macbook Promotion', 'common module name', 'bulkmail' ),
			'quotation'                => esc_html_x( 'Quotation', 'common module name', 'bulkmail' ),
			'quotation left'           => esc_html_x( 'Quotation left', 'common module name', 'bulkmail' ),
			'quotation right'          => esc_html_x( 'Quotation right', 'common module name', 'bulkmail' ),
			'plans'                    => esc_html_x( 'Plans', 'common module name', 'bulkmail' ),
			'1/2 image full'           => sprintf( esc_html_x( '%s Image Full', 'common module name', 'bulkmail' ), '&#xBD;' ),
			'1/2 text invert'          => sprintf( esc_html_x( '%s Text Invert', 'common module name', 'bulkmail' ), '&#xBD;' ),
			'1/3 image full'           => sprintf( esc_html_x( '%s Image Full', 'common module name', 'bulkmail' ), '&#x2153;' ),
			'1/3 text invert'          => sprintf( esc_html_x( '%s Text Invert', 'common module name', 'bulkmail' ), '&#x2153;' ),
			'1/4 image full'           => sprintf( esc_html_x( '%s Image Full', 'common module name', 'bulkmail' ), '&#xBC;' ),
			'1/4 text invert'          => sprintf( esc_html_x( '%s Text Invert', 'common module name', 'bulkmail' ), '&#xBC;' ),
			'image on the left'        => esc_html_x( 'Image on the Left', 'common module name', 'bulkmail' ),
			'image on the right'       => esc_html_x( 'Image on the Right', 'common module name', 'bulkmail' ),
			'1/2 image on the left'    => sprintf( esc_html_x( '%s Image on the Left', 'common module name', 'bulkmail' ), '&#xBD;' ),
			'1/2 image on the right'   => sprintf( esc_html_x( '%s Image on the Right', 'common module name', 'bulkmail' ), '&#xBD;' ),
			'1/3 image on the left'    => sprintf( esc_html_x( '%s Image on the Left', 'common module name', 'bulkmail' ), '&#x2153;' ),
			'1/3 image on the right'   => sprintf( esc_html_x( '%s Image on the Right', 'common module name', 'bulkmail' ), '&#x2153;' ),
			'1/4 image on the left'    => sprintf( esc_html_x( '%s Image on the Left', 'common module name', 'bulkmail' ), '&#xBC;' ),
			'1/4 image on the right'   => sprintf( esc_html_x( '%s Image on the Right', 'common module name', 'bulkmail' ), '&#xBC;' ),
			'1/2 floating image left'  => sprintf( esc_html_x( '%s Floating Image left', 'common module name', 'bulkmail' ), '&#xBD;' ),
			'1/2 floating image right' => sprintf( esc_html_x( '%s Floating Image right', 'common module name', 'bulkmail' ), '&#xBD;' ),
			'1/2 image features left'  => sprintf( esc_html_x( '%s Image Features left', 'common module name', 'bulkmail' ), '&#xBD;' ),
			'1/2 image features right' => sprintf( esc_html_x( '%s Image Features right', 'common module name', 'bulkmail' ), '&#xBD;' ),
			'1/3 image features left'  => sprintf( esc_html_x( '%s Image Features left', 'common module name', 'bulkmail' ), '&#x2153;' ),
			'1/3 image features right' => sprintf( esc_html_x( '%s Image Features right', 'common module name', 'bulkmail' ), '&#x2153;' ),
			'1/1 text'                 => sprintf( esc_html_x( '%s Text', 'common module name', 'bulkmail' ), '1/1' ),
			'1/2 text'                 => sprintf( esc_html_x( '%s Text', 'common module name', 'bulkmail' ), '&#xBD;' ),
			'1/3 text'                 => sprintf( esc_html_x( '%s Text', 'common module name', 'bulkmail' ), '&#x2153;' ),
			'1/4 text'                 => sprintf( esc_html_x( '%s Text', 'common module name', 'bulkmail' ), '&#xBC;' ),
			'1/1 column'               => sprintf( esc_html_x( '%s Text', 'common module name', 'bulkmail' ), '1/1' ),
			'1/2 column'               => sprintf( esc_html_x( '%s Text', 'common module name', 'bulkmail' ), '&#xBD;' ),
			'1/3 column'               => sprintf( esc_html_x( '%s Text', 'common module name', 'bulkmail' ), '&#x2153;' ),
			'1/4 column'               => sprintf( esc_html_x( '%s Text', 'common module name', 'bulkmail' ), '&#xBC;' ),
		);
		...
		for ( $i = 0; $i < $count; $i++ ) {
			...
			if ( isset( $labels[ strtolower( $label ) ] ) ) {
				...
				$label = sprintf( esc_html__( 'Module %s', 'bulkmail' ), '#' . ( $i + 1 ) );
			}
			$list[] = array(
				'name' => $label,
	...
	public function get_updates() {
		$updates = get_site_transient( 'bulkmail_updates' );
	...
	public function buttons( $basefolder = 'img' ) {
		...
		// common_button_folder_names in use for __($name, 'bulkmail')
		esc_html__( 'light', 'bulkmail' );
		esc_html__( 'dark', 'bulkmail' );
		...
		<div class="button-nav-wrap">
			...
			foreach ( $files as $file ) {
				if ( is_dir( $file ) ) {
					...
					$name      = basename( $file );
					$folders[] = $name;
					$nav      .= '<a class="nav-tab" href="#buttons-' . $id . '-' . $name . '">' . __( $name, 'bulkmail' ) . '</a>';
	...
	public function list_buttons( $folder, $id ) {
		...
		foreach ( $files as $file ) {
			...
			$btn     .= '<li><a class="btnsrc" title="' . $service . '" data-link="' . bulkmail( 'helper' )->get_social_link( $service, 'USERNAME' ) . '"><img src="' . str_replace( $this->path . '/', $this->url . '/', $file ) . '"></a></li>';
	...
	public function get_module_screenshots( $slug = null, $file = null ) {
		...
		$screenshot_modules_folder     = BULKEMAIL_UPLOAD_DIR . '/screenshots/' . $slug . '/modules/' . $hash;
		$screenshot_modules_folder_uri = BULKEMAIL_UPLOAD_URI . '/screenshots/' . $slug . '/modules/' . $hash;

		if ( ! is_dir( $screenshot_modules_folder ) ) {

			bulkmail( 'templates' )->schedule_screenshot( $slug, $file, true, 0, false );
		...
		if ( count( $return ) < $modules->length ) {
			bulkmail( 'templates' )->schedule_screenshot( $slug, $file, true, 10, false );
		}
	...
	private function get_template_data( $file ) {
		...
		return bulkmail( 'templates' )->get_template_data( $file );
